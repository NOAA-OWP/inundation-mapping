...
def preprocess_nws(source_dir, destination, reference_raster):
    ...
    for code in ahps_codes:
        ...
        if code in ['bgwn7', 'smit2']:
            [grids_dir] = [
                directory for directory in parent_path.glob('*custom*') if directory.is_dir()
            ]
        else:
            # Find the directory containing depth grids. Assumes only one directory will be returned.
            [grids_dir] = [
                directory for directory in parent_path.glob('*depth_grid*') if directory.is_dir()
            ]

        # Get grids (all NWS ESRI grids were converted to Geotiff)
        grid_paths = [
            grids for grids in grids_dir.glob('*.tif*') if grids.suffix in ['.tif', '.tiff']
        ]
        ...
        if grid_paths:
            ...
            df = pd.DataFrame(
                {'code': code, 'path': grid_paths, 'name': grid_names, 'datum88': datum88}
            )
