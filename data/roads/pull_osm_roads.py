...
def combine_hucs(output_dir):
    ...
    # Alaska
    if alaska_files:
        alaska_all_roads_gdf = pd.concat(
            [gpd.read_file(gpkg) for gpkg in alaska_files], ignore_index=True)
        alaska_all_roads_gdf["osmid"] = alaska_all_roads_gdf["osmid"].astype(str)
        alaska_all_roads_gdf.to_file(os.path.join(output_dir, "alaska_osm_roads.gpkg"), driver="GPKG", engine='fiona')
        print(f"Compiled Alaska road lines!")
    ...
    #conus
    if conus_files:
        conus_all_roads_gdf = pd.concat(
            [gpd.read_file(gpkg) for gpkg in conus_files], ignore_index=True)
        conus_all_roads_gdf["osmid"] = conus_all_roads_gdf["osmid"].astype(str)
        conus_all_roads_gdf.to_file(os.path.join(output_dir, "conus_osm_roads.gpkg"), driver="GPKG", engine='fiona')
        print(f"Compiled conus road lines!")
...
def pull_roads(HUC_no, huc_geom, file_logger, screen_queue, task_id):
    ...
    for attempt in range(1, max_attempts + 1):
        try:
            ...
            file_logger.warning(f"[{task_id}] Server busy ({e.__class__.__name__}) — retrying in {wait_time:.1f}s (attempt {attempt})")
            ...
            if attempt >= 2: #no need to retry more than twice , because it is most likely due to large size. directly go to split part
                file_logger.warning(f"[{task_id}] Stopping retries due to repeated IncompleteRead or OverpassRuntimeError ")
                break
    ...
    if result:
        road_data = pd.DataFrame([
            ...
            for way in result.ways if len(way.nodes) > 1
        ])
    ...
    if result is None and incomplete_read_occurred:
        file_logger.warning(f"[{task_id}] Splitting HUC {HUC_no} into sub-regions due to repeated IncompleteRead errors.")
        screen_queue.put(f"[{task_id}] Splitting HUC {HUC_no} into sub-regions due to repeated IncompleteRead errors.")
        num_splits=4
            ...
            for attempt in range(1, max_attempts + 1):
                try:
                    ...
                    df = pd.DataFrame([
                        ...
                        for way in sub_result.ways if len(way.nodes) > 1
                    ])
                    all_dfs.append(df)
                    break
                except Exception as e:
...
def split_roads(gdf_roads, catchment_path,file_logger, screen_queue,task_id):
    ...
    #skip splitting for Alaska because there are too many catchments
        ...
        if not gdf_roads_splitted.empty: 
            ...
            gdf_roads_splitted["omsid_catchid"] = gdf_roads_splitted["osmid"].astype(str) + "_" + gdf_roads_splitted["catchment_id"].astype(str)
...
def single_huc_job(HUC_no, huc_boundary_path, split_boundary_path,output_dir,file_logger, screen_queue,task_id):
    ...
    try:
        ...
        if osm_roads is not None:
            ...
            if not osm_roads_clipped.empty:
                splitted_osm_roads = split_roads(osm_roads_clipped, split_boundary_path,file_logger, screen_queue,task_id)
...
def pull_osm_roads(preclip_dir,output_dir,number_jobs):
    ...
    #get list of HUC8s
    huc_numbers = [
        str(huc) for huc in os.listdir(preclip_dir)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        '-o', '--output_dir', help='REQUIRED: folder path for output.', required=True
    )
