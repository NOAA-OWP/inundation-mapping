...
# ***************************************************************
def merge_nwm_streams_with_ripples(metrix_dir, out_dir, ripple_model_name):
    ...
    if os.path.exists(ripple_gpkg):
        ...
        if os.path.exists(nwm_stream_gpkg):
            ...
            huc_out_folder = os.path.join(out_dir, ripple_model_name)
            os.makedirs(huc_out_folder, exist_ok=True)

            # Save as a new GeoPackage
            path_ripple_reaches = os.path.join(
                huc_out_folder, ripple_model_name, f'ripple_reaches_order_sourcemodels_{huc}.gpkg'
            )
...
# ***************************************************************
def merge_ripple_reaches_sourcemodels_with_metrix_db(metrix_dir, out_dir, ripple_model_name):

    huc = re.search(r'\d+', ripple_model_name).group(0)

    # Read metrics geopackage
    path_ripple_collection_out = os.path.join(out_dir, ripple_model_name)
    path_ripple_reaches = os.path.join(
    ...
    log_text = ''
    if not os.path.exists(path_ripple_reaches):
        log_text += merge_nwm_streams_with_ripples(metrix_dir, out_dir, ripple_model_name)
    ...
    if len(db_paths) == 0:
        ...
        log_text += f'{ripple_model_name} does not have any metrics database\n'
        ...
        path_metrix_table_huc = os.path.join(path_ripple_collection_out, f'ripple_reaches_sourcemodels_metrics_{huc}.csv')
        ...
        if os.path.exists(path_ripple_reaches) and os.path.exists(path_metrix_table_huc):
            ...
            path_ripple_reaches_metrix = os.path.join(
                path_ripple_collection_out, f'ripple_reaches_order_source_models_metrix_{huc}.gpkg'
            )
...
# ***************************************************************
def create_ripple_STREAMS_gdf_csv(metrix_dir, out_dir):
    ...
    for rmi in range(len(ripple_models)):
        try:
            ...
            # Read metrics geopackage
            path_ripple_collection_out = os.path.join(out_dir, ripple_models[rmi])
            path_ripple_reaches = os.path.join(
                path_ripple_collection_out, f'ripple_reaches_order_source_models_metrix_{huc}.gpkg'
            )
            ...
            if not os.path.exists(path_ripple_reaches):
                corr_matrix, log_text_m = merge_ripple_reaches_sourcemodels_with_metrix_db(
                    metrix_dir, out_dir, ripple_models[rmi]
                )
                log_text += log_text_m
                ...
                corr_matrix_path = os.path.join(path_ripple_collection_out, f'ripple_reaches_order_corr_{huc}.csv')
            ...
            path_metrics_csv = os.path.join(path_ripple_collection_out, f'ripple_reaches_sourcemodels_metrics_{huc}.csv')
            ...
            if os.path.exists(path_sourcemodels_gpkg):
                ...
                if os.path.exists(path_metrics_csv):
                    ...
                    if 'db_path' in metric_df.columns:
                        ...
                        sourcemodels_df['feature_id'] = pd.to_numeric(sourcemodels_df['feature_id'], errors='coerce').astype('Int64')
                        db_path_df['feature_id'] = pd.to_numeric(db_path_df['feature_id'], errors='coerce').astype('Int64')
            ...
            # Save the gdf
            path_streams_metrix = os.path.join(path_ripple_collection_out, f'streams_metrix_{huc}.gpkg')
            ...
            error_msg = f"Error processing folder {ripple_models[rmi]}: {str(e)}\n"
            print(error_msg)
            log_text += error_msg
            continue
    ...
    model_metrix_corr_path = os.path.join(out_dir, 'model_metrix_corr_new.csv')
    ...
    path_metrix_reaches_conus = os.path.join(out_dir, 'metrix_reaches_ripple_submodels_conus.csv')
    ...
    path_metrix_streams_conus_gpkg = os.path.join(out_dir, 'metrix_streams_ripple_submodels_conus.gpkg')
    ...
    path_metrix_streams_conus = os.path.join(out_dir, 'metrix_streams_ripple_submodels_conus.csv')
...
# ***************************************************************
def process_ripple_STREAMS_create_blackList(metrix_dir, out_dir):

    path_ripple_streams = os.path.join(out_dir, 'metrix_streams_ripple_submodels_conus.csv')
    path_ripple_reaches = os.path.join(out_dir, 'metrix_reaches_ripple_submodels_conus.csv')
    ...
    ripple_streams_metrix_df = gpd.read_csv(path_ripple_streams)
    ...
    if os.path.exists(path_all_sourcemodels_conus):
        ...
        bad_features['feature_id'] = pd.to_numeric(bad_features['feature_id'], errors='coerce').astype('Int64')
        all_sourcemodels_conus_df['feature_id'] = pd.to_numeric(all_sourcemodels_conus_df['feature_id'], errors='coerce').astype('Int64')

        
        # Merge
        merged_all = all_sourcemodels_conus_df.merge(bad_features, on=['collection_id', 'feature_id'], how='left')
...
def apply_ripple_streams_blacklist(metrix_dir, out_dir, log_file_path):  # bankfull_flows_file,
    """
    ...
    try:
        ...
        log_text += msg
        print(msg)
        log_text += process_ripple_STREAMS_create_blackList(metrix_dir, out_dir)
...
# -------------------------------------------------------
def log_create_blacklist(metrix_dir, out_dir):
    """
    ...
    apply_ripple_streams_blacklist(metrix_dir, out_dir, log_file_path)
...
if __name__ == '__main__':
    ...
    parent_dir = os.path.dirname(os.path.normpath(metrix_dir))
    out_dir = os.path.join(parent_dir, 'ripple_metrics')
    os.makedirs(out_dir, exist_ok=True)

    log_create_blacklist(metrix_dir, out_dir)
