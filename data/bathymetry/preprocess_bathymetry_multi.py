...
To use this script you will need to create a tif file from the eHydro depth survey tin file.
    To create depth tif:
        ...
        Optional:
        3. Put all converted rasters and their corresponding survey geodatabases into a folder to be 
        processessed together.
"""



def preprocessing_ehydro(tif, bathy_bounds, survey_gdb, survey_folder, output, min_depth_threshold):
    ...
    area and wetted perimeter by NWM feature_id.
        ...
        survey_folder : str
            Path to folder containing all rasterized tin surveys and their geodatabases, e.g.
            "/data/inputs/bathymetry/ohio_bathymetric_surveys".
        output : str
    ...
    bathy_nwm_streams.to_file(output, driver = 'GPKG', engine="pyogrio", index=False)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-tif', '--tif', help='Survey Depth Raster', required=False, type=str)
    parser.add_argument(
        '-survey_gdb',
        '--survey_gdb',
    ...
    parser.add_argument(
        '-survey_folder',
        '--survey_folder',
        help='Folder containing all survey tif files and geodatabases',
        default=None,
        required=False,
        type=str,
    )
    ...
    survey_folder = args['survey_folder']
    output = args['output']
    min_depth_threshold = args['min_depth_threshold']

    if survey_folder is not None:
        dirs = [x[1] for x in os.walk(f"{survey_folder}")] [0]
        dirs = [x[:-4] for x in dirs]
        for d in dirs:
            print (f'preprocessing survey: {d} now...')
            tif_1 = f'{survey_folder}/{d}.tif'
            survey_gdb_1 = f'{survey_folder}/{d}.gdb'
            preprocessing_ehydro(tif_1, bathy_bounds, survey_gdb_1, survey_folder, output, min_depth_threshold)
    else:
        preprocessing_ehydro(tif, bathy_bounds, survey_gdb, survey_folder, output, min_depth_threshold)
    print("success :)")
