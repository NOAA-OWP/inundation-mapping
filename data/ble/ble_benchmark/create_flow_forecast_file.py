...
def create_flow_forecast_file(huc,
                              ble_geodatabase,
                              nwm_geodatabase,
                              output_parent_dir, 
        ...
        while True:
            ...
            for i, row in nonintersects.iterrows():
                ...
                if len(downstream) > 0:
                    ...
                    if (downstream['discharge'].isnull().sum() + downstream_upstreams['discharge'].isnull().sum()) == 1: 
                        new_flow.append(downstream['discharge'].iloc[0] - downstream_upstreams['discharge'].sum())

                # Add row data to forecast
                if len(new_flow) > 0:
                    forecast = pd.concat([forecast, pd.DataFrame({'feature_id': row['ID'], 
                                                                  'discharge': np.mean(new_flow)}, 
...
if __name__ == '__main__':
    ...
    parser.add_argument('-b', '--ble-geodatabase', help='BLE geodatabase (.gdb file extension).                               \
        Will look for layer with "XS" in name. It is assumed the 100 year flow field is "E_Q_01PCT"                           \
        and the 500 year flow field is "E_Q_0_2_PCT" as these are the default field names.', required=True)
    parser.add_argument('-n', '--nwm-geodatabase',  help='NWM geodatabase (.gdb file extension).', required=True)
    parser.add_argument('-o', '--output-parent-dir', help='Output directory where forecast files will be stored.              \
        Two subdirectories are created (100yr and 500yr) and in each subdirectory a forecast file is written', required=True)
    parser.add_argument('-xs', '--ble-xs-layer-name', help='BLE cross section layer.                                          \
        Default layer is "XS" (sometimes it is "XS_1D").', required = False, default='XS')
    parser.add_argument('-hu', '--huc', help = 'HUC', required=True)
    parser.add_argument('-l', '--nwm-stream-layer-name', help='NWM streams layer. Default layer is "nwm_streams")',           \
        required=False, default='nwm_streams')
    parser.add_argument('-f', '--nwm-feature-id-field', help='id field for nwm streams.                                       \
        Not required if NWM v2.1 is used (default id field is "ID")', required=False, default='ID')
