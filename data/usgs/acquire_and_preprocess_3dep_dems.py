...
def acquire_and_preprocess_3dep_dems(
    ...
    By us downloading the rasters using WBD huc clips and gdal, we can accomplish a few extra
    steps.
    ...
    if number_of_jobs > total_cpus_available:
        raise ValueError(
            f"The number of jobs {number_of_jobs}"
    ...
    if not os.path.exists(extent_file_path):
        raise ValueError(
            f"extent_file_path value of {extent_file_path}" " not set to a valid path"
        )
...
def retrieve_and_reproject_3dep_for_huc(
    extent_file: str,
    ...
    def __fishnet_geometry(geometry, cell_size_meters=1000, cell_buffer=10):
        ...
        grid_cells = [
            ...
            for x0, y0 in product(
                np.arange(xmin, xmax + cell_size_meters, cell_size_meters),
        ...
        check_intersection = (
            lambda g: (not g.is_empty)
            & g.is_valid
            & (isinstance(g, Polygon) | isinstance(g, MultiPolygon))
        )
    ...
    def __get_tile_from_nhd(geometry, huc):
        # open
        huc = huc[:4]
        nhd_dem_fp = os.path.join(
            f"/data/inputs/nhdplus_rasters/HRNHDPlusRasters{huc}/elev_m.tif"
        )
    ...
    def __retrieve_and_process_single_3dep_dem(
        geometry, dem_resolution, wbd_crs, ndv, target_output_folder_path, huc, idx
    ):
        # make file name
        # dem_file_name = dask.delayed(os.path.join)(target_output_folder_path,f'dem_3dep_{huc}_{i}.tif')
        dem_file_name = os.path.join(
            target_output_folder_path,
            "DEMs",
            f"dem_3dep_{huc}_{int(dem_resolution)}m_{idx}.tif",
        )
        ...
        while True:
            try:
                dem = py3dep.get_map(
                    "DEM", geometry=geometry, resolution=dem_resolution, geo_crs=wbd_crs
                )
                break
        ...
        log_dict = {
            ...
            "huc": huc,
            "retries": retries,
            "NHD Failed": nhd_failed,
            "3DEP Failed": failed_3dep,
        ...
        dem = dem.rio.reproject(
            wbd_crs, resolution=dem_resolution, resampling=Resampling.bilinear
        )
    ...
    def __build_VRT_and_TIFF(input_dict):
        ...
        for huc in tqdm(hucs, desc=" Merging tiles"):
            ...
            sourceFiles = glob.glob(
                os.path.join(target_output_folder_path, "DEMs", "*.tif")
            )
            destVRT = os.path.join(
                target_output_folder_path,
                f"dem_{dem_resolution}m_3dep_{huc}_at_{vrt_resolution}m.vrt",
            )
            ...
            merge_tiff = False
            if merge_tiff:
                destTiff = os.path.join(
                    target_output_folder_path, f"dem_3dep_{huc}.tif"
                )
    ...
    log_file_dir = os.path.join(
        target_output_folder_path, f"{huc}_{int(dem_resolution)}m"
    )
    ...
    if retry:
        ...
        if os.path.exists(log_file_path):
            ...
            input_log.loc[:, "geometry"] = input_log.loc[:, "geometry"].apply(
                convert_to_shape
            )

            input_log = gpd.GeoDataFrame(input_log, geometry="geometry")

            input_log = input_log.loc[
                :,
                [
                    "geometry",
                    "dem_resolution",
    ...
    operations = [
        dask.delayed(__retrieve_and_process_single_3dep_dem)(*inputs)
        for inputs in zip(*input_dict)
    ]
    ...
    if retry:
        if os.path.exists(log_file_path):
            input_log_save = input_log_save.loc[
                ~input_log_save.loc[:, "idx"].isin(output_log.loc[:, "idx"]), :
            ]
        ...
        print(
            f"3DEP failed tiles: {failed_3dep} | NHD failed tiles: {failed_nhd} | Both failed tiles: {failed_both}"
        )
        ...
        for idx, extent_file in enumerate(extent_files):
            ...
            try:
                ...
                future = executor.submit(
                    retrieve_and_reproject_3dep_for_huc, **download_dem_args
                )
...
def polygonize(extent_file_names, target_output_folder_path, dem_resolution):
    ...
    def __polygonize_single(dem_file):
        ...
        if not os.path.exists(edge_gpkg):
            ...
            subprocess.run(
                ["gdal_polygonize.py", "-8", edge_tif, "-q", "-f", "GPKG", edge_gpkg]
            )
    ...
    dem_files = glob.glob(
        os.path.join(
            target_output_folder_path, "DEMs", f"dem_3dep_*_{dem_resolution}m_*.tif"
        )
    )
