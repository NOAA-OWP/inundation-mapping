...
def get_threshold_data(huc, huc_path, valid_nwm_lids):
    ...
    nws_lid_col = thresholds_merged_df.pop('nws_lid')
    thresholds_merged_df.insert(0, 'nws_lid', nws_lid_col)
...
def process_theshold_data(catfim_type, valid_nwm_lids, sites_gdf, huc, huc_path, thresholds_merged_df, metadata_json):
    ...
    # The threshold_emrged_df is for this huc only and has some extra columns but the main ones are in the pattern of
    #    action_stage, action_flow, minor_stage, minor_flow.... etc
    # This keeps clearlly defined valuse for all columns instead of two seperate dictionaries.

    # HUMM... still not sure this is a good idea. TBD
    ...
    for lid in valid_nwm_lids:
        ...
        # shoudl be 0 or 1 record, not more than one. If so.. we have a bug
