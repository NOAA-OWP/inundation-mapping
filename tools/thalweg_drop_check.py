...
def compare_thalweg(args):
    ...
    if stream_type == "derived":
        ...
        for i, point in headwater_list.iterrows():
            ...
            streams.loc[
                streams.min_dist == np.min(streams.min_dist), "nws_lid"
            ] = point.site_id
    ...
    for index, headwater_site in enumerate(headwater_list):
        stream_path = get_downstream_segments(
            streams.copy(),
            "nws_lid",
            headwater_site,
            "downstream",
        ...
        if stream_type == "burnline":
            ...
            nhd_reaches_raster = np.where(
                flow_bool_data == int(0),
                -9999.0,
                (nhd_reaches_raster).astype(rasterio.float32),
            )
            out_dem_filename = os.path.join(huc_dir, "NHDPlusBurnLineEvent_raster.tif")
            with rasterio.open(
                out_dem_filename, "w", **dem_meters.profile, BIGTIFF="YES"
            ) as dest:
            ...
            if stream_type == "derived":
                ...
                if point_density == "midpoints":
                    ...
                    dem_m_elev = dem_m_elev + [
                        np.array(
                            list(dem_meters.sample((Point(midpoint).coords), indexes=1))
                        ).item()
                    ]
                        ...
                        np.array(
                            list(
                                dem_lateral_thalweg_adj.sample(
                                    (Point(midpoint).coords), indexes=1
                                )
                            )
                        ).item()
                    ]
                    ...
                    for point in zip(*linestring.coords.xy):
                        ...
                        dem_m_elev = dem_m_elev + [
                            np.array(
                                list(
                                    dem_meters.sample((Point(point).coords), indexes=1)
                                )
                            ).item()
                        ]
                        dem_lat_thal_adj_elev = dem_lat_thal_adj_elev + [
                ...
                dem_m_elev = dem_m_elev + [
                    np.array(
                        list(
                            dem_meters.sample(
                                (Point(segment.geometry).coords), indexes=1
                            )
                        )
                    ).item()
                ]
    ...
    try:
        ...
        thal_adj_points = thalweg_points.loc[
            thalweg_points.source == "thal_adj_dem"
        ].copy()
        ...
        thal_adj_points["elev_change"] = thal_adj_points.groupby(
            ["headwater_path", "source"]
        )["elevation_m"].apply(lambda x: x - x.shift())
...
def get_downstream_segments(
    streams, headwater_col, headwater_id, flag_column, stream_id, stream_type
):
    ...
    while Q:
        ...
        if stream_type == "burnline":
            ...
            downstream_ids = streams.loc[
                streams["FromNode"] == toNode, :
            ].index.tolist()
            ...
            if (
                len(set(downstream_ids)) > 1
            ):  # special case: remove duplicate NHDPlusIDs

                relevant_ids = [
                    segment
                    for segment in downstream_ids
                    if DnLevelPat == streams.loc[segment, "LevelPathI"]
                ]
...
def plot_profile(elevation_table, profile_plots_filename):
    ...
    if len(unique_rasters) > 1:
        g = sns.FacetGrid(
            ...
            col="headwater_path",
            hue="source",
            hue_order=["dem_m", "dem_lat_thal_adj", "thal_adj_dem"],
            sharex=False,
            sharey=False,
            col_wrap=columns,
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-fim_dir", "--fim-dir", help="FIM output dir", required=True, type=str
    )
    parser.add_argument(
        "-output_dir",
        "--output-dir",
        help="rating curves output folder",
        required=True,
        type=str,
    )
    ...
    parser.add_argument(
        "-stream_type",
        "--stream-type",
        help="stream layer to be evaluated",
        required=True,
        type=str,
        choices=["derived", "burnline"],
    )
    parser.add_argument(
        "-point_density",
        "--point-density",
        help="elevation sampling density",
        required=True,
        type=str,
        choices=["midpoints", "all_points"],
    )
    parser.add_argument(
        "-th",
        "--elevation_threshold",
        help="significant elevation drop threshold in meters.",
        required=True,
    )
    parser.add_argument(
        "-j",
    ...
    for huc in huc_list:
        if huc != "logs":
            ...
            flows_grid_boolean_filename = os.path.join(
                huc_dir, "flows_grid_boolean.tif"
            )
            dem_meters_filename = os.path.join(huc_dir, "dem_meters.tif")
            dem_lateral_thalweg_adj_filename = os.path.join(
                huc_dir, "dem_lateral_thalweg_adj.tif"
            )
            dem_thalwegCond_filename = os.path.join(huc_dir, "dem_thalwegCond.tif")
            profile_plots_filename = os.path.join(
                plots_dir, f"profile_drop_plots_{huc}_{point_density}_{stream_type}.png"
            )
            profile_gpkg_filename = os.path.join(
                spatial_dir,
                f"thalweg_elevation_changes_{huc}_{point_density}_{stream_type}.gpkg",
            )
            profile_table_filename = os.path.join(
                spatial_dir,
                f"thalweg_elevation_changes_{huc}_{point_density}_{stream_type}.csv",
            )
    ...
    print(
        f"Generating thalweg elevation profiles for {len(procs_list)} hucs using {number_of_jobs} jobs"
    )
    ...
    agg_thalweg_elevations_gpkg_fileName = os.path.join(
        output_dir, f"agg_thalweg_elevation_changes_{point_density}_{stream_type}.gpkg"
    )
    agg_thalweg_elevation_table_fileName = os.path.join(
        output_dir, f"agg_thalweg_elevation_changes_{point_density}_{stream_type}.csv"
    )
