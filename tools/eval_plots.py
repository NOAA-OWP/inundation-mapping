...
def filter_dataframe(dataframe, unique_field):
    ...
    unique_sites = dataframe.groupby(["magnitude", "version"])[unique_field].agg(
        "unique"
    )
    ...
    for magnitude in magnitudes:
        ...
        final_filtered_dataframe = final_filtered_dataframe.append(
            filtered_common_sites, ignore_index=True
        )
...
def eval_plots(
    ...
    for (dataset_name, configuration), (dataset, sites) in all_datasets.items():
        ...
        dataset.to_csv(
            output_workspace
            / (f"{dataset_name}_{configuration.lower()}_analyzed_data.csv"),
            index=False,
        )
        sites_pd = pd.DataFrame.from_dict(sites, orient="index").transpose()
        sites_pd.to_csv(
            output_workspace
            / (f"{dataset_name}_{configuration.lower()}_common_sites.csv"),
            index=False,
        )
        ...
        dataset_sums["csi"] = dataset_sums["TP_area_km2"] / (
            dataset_sums["TP_area_km2"]
            + dataset_sums["FP_area_km2"]
            + dataset_sums["FN_area_km2"]
        )
        ...
        dataset_sums.to_csv(
            output_workspace / f"aggregate_{dataset_name}_{configuration.lower()}.csv",
            index=False,
        )
        ...
        for version in versions_list:
            ...
            selected_versions = [
                sel_version
                for sel_version in all_versions
                if sel_version.startswith(version)
            ]
        ...
        aggregate_file = output_workspace / (
            f"csi_aggr_{dataset_name}_{configuration.lower()}.png"
        )
        barplot(
        ...
        if len(version_order) == 2:
            ...
            for magnitude in magnitude_order:
                ...
                x_csi = dataset.query(
                    f'version == "{x_version}" & magnitude == "{magnitude}"'
                )[[base_resolution, "CSI"]]
                y_csi = dataset.query(
                    f'version == "{y_version}" & magnitude == "{magnitude}"'
                )[[base_resolution, "CSI"]]
                plotdf = pd.merge(
                    x_csi,
                    y_csi,
                    on=base_resolution,
                    suffixes=(f"_{x_version}", f"_{y_version}"),
                )
                # Define arguments for scatterplot function
                title_text = f"CSI {magnitude}"
                dest_file = (
                    output_workspace
                    / f"csi_scatter_{magnitude}_{configuration.lower()}.png"
                )
                scatterplot(
                    dataframe=plotdf,
                ...
                plotdf["Diff (C-B)"] = (
                    plotdf[f"CSI_{y_version}"] - plotdf[f"CSI_{x_version}"]
                )
                plotdf.to_csv(
                    output_workspace
                    / f"csi_scatter_{magnitude}_{configuration.lower()}_data.csv",
                    index=False,
                )
    ...
    if spatial:
        ...
        if all_datasets.get(("nws", "MS")) and all_datasets.get(("usgs", "MS")):
            ...
            all_ahps_datasets.rename(
                columns={"benchmark_source": "source"}, inplace=True
            )
        ...
        if all_datasets.get(("ble", "FR")) and all_datasets.get(("ifc", "FR")):
            ...
            print(
                "BLE/IFC FR datasets not analyzed, no spatial data created.\nTo produce spatial data analyze a FR version"
            )
