...
def generate_rating_curve_metrics(args):
    ...
    if len(usgs_gages) > 0:
        ...
        # Add stream order
        stream_orders = hydrotable.filter(
            items=["location_id", "str_order"]
        ).drop_duplicates()
        ...
        nwm_recurr_intervals_all = reduce(
            lambda x, y: pd.merge(x, y, on="feature_id", how="outer"),
            [recurr_1_5_yr, recurr_5_yr, recurr_10_yr],
        )
        nwm_recurr_intervals_all = pd.melt(
            nwm_recurr_intervals_all,
            id_vars=["feature_id"],
            value_vars=["1.5", "5.0", "10.0"],
        ...
        for index, gage in usgs_crosswalk.iterrows():
            ...
            if len(usgs_pred_elev) < 1:
                print(
                    f"missing USGS elevation data for usgs station {gage.location_id} in huc {huc}"
                )
                continue
            ...
            if len(fim_pred_elev) < 1:
                print(
                    f"missing FIM elevation data for usgs station {gage.location_id} in huc {huc}"
                )
                continue
        ...
        if not nwm_recurr_data_table.empty:
            ...
            nwm_recurr_data_table.elevation_ft = np.round(
                nwm_recurr_data_table.elevation_ft, 2
            )
...
def aggregate_metrics(output_dir, procs_list, stat_groups):
    ...
    for huc in procs_list:
        ...
        if os.path.isfile(huc[4]):
            nwm_recurr_data = pd.read_csv(
                huc[4], dtype={"location_id": str, "feature_id": str}
            )
    ...
    agg_stats = pd.read_csv(
        agg_nwm_recurr_flow_elev, dtype={"location_id": str, "feature_id": str}
    )
...
def generate_facet_plot(rc, plot_filename):
    ...
    for gage in rc.location_id.unique():
        ...
        min_elev = rc.loc[
            (rc.location_id == gage) & (rc.source == "USGS")
        ].elevation_ft.min()
        max_elev = rc.loc[
            (rc.location_id == gage) & (rc.source == "USGS")
        ].elevation_ft.max()
    ...
    g.map(
        sns.scatterplot, "discharge_cfs", "elevation_ft", palette="tab20c", marker="o"
    )
...
def get_reccur_intervals(site_rc, usgs_crosswalk, nwm_recurr_intervals):
    ...
    if nwm_ids > 0:
        ...
        nwm_recurr_intervals = nwm_recurr_intervals.copy().loc[
            nwm_recurr_intervals.feature_id
            == usgs_site.feature_id.drop_duplicates().item()
        ]
...
def calculate_rc_stats_elev(rc, stat_groups=None):
    ...
    nrmse_table = reduce(
        ...
        [sum_y_diff, n, y_max, y_min],
    )
    nrmse_table_group = nrmse_table.groupby(stat_groups)

    # Calculate nrmse
    nrmse = (
        nrmse_table_group.apply(
            lambda x: ((x["sum_y_diff"] / x["n"]) ** 0.5) / (x["y_max"] - x["y_min"])
        )
        .reset_index(stat_groups, drop=False)
    ...
    rc_stat_table = reduce(
        lambda x, y: pd.merge(x, y, on=stat_groups, how="outer"),
        [nrmse, mean_abs_y_diff, percent_bias],
    )
...
if __name__ == "__main__":
    ...
    for huc in huc_list:
        ...
        if huc != "logs":
            ...
            nwm_recurr_data_filename = join(
                tables_dir, f"nwm_recurrence_flow_elevations_{huc}.csv"
            )
            rc_comparison_plot_filename = join(
                plots_dir, f"FIM-USGS_rating_curve_comparison_{huc}.png"
            )
    ...
    if merged_elev_table:
        ...
        concat_elev_table.to_csv(
            join(output_dir, "agg_usgs_elev_table.csv"), index=False
        )
