...
def get_all_usgs_gridnames():
    ...
    for i in sites:
        # Get gridnames
        url = (
            f"{USGS_METADATA_URL}/server/rest/services/FIMMapper/{i}/MapServer?f=pjson"
        )
...
def obtain_usgs_data(workspace):
    ...
    target_ahps_codes = target_ahps_codes.query('Source in ["Both","USGS"]')[
        "Total_List"
    ].to_list()
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-w",
        "--workspace",
        help="Workspace where all outputs will be saved.",
        required=True,
    )
