...
def gms_box_plots(metrics_table, output_fig=None):
    ...
    facetgrid.fig.legend(
        loc='lower left', ncol=3, bbox_to_anchor=(0.22, -0.06), handles=handles, labels=labels
    )
    ...
    facetgrid.fig.subplots_adjust(wspace=0, hspace=0.1)
...
def set_axes(facetgrid):
    range_by_row = ((0.45, 0.75, 0.05), (0.50, 0.90, 0.05), (0.00, 0.40, 0.05))
    ...
    for rowIdx in range(nrows):
        ...
        for colIdx in range(ncols):
            ...
            if colIdx == 1:
                facetgrid.axes[rowIdx, colIdx].axvline(
                    x=facetgrid.axes[rowIdx, colIdx].get_xlim()[1], color='black'
                )
...
def preparing_data_for_plotting(metrics_table):
    ...
    metrics_table = pd.melt(
        metrics_table,
        id_vars=('Model', 'Magnitude', 'Mannings N'),
...
def annotate_rlm(facetgrid, metrics_table):
    ...
    for met, man in metrics_table.index.unique():
        ...
        facetgrid.axes[rowIdx, colIdx].annotate(
            r'$\beta_1$  =  {:.4f}  |  p-value  =  {:.3f}'.format(beta1, beta1_pvalue),
            xy=(0.01, 1.025),
