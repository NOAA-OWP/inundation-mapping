...
def __go_fast_mapping(rem, catchments, catchmentStagesDict, inundation, depths):
    for i, (r, cm) in enumerate(zip(rem, catchments)):
        if cm in catchmentStagesDict:
            if r >= 0:  # 0.027 meters representing an inch or more of inundation depth
                depth = catchmentStagesDict[cm] - r
                if np.round(depth * 1000) / 1000 < 0.03048:
                    depths[i] = 0
                else:
                    depths[i] = depth
            else:
            ...
            if depths[i] >= 0.03048:  # set positive depths to positive
                inundation[i] *= -1
