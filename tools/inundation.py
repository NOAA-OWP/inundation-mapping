...
class Inundation(object):
    ...
    def SRC_lookup(self, flow_df, units):
        ...
        forecast['stage'] = forecast.apply(
            lambda row: self.interp_stage(
                row.discharge, self.hydroTable.loc[self.hydroTable.HydroID == int(row.HydroID)]
            ),
            axis=1,
        )
    ...
    def inundate_with_flows(
        cls, rem_path, catch_path, hydroTable_path, flow_file, nodata=-9999.0, units='cms', depth=False
    ):
        """
        ...
        for huc in hucs:
            ...
            for branch in list(fim_inputs[fim_inputs['huc8'] == huc]['branch'].unique()):
                ...
                i = Inundation.inundate_with_flows(
                    rem_path, catch_path, hydroTable_path, flow_file, nodata=nodata, units=units, depth=depth
                )
    ...
    @classmethod
    def composite_branches(cls, huc_dir, flow_file, nodata=-9999.0, units='cms', depth=False):
        ...
        for branch in branches:
            ...
            catch_path = os.path.join(
                branches_dir, branch, f'gw_catchments_reaches_filtered_addedAttributes_{branch}.tif'
            )
