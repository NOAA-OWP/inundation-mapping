...
def preprocess_usgs(source_dir, destination, reference_raster):
    ...
    ahps_codes = [
        folder.name for folder in source_dir.glob("*") if len(folder.name) == 5
    ]
    ...
    for code in ahps_codes:
        ...
        if code in [
            ...
            f.write(
                f"{code} : Custom workaround related to benchmark data (mismatch crs, no grid data, etc)\n"
            )
        ...
        grid_paths = [
            grids
            for grids in ahps_dir.glob("*.tif*")
        ...
        df.rename(
            columns={"QCFS": "flow", "STAGE": "stage", "ELEV": "elevation"},
            inplace=True,
        )
        ...
        if code == "vdsg1":
            ...
            f.write(
                f"{code} : Custom workaround because thresholds are reported as elevations\n"
            )
