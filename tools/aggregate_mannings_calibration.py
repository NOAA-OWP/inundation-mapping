...
def aggregate_parameter_sets(
    huc_list_path, calibration_stat_folder, summary_file, mannings_json
):
            ...
            for mannings_value in os.listdir(stream_order_dir):
                ...
                for flood_recurrence in os.listdir(mannings_value_dir):
                    ...
                    total_area_stats = pd.read_csv(
                        os.path.join(flood_recurrence_dir, "total_area_stats.csv"),
                        index_col=0,
                    )
        ...
        if (len(mannings_100yr) == 1) & (len(mannings_500yr) == 1):
            if mannings_100yr.iloc[0] == mannings_500yr.iloc[0]:
                ...
                print(
                    ...
                    + str(
                        round(abs(mannings_100yr.iloc[0] - mannings_500yr.iloc[0]), 2)
                    )
                    + " for stream order "
                    + str(stream_order)
                )
