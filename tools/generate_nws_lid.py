...
def generate_nws_lid(workspace):
    ...
    fcst_list_sites = [
        record.get("identifiers").get("nws_lid").lower() for record in fcst_list
    ]
    evaluated_sites = (
        pd.read_csv(EVALUATED_SITES_CSV)["Total_List"].str.lower().to_list()
    )
    ...
    trash, nws_lid_gdf = aggregate_wbd_hucs(
        all_lists, WBD_LAYER, retain_attributes=False
    )
    nws_lid_gdf.columns = [
        name.replace("identifiers_", "") for name in nws_lid_gdf.columns
    ]
    ...
    final_dict_pd = pd.DataFrame(
        list(final_dict.items()), columns=["nws_lid", "is_headwater"]
    )
