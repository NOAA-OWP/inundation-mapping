...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description="Simple tool to search for csv files (using wildcard text) within a fim output directory and then aggregate all files into a single csv")
    parser.add_argument('-fim_dir','--fim-dir', help='FIM output dir (e.g. data/outputs/xxxx/', required=True,type=str)
    parser.add_argument('-search_str','--file-search-str', help='File search string', required=True,type=str)
    parser.add_argument('-head_row','--header-row', help='Optional: header row to parse (default=0)',default=0, required=False,type=int)
    parser.add_argument('-out_csv','--output-csv', help='full filepath to write new csv', required=True,type=str)
