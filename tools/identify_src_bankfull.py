...
def nwm_1_5_bankfull_lookup(args):
    ...
    if check_null > 0:
        log_text += (
            "Missing feature_id in crosswalk for huc: "
            + str(huc)
    ...
    negative_flows = len(
        df_src.loc[(df_src.discharge_1_5 <= 0) & (df_src.discharge_1_5 != -999)]
    )
    if negative_flows > 0:
        log_text += (
            "HUC: "
            + str(huc)
            + " --> Negative or zero flow values found (likely lakeid loc)\n"
        )
    ...
    if (
        df_src["Q_1_5_find"].isnull().values.any()
    ):  # there may be null values for lake or coastal flow lines (need to set a value to do groupby idxmin below)
        log_text += (
            "HUC: "
            + str(huc)
            + ' --> Null values found in "Q_1_5_find" calc. These will be filled with 999999 () \n'
        )
    ...
    df_1_5 = df_src[
        ["Stage", "HydroID", volume_var, hradius_var, "Q_1_5_find"]
    ...
    df_1_5 = df_1_5.loc[df_1_5.groupby("HydroID")["Q_1_5_find"].idxmin()].reset_index(
        drop=True
    )  # find the index of the Q_1_5_find (closest matching flow)
    df_1_5 = df_1_5.rename(
        columns={
            "Stage": "Stage_1_5",
    ...
    df_src = df_src.merge(
        df_1_5[["Stage_1_5", "HydroID", "Volume_bankfull", "HRadius_bankfull"]],
        how="left",
        on="HydroID",
    )
    ...
    df_src[
        "chann_volume_ratio"
    ] = 1.0  # At stage=0 set channel_ratio to 1.0 (avoid div by 0)
    df_src["chann_volume_ratio"].where(
        df_src["Stage"] == 0,
        df_src["Volume_bankfull"] / (df_src[volume_var]),
        inplace=True,
    )
    # df_src['chann_volume_ratio'] = df_src['chann_volume_ratio'].clip_upper(1.0)
    df_src["chann_volume_ratio"].where(
        df_src["chann_volume_ratio"] <= 1.0, 1.0, inplace=True
    )  # set > 1.0 ratio values to 1.0 (these are within the channel)
    df_src["chann_volume_ratio"].where(
        df_src["discharge_1_5"] > 0.0, 0.0, inplace=True
    )  # if the discharge_1_5 value <= 0 then set channel ratio to 0 (will use global overbank manning n)
    ...
    df_src["chann_hradius_ratio"].where(
        df_src["Stage"] == 0,
        df_src["HRadius_bankfull"] / (df_src[hradius_var]),
        inplace=True,
    )
    ...
    df_src["chann_hradius_ratio"].where(
        df_src["chann_hradius_ratio"] <= 1.0, 1.0, inplace=True
    )  # set > 1.0 ratio values to 1.0 (these are within the channel)
    df_src["chann_hradius_ratio"].where(
        df_src["discharge_1_5"] > 0.0, 0.0, inplace=True
    )  # if the discharge_1_5 value <= 0 then set channel ratio to 0 (will use global overbank manning n)
    ...
    df_src.loc[
        df_src["Stage"] > df_src["Stage_1_5"], "channel_fplain_1_5"
    ] = "floodplain"
...
def generate_src_plot(df_src, plt_out_dir):
    ...
    for hydroid in hydroids:
        ...
        ax.legend()
        plt.savefig(
            plt_out_dir + os.sep + str(hydroid) + ".png", dpi=175, bbox_inches="tight"
        )
        plt.close()
...
if __name__ == "__main__":
    ...
    print(
        ...
        + str(join(fim_dir, "bankfull_detect.log"))
    )
    ...
    if not isfile(bankfull_flow_filepath):
        print(
            "!!! Can not find the input recurr flow file: "
            + str(bankfull_flow_filepath)
        )
    else:
        ...
        for huc in huc_list:
            if huc != "logs" and huc[-3:] != "log" and huc[-4:] != ".csv":
                ...
                src_modify_filename = join(
                    fim_dir, huc, "src_full_crosswalked_bankfull.csv"
                )
        ...
        print(
            f"Identifying bankfull thresholds for {len(procs_list)} hucs using {number_of_jobs} jobs"
        )
