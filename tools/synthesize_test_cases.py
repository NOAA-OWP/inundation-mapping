                    ...
                    for iteration in iteration_list:
                        ...
                        for magnitude in magnitude_list:
                            for version in versions_to_aggregate:
                                # this may be pulling in older fim3 versions (so keep the _ms, _fr and _c)
                                if '_ms' in version:
                                    extent_config = 'MS'
...
if __name__ == '__main__':
    ...
    python /foss_fim/tools/synthesize_test_cases.py -c DEV -v some_runtime_folder_name -jh 3 -ji 25 -m /outputs/some_runtime_folder_name/some_runtime_folder_name_metrics.csv -o
     ...
     To see your outputs in the test_case folder (hard coded path), you can check for outputs using
         (cd .... to your test_case folder), then command becomes  find . -name some_runtime_folder_name* -type d (Notice the
     ...
     If you want to delete the test outputs, test the outputs as suggest immediately above, but this time your
         command becomes:  find . -name some_runtime_folder_name* -type d  -exec rm -rdf {} +
    '''
    ...
    parser.add_argument('-jb','--job-number-inundate',help='Number of processes to use for inundating. HUC and inundate job numbers should multiply to no more than one less than the CPU count of the machine.',required=False, default=1,type=int)    
    ...
    job_number_inundate = args['job_number_inundate']
    ...
    if total_cpus_requested > total_cpus_available:
        raise ValueError('The HUC job number, {}, multiplied by the inundate job number, {}, '\
                          ...
                          'Please lower the -j (job_number_huc) or job_number_inundate'\
    ...
    with ProcessPoolExecutor(max_workers=job_number_huc) as executor:
        ...
        for test_case_class in all_test_cases:
            ...
            alpha_test_args = { 
                                ...
                                'overwrite': overwrite,
                                'verbose':gms_verbose,
                                'num_workers_inundate': job_number_inundate
                                }
        ...
        progress_bar_handler(executor_dict, True, f"Running alpha test cases with {job_number_huc} workers")
