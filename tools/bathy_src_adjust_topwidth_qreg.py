...
def bathy_rc_lookup(args):
    ...
    out_src_bankfull_filename           = args[8]
    src_plot_option                     = args[9]
    huc_output_dir                      = args[10]
    ...
    print(out_src_bankfull_filename)
...
if __name__ == '__main__':
    ...
    if not isfile(bankfull_flow_filepath):
        ...
        if not isfile(bankfull_regres_filepath):
            ...
            df_bfull_geom = pd.read_csv(bankfull_regres_filepath,dtype= {'feature_id': int})
            if 'COMID' not in df_bfull_geom.columns and 'feature_id' not in df_bfull_geom.columns:
                ...
                for huc in huc_list:
                    if huc != 'logs' and huc[-3:] != 'log' and huc[-4:] != '.csv':
                        ...
                        new_htable_filename = join(fim_dir,huc,'hydroTable_' + hydrotable_suffix + '.csv')
                        ...
                        if isfile(in_src_bankfull_filename):
                            print(str(huc))
                            procs_list.append([in_src_bankfull_filename,df_bfull_geom,output_bath_filename,output_bathy_streamorder_fileName,output_bathy_thalweg_fileName,output_bathy_xs_lookup_fileName,htable_filename,new_htable_filename,out_src_bankfull_filename,src_plot_option,huc_plot_output_dir])
                        else:
