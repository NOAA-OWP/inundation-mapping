...
def initial_func(category):
    """Function that handles benchmark data.
    ...
    assert category in list(MAGNITUDE_DICT.keys()), f"Category must be one of {list(MAGNITUDE_DICT.keys())}"
    validation_data = os.path.join(TEST_CASES_DIR, f'{category}_test_cases', f'validation_data_{category}')

    if category in AHPS_BENCHMARK_CATEGORIES:
        is_ahps = True
    else: 
        is_ahps = False

    magnitudes = MAGNITUDE_DICT[category]
    
    return validation_data, is_ahps, magnitudes

def huc_data(validation_data):
    '''Returns a dict of HUC8, magnitudes, and sites.'''
    huc_mags = {}
    for huc in os.listdir(validation_data):
        if not re.match(r'\d{8}', huc):
            continue
        huc_mags[huc] = data(huc)

    return huc_mags

def data(huc, validation_data, is_ahps, magnitudes):
    ...
    if is_ahps == is_ahps:
        ...
        for lid in lids:
            ...
            for mag in [file for file in os.listdir(lid_dir) if file in magnitudes()]:
                if mag in mag_dict:
...
def initial_Test_Case(test_id, version, archive=True):
    ...
    """
    test_id = test_id
    huc, benchmark_cat = test_id.split('_')

    validation_data, is_ahps, magnitudes = initial_func(benchmark_cat)
    version = version
    archive = archive
    # FIM run directory path - uses HUC 6 for FIM 1 & 2
    fim_dir = os.path.join(PREVIOUS_FIM_DIR if archive else OUTPUTS_DIR, version,
        huc if not re.search('^fim_[1,2]', version, re.IGNORECASE) else huc[:6])
...
def alpha_test(
    ...
    try:
        if not overwrite and os.path.isdir(dir_TC):
            print(f"Metrics for {dir_TC} already exist. Use overwrite flag (-o) to overwrite metrics.")
            return

        fh.vprint(f"Starting alpha test for {dir_TC}", verbose)

        stats_modes_list = ['total_area']

        # Create paths to fim_run outputs for use in inundate()
        if model != 'GMS':
            rem = os.path.join(fim_dir, 'rem_zeroed_masked.tif')
            if not os.path.exists(rem):
                rem = os.path.join(fim_dir, 'rem_clipped_zeroed_masked.tif')
            catchments = os.path.join(
                fim_dir, 'gw_catchments_reaches_filtered_addedAttributes.tif'
            )
            if not os.path.exists(catchments):
                catchments = os.path.join(
                    fim_dir, 'gw_catchments_reaches_clipped_addedAttributes.tif'
                )
            mask_type = mask_type
            if mask_type == 'huc':
                catchment_poly = ''
                ...
                catchment_poly = os.path.join(
                    fim_dir, 'gw_catchments_reaches_filtered_addedAttributes_crosswalked.gpkg'
                )
            hydro_table = os.path.join(fim_dir, 'hydroTable.csv')
        ...
        if os.path.exists(dir_TC):
            shutil.rmtree(dir_TC)
        os.mkdir(dir_TC)
        ...
        for magnitude in validation_data:
            ...
            if self.is_ahps:
                self.clean_ahps_outputs(os.path.join(dir_TC, magnitude))
