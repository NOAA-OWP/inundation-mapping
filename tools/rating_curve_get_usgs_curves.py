...
def get_all_active_usgs_sites():
    ...
    for metadata in metadata_list:
        ...
        if (
            coord_accuracy_code in acceptable_coord_acc_code
            and coord_method_code in acceptable_coord_method_code
            and alt_accuracy_code <= acceptable_alt_acc_thresh
            and alt_method_code in acceptable_alt_meth_code
            and site_type in acceptable_site_type
        ):
    ...
    dictionary, gdf = aggregate_wbd_hucs(
        acceptable_sites_metadata, Path(WBD_LAYER), retain_attributes=False
    )
...
def usgs_rating_to_elev(list_of_gage_sites, workspace=False, sleep_time=1.0):
    ...
    if list_of_gage_sites == ["all"]:
        (
            acceptable_sites_gdf,
            acceptable_sites_list,
            metadata_list,
        ) = get_all_active_usgs_sites()
    # Otherwise, if a list of sites is passed, retrieve sites from WRDS.
        ...
        if len(selector) > max_sites:
            chunks = [
                selector[i : i + max_sites] for i in range(0, len(selector), max_sites)
            ]
    ...
    acceptable_sites_gdf = acceptable_sites_gdf.merge(
        sites_with_data, on="location_id", how="left"
    )
    ...
    acceptable_sites_gdf.loc[
        acceptable_sites_gdf.eval("feature_id in @ms_segs"), "mainstem"
    ] = "yes"
    ...
    if workspace:
        ...
        all_rating_curves.to_csv(
            Path(workspace) / "usgs_rating_curves.csv", index=False
        )
