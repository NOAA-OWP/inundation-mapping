...
def run_inundation(args):
    ...
    depth_raster = os.path.join(
        magnitude_output_dir, magnitude + "_" + config + "_inund_depth.tif"
    )
    ...
    if not os.path.exists(depth_raster):
        print(
            "Running the NWM recurrence intervals for HUC: "
            + huc
            + ", "
            + magnitude
            + "..."
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-j", "--job-number", help="The number of jobs", required=False, default=1
    )
    ...
    for huc in huc_list:
        if huc != "logs":
            for magnitude in magnitude_list:
                magnitude_output_dir = os.path.join(
                    output_dir, magnitude + "_" + config
                )
                if not os.path.exists(magnitude_output_dir):
                    os.mkdir(magnitude_output_dir)
                    print(magnitude_output_dir)
                procs_list.append(
                    [fim_run_dir, huc, magnitude, magnitude_output_dir, config]
                )
