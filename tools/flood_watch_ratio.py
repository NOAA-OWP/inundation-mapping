...
def process_catchments(group):
    ...
    return_area_nbm = np.interp(high_flow_nbm, sorted_group['discharge_cms'], sorted_group['SurfaceArea (m2)'])
...
def process_huc(huc, nbm_df_bflows, df_bflows, huc12, huc_index, total_hucs, flow_huc12, output_dir):
    ...
    huc12_df = water_table.merge(flow_huc12[['HydroID', 'feature_id', 'HUC12', 'branch_id']], on=['HydroID', 'feature_id', 'branch_id'], how='left')
    valid_sur = huc12_df.dropna(subset=['SurfaceArea_nrp', 'SurfaceArea_nbm'])
    aggreagtion = valid_sur.groupby(['HUC12', 'branch_id'], as_index=False).agg({'SurfaceArea_nbm' : lambda x : np.nanmean(x), 'SurfaceArea_nrp': lambda x : np.nanmean(x)})

    aggregate_final = aggreagtion.groupby('HUC12').agg({'SurfaceArea_nbm' : lambda x : np.nanmean(x), 'SurfaceArea_nrp': lambda x : np.nanmean(x)}).reset_index()
...
def process_chunk(huc_chunk, nbm_df_bflows, df_bflows,huc12, total_hucs, chunk_idx, flow_huc12, output_dir, args):
    print(f"Processing chunk {chunk_idx + 1} with {len(huc_chunk)} HUCs")
    job_number = args.job_number
    with Pool(processes=job_number) as pool:
        result_files = pool.starmap(process_huc, [(huc, nbm_df_bflows, df_bflows, huc12, idx, total_hucs, flow_huc12, output_dir) for idx, huc in enumerate(huc_chunk)])
...
def main(args):
    ...
    for chunk_idx, huc_chunk in enumerate(huc_chunks):
        chunk_file = process_chunk(huc_chunk, nbm_df_bflows, df_bflows, huc12, total_hucs, chunk_idx, flow_huc12, args.output_dir, args)
        if chunk_file:
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Process HUCs for NBM and NRP surface area and calculating ratio value required for flood watch workflow.')
    parser.add_argument('-huc', '--huc_file', required=True, help='Path to the text file containing list of HUCs')
    parser.add_argument("-d", "--fim_dir", required=True, help="Directory path to FIM hydrofabric by processing unit.", type=str)
    ...
    parser.add_argument("-chunk", "--chunk_size", type=int, required=False, default=20, help="Number of HUCs per chunk")
    parser.add_argument("-j", "--job_number", required=False, type=int, default=min(8, cpu_count()), help='Optional, (default: min(8, CPU cores))')
