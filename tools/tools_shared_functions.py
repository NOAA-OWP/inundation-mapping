...
def download_all_thresholds(threshold_url, metadata_pkl_file, output_folder):
    ...
    - metadata_pkl_file: str, path to the metadata pickle file containing site information.
    ...
    # Make an intermediate folder in the output folder
    thresholds_folder = os.path.join(output_folder, 'threshold_download')
    try:
        os.makedirs(thresholds_folder, exist_ok=True)
    except Exception as e:
        raise OSError(f"Error creating directory {thresholds_folder}: {e}")
    ...
    # Compile thresholds into a dataframe
    all_dataframes = []
    ...
    try:
        ...
        print(f"Successfully combined {len(csv_files)} CSVs into {output_pickle_path}")
    except Exception as e:
