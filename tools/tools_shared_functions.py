...
def check_for_regression(
    stats_json_to_test,
    previous_version,
    previous_version_stats_json_path,
    ...
    for stats_mode in contingency_table_dictionary:
        ...
        if stats_csv != None:
            stats_csv = os.path.join(
                os.path.split(stats_csv)[0], file_handle + "_stats.csv"
            )
            df = pd.DataFrame.from_dict(
                mode_stats_dictionary, orient="index", columns=["value"]
            )
            df.to_csv(stats_csv)
        ...
        if stats_json != None:
            stats_json = os.path.join(
                os.path.split(stats_csv)[0], file_handle + "_stats.json"
            )
...
def get_contingency_table_from_binary_rasters(
    ...
    benchmark_array = np.where(
        predicted_array == predicted_src.nodata, 10, benchmark_array
    )
    ...
    if agreement_raster != None:
        ...
        with open(legend_txt, "w") as f:
            ...
            f.write(
                "%s\n"
                % "Results produced at: {current_time}".format(
            ...
            if operation == "include":
                ...
                if poly_all_proj.empty:
                    ...
                    layer_agreement_raster = os.path.join(
                        os.path.split(agreement_raster)[0],
                    ...
                    with rasterio.Env():
                        ...
                        profile.update(nodata=10)
                        with rasterio.open(
                            layer_agreement_raster, "w", **profile
                        ) as dst:
                    ...
                    contingency_table_dictionary.update(
                        {
                            poly_handle: {
                                "true_negatives": int(
                                    (temp_agreement_array == 0).sum()
                                ),
                                "false_negatives": int(
                                    (temp_agreement_array == 1).sum()
                                ),
                                "false_positives": int(
                                    (temp_agreement_array == 2).sum()
                                ),
                                "true_positives": int(
                                    (temp_agreement_array == 3).sum()
                                ),
...
def aggregate_wbd_hucs(metadata_list, wbd_huc8_path, retain_attributes=False):
    ...
    joined_gdf = gpd.sjoin(
        metadata_gdf, huc8, how="inner", op="intersects", lsuffix="ahps", rsuffix="wbd"
    )
    ...
    if response.ok:
        ...
        if thresholds_info:
            ...
            if threshold_data:
                ...
                stages["source"] = threshold_data.get("metadata").get(
                    "threshold_source"
                )
                ...
                stages["usgs_site_code"] = threshold_data.get("metadata").get(
                    "usgs_site_code"
                )
                flows["usgs_site_code"] = threshold_data.get("metadata").get(
                    "usgs_site_code"
                )
        ...
        if not stages[threshold] is None:
            ...
            if threshold in ["action", "minor", "moderate"]:
                ...
                value = dataframe.query(
                    f"({lower_bound}<=elevation) & (elevation<{upper_bound})"
                )["elevation"].min()
            # For major threshold
            else:
            ...
            if np.isfinite(value):
                ...
                map_flow = round(
                    dataframe.query(f"elevation == {value}")["flow"].item(), 0
                )
...
def process_extent(extent, profile, output_raster=False):
    ...
    poly_extent_fill_holes = MultiPolygon(
        Polygon(p.exterior) for p in poly_extent["geometry"]
    )
...
def raster_to_feature(grid, profile_override=False, footprint_only=False):
    ...
    for geom, val in rasterio.features.shapes(data, mask=msk, transform=data_transform):
        ...
        values.append(val)
    spatial_geodataframe = gpd.GeoDataFrame(
        {"values": values, "geometry": spatial}, crs=coord_sys
    )
...
########################################################################
def process_grid(
    benchmark, benchmark_profile, domain, domain_profile, reference_raster
):
    """
    ...
    benchmark_fit_to_domain_bool = np.where(
        benchmark_fit_to_domain == benchmark.nodata, 0, 1
    )
