...
def inundate_and_mosaic(
    ...
    Mosaic_inundation(
        ...
        mosaic_output=inundation_raster if depths_raster is None else depths_raster,
...
def produce_mosaicked_inundation(
    ...
    """
    This function calls Inundate_gms and Mosaic_inundation to produce inundation maps. Possible outputs
    include inundation rasters encoded by HydroID (negative HydroID for dry and positive HydroID for wet),
    polygons depicting extent, and depth rasters. The function requires a flow file organized by NWM
    feature_id and discharge in cms. "feature_id" and "discharge" columns MUST be present in the flow file.

    Args:
        hydrofabric_dir (str): Path to hydrofabric directory where FIM outputs were written by fim_pipeline.
    ...
    if not os.path.exists(os.path.join(hydrofabric_dir, huc)):
        raise FileNotFoundError((errno.ENOENT, os.strerror(errno.ENOENT), os.path.join(hydrofabric_dir, huc)))
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '-y', '--hydrofabric_dir', help='Path to FIM hydrofabric by processing unit.', required=True
    )
    ...
    parser.add_argument(
        '-i', '--inundation-raster', help='Inundation raster output.', required=True, default=None
    )
    parser.add_argument(
        '-p',
        '--inundation-polygon',
