...
def catchment_boundary_errors(hydrofabric_dir, hucs, inundation_raster, output):
    """
    ...
    for huc in hucs:
        ...
        for shp, val in rasterio.features.shapes(band, transform = affine):
            if val == 1.0:
            # print('%s: %s' % (val, shape(shp)))
                t = {'val': val, 'geometry' : gpd.GeoSeries(shape(shp))}
                t = gpd.GeoDataFrame(t)
                inund_poly = pd.concat([inund_poly,t], ignore_index = True)
        ...
        error_lines_final = hydroid_join_len.loc[hydroid_join_len['Length'] >= 100.0]        
