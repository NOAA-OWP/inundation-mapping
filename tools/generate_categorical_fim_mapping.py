...
def generate_categorical_fim(
    fim_run_dir, source_flow_dir, output_cat_fim_dir, number_of_jobs, depthtif, log_file
):
    ...
    for huc in matching_hucs:
        ...
        if "." not in huc:
            ...
            catchments = os.path.join(
                fim_run_huc_dir, "gw_catchments_reaches_filtered_addedAttributes.tif"
            )
                    ...
                    if "." not in magnitude:
                        ...
                        if os.path.exists(magnitude_flows_csv):
                            ...
                            output_extent_grid = os.path.join(
                                cat_fim_huc_ahps_dir,
                                ahps_site + "_" + magnitude + "_extent.tif",
                            )

                            if depthtif:
                                output_depth_grid = os.path.join(
                                    cat_fim_huc_ahps_dir,
                                    ahps_site + "_" + magnitude + "_depth.tif",
                                )
                            else:
...
def post_process_cat_fim_for_viz(
    number_of_jobs, output_cat_fim_dir, nws_lid_attributes_filename, log_file
):
            ...
            for huc in huc_ahps_dir_list:
                ...
                if huc not in skip_list:
                    ...
                    for ahps_lid in ahps_dir_list:
                        ...
                        extent_grid = os.path.join(
                            ahps_lid_dir,
                            ahps_lid + "_" + magnitude + "_extent_" + huc + ".tif",
                        )
...
def reformat_inundation_maps(args):
    ...
    try:
        ...
        nws_lid_attributes_table = nws_lid_attributes_table.loc[
            ...
            & (nws_lid_attributes_table.nws_lid == lid)
        ]
        ...
        if not extent_poly_diss.empty:
            ...
            extent_poly_diss.to_file(
                diss_extent_filename,
                driver=getDriver(diss_extent_filename),
                index=False,
            )
...
if __name__ == "__main__":
    ...
    nws_lid_attributes_filename = os.path.join(
        source_flow_dir, "nws_lid_attributes.csv"
    )
