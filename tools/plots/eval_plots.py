...
def eval_plots(
    ...
    for (benchmark_source, extent_configuration), benchmark_metrics in benchmark_by_source:
        ...
        if benchmark_source in ["usgs", "nws"]:
            ...
            # Default query (used for APG) it could be that bad_sites should be modified.
            # If so pass an alternate query using the "alternate_ahps_query"
    ...
    if spatial_ahps:
        ...
        preserved_static_library_fields = ["nws_lid"] + [
            i for i in static_library.columns if i.startswith(("Q", "S"))
        ]
        # Get list of fields to keep in merge.
        ...
        evaluated_ahps_extent = evaluated_ahps_extent.merge(
            static_library[preserved_static_library_fields], on="nws_lid"
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-v",
        "--versions",
        help="List of versions to be plotted/aggregated. "
        'Versions are filtered using the "startswith" approach.'
        'Example: ["fim_","fb1"] would retain all versions that began with "fim_" '
        'e.g. fim_1..., fim_3...,) as well as any feature branch that began with "fb". '
        'Example: ["fim_3","fb"] results in all fim_3 versions being plotted along with the fb.',
        nargs="+",
        default=[],
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "-q",
        "--alternate_ahps_query",
        help="Alternate filter query for AHPS. Default is: "
        '"not nws_lid.isnull() & not flow.isnull() & masked_perc<97 & not nws_lid in @bad_sites'
        'where bad_sites are (grfi2,ksdm7,hohn4,rwdn4)"',
        default=False,
        required=False,
    )
    parser.add_argument(
        "-sp",
        "--spatial_ahps",
        help="If spatial point layer is desired, supply a csv with 3 lines of the following:"
        "metadata, path/to/metadata/shapefileevaluated, path/to/metadata/shapefilestatic.",
        default=False,
        required=False,
    )
    parser.add_argument(
        "-f",
        "--fim_1_ms",
        help='If enabled, fim_1 rows will be duplicated and extent config assigned "ms", '
        "so that fim_1 can be shown on mainstems plots/stats",
        action="store_true",
        required=False,
    )
