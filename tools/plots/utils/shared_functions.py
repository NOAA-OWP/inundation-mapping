    ...
    if simplify_legend:
        ...
        for label in org_labels:
            if "fim_1" in label:
                ...
                label_dict[label] = (
                    re.split("_fr|_ms", label)[0].replace("_", ".").replace("fim.", "FIM ")
                    + " "
...
def filter_dataframe(dataframe, unique_field):
    ...
    final_filtered_dataframe : Pandas Dataframe
        Filtered dataframe that contains only common sites (lids or hucs)
        between versions for each magnitude.
        For example, for AHPS all sites which were run for each version,
        for a given magnitude will be kept or for ble,
        all hucs which ran for all versions for a given magnitude.
    unique_sites: DICT
    ...
    # Create new dataframe to hold metrics for the common sites and
    # the actual lists of common sites.
    ...
    for magnitude in magnitudes:
        ...
        # List contains 3 unique sets:
        # [{fim1:unique lids},{fim2: unique lids},{fim3: unique lids}]
        ...
        # Query filtered dataframe and only include data associated with
        # the common sites for that magnitude
        filtered_common_sites = dataframe.query(
            f'magnitude == "{magnitude}" & {unique_field} in @common_sites_per_magnitude'
        )
        # Append the data for each magnitude to a final dataframe that will contain data for
        # all common sites for all magnitudes.
