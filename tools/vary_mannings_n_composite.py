...
def variable_mannings_calc(args):
    ...
    check_null = (
        df_src["channel_n"].isnull().sum() + df_src["overbank_n"].isnull().sum()
    )
    if check_null > 0:
        log_text += (
            str(huc)
    ...
    df_src["comp_ManningN"] = (
        df_src[channel_ratio_src_column] * df_src["channel_n"]
    ...
    df_src_trim = df_src[
        ["HydroID", "Stage", "Discharge (m3s-1)_varMann", "comp_ManningN"]
    ]
    df_src_trim = df_src_trim.rename(
        columns={
            "Stage": "stage",
            "Discharge (m3s-1)_varMann": "discharge_cms",
            "comp_ManningN": "ManningN",
        }
    )
    df_htable = pd.read_csv(htable_filename, dtype={"HUC": str})
    df_htable.drop(
        ["discharge_cms", "ManningN"], axis=1, inplace=True
    )  # drop the original discharge column to be replaced with updated version
...
def generate_src_plot(df_src, plt_out_dir):
    ...
    for hydroid in hydroids:
        ...
        sns.lineplot(
            x="Discharge (m3s-1)", y="Stage_1_5", data=plot_df, color="green", ax=ax
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-suff",
        "--hydrotable-suffix",
        help="Suffix to append to the new hydroTable csv file (e.g. '_global_06_011')",
        required=True,
        type=str,
    )
    parser.add_argument(
    ...
    print(
        ...
        + str(join(fim_dir, "log_composite_n" + hydrotable_suffix + ".log"))
    )
    ...
    if not isfile(mann_n_table):
        print(
            "!!! Can not find the input roughness/feature_id file: " + str(mann_n_table)
        )
    else:
        ...
        if (
            ...
            log_file = open(
                join(fim_dir, "log_composite_n" + hydrotable_suffix + ".log"), "w"
            )
            sys.stdout = log_file
            ...
            for huc in huc_list:
                if huc != "logs" and huc[-3:] != "log" and huc[-4:] != ".csv":
                    ...
                    out_src_bankfull_filename = join(
                        fim_dir, huc, "src_full_crosswalked_vmann.csv"
                    )
                    htable_filename = join(fim_dir, huc, "hydroTable.csv")
                    new_htable_filename = join(
                        fim_dir, huc, "hydroTable" + hydrotable_suffix + ".csv"
                    )
                    ...
                    if isfile(in_src_bankfull_filename):
                        ...
                        print(
                            str(huc)
                            + " --> can not find the src_full_crosswalked_bankfull.csv in the fim output dir: "
