...
def cygnss_preprocessing(
    ...
    daily_mean_forecast_files=None,
):
    ...
    datetime_indices = bounding_boxes.apply(
        lambda df: pd.date_range(df['event_date'], periods=24, closed=None, freq='H', tz='UTC'),
        axis=1,
    )
    ...
    final_forecast_df.set_index(
        pd.to_datetime(final_forecast_df.index.date, utc=True), drop=True, inplace=True
    )
    ...
    final_forecast_df.drop(
        columns={'huc_x', 'Name_x', 'forecast_file_x', 'forecast_url_x', 'forecast_url_y'},
        inplace=True,
    )

    final_forecast_df.rename(
        columns={'Name_y': 'Name', 'huc_y': 'huc', 'forecast_file_y': 'forecast_file'}, inplace=True
    )

    def update_daily(
        daily_nwm_forecast_df, current_date, daily_mean_forecast_files, final_forecast_df
    ):
        ...
        outfile = os.path.join(
            os.path.dirname(daily_mean_forecast_files),
            filename + '_' + current_date_string + '.' + ext,
        )
        daily_mean.to_csv(outfile, index=True)

        final_forecast_df.loc[
            final_forecast_df['date_time'] == current_date, 'forecast_file'
        ] = outfile
    ...
    for i, row in tqdm(
        unique_forecast_df.iterrows(), total=len(unique_forecast_df), desc='Daily Means'
    ):
        current_nwm_forecast_df = get_forecast(row)

        if ii == unique_dict[current_date]:
            (
                daily_nwm_forecast_df,
                current_date,
                daily_mean_forecast_files,
                final_forecast_df,
            ) = update_daily(
                daily_nwm_forecast_df, current_date, daily_mean_forecast_files, final_forecast_df
            )
        ...
        if current_nwm_forecast_df is not None:
            if daily_nwm_forecast_df is None:
                ii = 0
                daily_nwm_forecast_df = pd.DataFrame(
                    np.empty((len(current_nwm_forecast_df), unique_dict[current_date]))
                )
                daily_nwm_forecast_df.set_index(
                    current_nwm_forecast_df.index, inplace=True, drop=True
                )
...
def construct_nwm_forecast_filenames_and_retrieve(
    ...
    for future in tqdm(
        as_completed(results), total=len(download_df), desc='Acquiring NWM forecasts'
    ):
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '-m',
        '--daily-mean-forecast-files',
        help='Daily Mean Forecast file',
        required=False,
        default=None,
    )
