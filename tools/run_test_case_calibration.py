...
def run_alpha_test(fim_run_dir, calib_dir, branch_name, test_id, mask_type, return_interval, compare_to_previous=False, run_structure_stats=False, run_levee_stats=False, archive_results=False):
    ...
    fim_run_calib = str(calib_dir)

    assert os.path.exists(fim_run_parent), "Cannot locate " + fim_run_parent
    assert os.path.exists(fim_run_calib), "Cannot locate " + fim_run_calib
...
if __name__ == '__main__':
    ...
    parser.add_argument('-d','--calib-dir',help='Name of directory containing parameter adjustment outputs',required=True)
    ...
    # Ensure calib_dir exists.
    if not os.path.exists(args['calib_dir']):
        print(TRED_BOLD + "Warning: " + WHITE_BOLD + "The provided calib_dir (-r) " + CYAN_BOLD + args['calib_dir'] + WHITE_BOLD + " could not be located in the 'outputs' directory." + ENDC)
        print(WHITE_BOLD + "Please provide the parent directory name for fim_run.sh outputs. These outputs are usually written in a subdirectory, e.g. outputs/123456/123456." + ENDC)
        print()
        exit_flag = True
