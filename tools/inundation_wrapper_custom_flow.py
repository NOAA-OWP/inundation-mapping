...
def run_recurr_test(fim_run_dir, branch_name, huc_id, input_flow_csv, mask_type="huc"):
    ...
    inundation_raster = os.path.join(
        branch_test_case_dir_parent, branch_name + "_inund_extent.tif"
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-t",
        "--huc-id",
        help="The huc id to use (single huc). Format as: xxxxxxxx, e.g. 12345678",
        required=True,
        default="",
    )
    parser.add_argument(
    ...
    if args["input_flow_csv"] == "":
        print(
            ...
            + "Error: "
            + WHITE_BOLD
            + "The provided input_flow_csv (-y) "
            + CYAN_BOLD
            + args["input_flow_csv"]
            + WHITE_BOLD
            + " is not provided. Please provide a csv file with nwm featureid and flow values"
            + ENDC
        )
        exit_flag = True
