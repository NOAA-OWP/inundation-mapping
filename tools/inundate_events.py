...
def inundate_events(hydrofabric_dir,forecast_file,inundation_file,inundation_polygon=None,jobs=1):
    ...
    results = {executor.submit(inundate,**kwargs) : (kwargs['hydro_table'],kwargs['forecast']) for kwargs in inputs}
...
def build_fim3_inputs(hydrofabric_dir,forecast_df,inundation_file=None,inundation_polygons=None):
    ...
    for idx,row in forecast_df.iterrows():
        ...
        catchments_raster = os.path.join(hydrofabric_dir,huc,'gw_catchments_reaches_filtered_addedAttributes.tif')
        catchment_poly = os.path.join(hydrofabric_dir,huc,'gw_catchments_reaches_filtered_addedAttributes_crosswalked.gpkg')
...
def append_meta_data_to_file_names(file_name,row):
    ...
    hucCode = row['huc']
    site_name = row['Name']
    date_time = str(row['date_time'].year) + str(row['date_time'].month).zfill(2) + str(row['date_time'].day).zfill(2) + '_' +str(row['date_time'].hour).zfill(2) + str(row['date_time'].minute).zfill(2) + 'Z'
