...
def create_master_metrics_csv_alt(master_metrics_csv_output, json_list, version):
    ...
    for full_json_path in json_list:
        ...
        real_json_path = os.path.join(
            os.path.split(full_json_path)[0], nws_lid + "_b0m_stats.json"
        )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Produces metrics for alternative CatFIM."
    )
    parser.add_argument(
        "-d",
        "--catfim-directory",
        help="Path to directory storing CatFIM outputs. This is the most parent dir, usually named by a version.",
        required=True,
    )
    parser.add_argument(
        "-w",
        ...
        if len(huc) == 8:
            ...
            for site in site_list:
                ...
                for category in ["action", "minor", "moderate", "major"]:
                    ...
                    category_grid_path = os.path.join(
                        site_dir, site + "_" + category + "_extent_" + huc + ".tif"
                    )
                    ...
                    if os.path.exists(category_grid_path):
                        ...
                        for benchmark_type in ["nws", "usgs"]:
                            benchmark_grid = os.path.join(
                                TEST_CASES_DIR,
                                benchmark_type + "_test_cases",
                                "validation_data_" + benchmark_type,
                                huc,
                                site,
                                category,
                                "ahps_"
                            ...
                            if os.path.exists(benchmark_grid):
                                ...
                                mask_dict = {
                                    "levees": {
                                        "path": "/data/test_cases/other/zones/leveed_areas_conus.shp",
                                        "buffer": None,
                                        "operation": "exclude",
                                    },
                                    "waterbodies": {
                                        "path": "/data/test_cases/other/zones/nwm_v2_reservoirs.shp",
                                        "buffer": None,
                                        "operation": "exclude",
                                    },
                                    site: {
