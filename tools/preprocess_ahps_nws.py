...
def preprocess_nws(source_dir, destination, reference_raster):
    ...
    ahps_codes = [
        i.name for i in source_dir.glob("*") if i.is_dir() and len(i.name) == 5
    ]
    ...
    for code in ahps_codes:
        ...
        stages, flows = get_thresholds(
            threshold_url, select_by, selector, threshold="all"
        )
        ...
        if code in ["bgwn7", "smit2"]:
            [grids_dir] = [
                directory
                for directory in parent_path.glob("*custom*")
                if directory.is_dir()
            ]
        else:
            ...
            [grids_dir] = [
                directory
                for directory in parent_path.glob("*depth_grid*")
                if directory.is_dir()
            ]
        ...
        grid_paths = [
            grids
            for grids in grids_dir.glob("*.tif*")
            if grids.suffix in [".tif", ".tiff"]
        ]
        ...
        if code in ["eagi1", "bigi1"]:
            ...
            f.write(
                f"{code} : Site workaround grids names based on flows not elevation\n"
            )
        ...
        try:
            ...
            for i in ["action", "minor", "moderate", "major"]:
                ...
                if not grid in ["No Map", "No Threshold", "No Flow"]:
                    ...
                    filled_domain_raster = (
                        outputdir.parent / f"{code}_filled_orig_domain.tif"
                    )
                    ...
                    if not benchmark.crs:
                        ...
                        [ahps_polygons_directory] = [
                            directory
                            for directory in parent_path.glob("*polygon*")
                            if directory.is_dir()
                        ]
                    ...
                    if not filled_domain_raster.exists():
                        ...
                        process_extent(
                            domain, domain_profile, output_raster=filled_domain_raster
                        )
                    ...
                    output_raster = outputdir / (
                        f"ahps_{code}_huc_{huc}_extent_{i}.tif"
                    )

                    with rasterio.Env():
                        with rasterio.open(
                            output_raster, "w", **boolean_profile
                        ) as dst:
                    ...
                    output_flow_file = outputdir / (
                        f"ahps_{code}_huc_{huc}_flows_{i}.csv"
                    )
        ...
        if ahps_directory.exists():
            ...
            grids_attributes = pd.DataFrame(
                data=grids.items(), columns=["magnitude", "path"]
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-s",
        "--source_dir",
        help="Workspace where all source data is located.",
        required=True,
    )
    parser.add_argument(
        "-d",
        "--destination",
        help="Directory where outputs are to be stored",
        required=True,
    )
    parser.add_argument(
        "-r",
