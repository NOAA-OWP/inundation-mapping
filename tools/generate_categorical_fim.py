...
def update_mapping_status(output_mapping_dir, output_flows_dir):
    ...
    flows_df.loc[
        flows_df.eval('HUC8 in @missing_mapping_hucs & mapped == "yes"'), "status"
    ] = (
        flows_df["status"]
        + " and all categories failed to map because missing HUC information"
    )
    flows_df.loc[
        flows_df.eval('HUC8 in @missing_mapping_hucs & mapped == "yes"'), "mapped"
    ] = "no"
...
if __name__ == "__main__":
    ...
    subprocess.call(
        [
            "python3",
            "/foss_fim/tools/generate_categorical_fim_flows.py",
            "-w",
            str(output_flows_dir),
            "-u",
            nwm_us_search,
            "-d",
            nwm_ds_search,
    ...
    subprocess.call(
        [
            "python3",
            "/foss_fim/tools/generate_categorical_fim_mapping.py",
            "-r",
            str(fim_run_dir),
            "-s",
            str(output_flows_dir),
            "-o",
            str(output_mapping_dir),
            "-j",
            str(number_of_jobs),
        ]
    )
