...
def run_alpha_test(
    ...
    if archive_results:
        version_test_case_dir_parent = os.path.join(
            TEST_CASES_DIR,
            benchmark_category + "_test_cases",
            test_id,
            "official_versions",
            version,
        )
    ...
    if os.path.exists(version_test_case_dir_parent):
        if overwrite == True:
            ...
            print(
                "Metrics for ({version}: {test_id}) already exist. Use overwrite flag (-o) to overwrite metrics.".format(
                    version=version, test_id=test_id
                )
            )
            return
    ...
    validation_data_path = os.path.join(
        TEST_CASES_DIR,
        benchmark_category + "_test_cases",
        "validation_data_" + benchmark_category,
    )
    for magnitude in magnitude_list:
        ...
        if benchmark_category in AHPS_BENCHMARK_CATEGORIES:
            ...
            for lid in lid_dir_list:
                ...
                benchmark_lid_raster_path = os.path.join(
                    lid_dir,
                    magnitude,
                    "ahps_"
                    + lid
                ...
                if os.path.exists(benchmark_lid_raster_path):
                    ...
                    lid_list.append(lid)
                    inundation_raster_list.append(
                        os.path.join(
                            version_test_case_dir, lid + "_inundation_extent.tif"
                        )
                    )
                    domain_file_list.append(os.path.join(lid_dir, lid + "_domain.shp"))

        else:
            benchmark_raster_file = os.path.join(
                TEST_CASES_DIR,
                benchmark_category + "_test_cases",
                "validation_data_" + benchmark_category,
                current_huc,
            ...
            inundation_raster_list = [
                os.path.join(version_test_case_dir, "inundation_extent.tif")
            ]
        ...
        for index in range(0, len(benchmark_raster_path_list)):
            ...
            if benchmark_category in AHPS_BENCHMARK_CATEGORIES:
                ...
                if (
                    not os.path.exists(benchmark_raster_path)
                    or not os.path.exists(ahps_domain_file)
                    or not os.path.exists(forecast)
                ):  # Skip loop instance if the benchmark raster doesn't exist.
                    continue
            else:  # If not in AHPS_BENCHMARK_CATEGORIES.
                if not os.path.exists(benchmark_raster_path) or not os.path.exists(
                    forecast
                ):  # Skip loop instance if the benchmark raster doesn't exist.
                    continue
            # Run inundate.
            ...
            try:
                ...
                if inundate_test == 0:
                    ...
                    if benchmark_category in AHPS_BENCHMARK_CATEGORIES:
                        agreement_raster, stats_json, stats_csv = (
                            os.path.join(
                                version_test_case_dir, lid + "total_area_agreement.tif"
                            ),
                            os.path.join(version_test_case_dir, "stats.json"),
                            os.path.join(version_test_case_dir, "stats.csv"),
                        )
                    else:
                        agreement_raster, stats_json, stats_csv = (
                            os.path.join(
                                version_test_case_dir, "total_area_agreement.tif"
                            ),
                            os.path.join(version_test_case_dir, "stats.json"),
                            os.path.join(version_test_case_dir, "stats.csv"),
                        )
                    ...
                    pass
                    print(
                        f"No matching feature IDs between forecast and hydrotable for magnitude: {magnitude}"
                    )
                    # return
            ...
            for output_file in output_file_list:
                if "total_area" in output_file:
                    full_output_file_path = os.path.join(
                        version_test_case_dir, output_file
                    )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Inundation mapping and regression analysis for FOSS FIM. Regression analysis results are stored in the test directory."
    )
    parser.add_argument(
        "-r",
        "--fim-run-dir",
    ...
    parser.add_argument(
        "-b",
        "--version",
        help="The name of the working version in which features are being tested",
        required=True,
        default="",
    )
    parser.add_argument(
        "-t",
        "--test-id",
        help="The test_id to use. Format as: HUC_BENCHMARKTYPE, e.g. 12345678_ble.",
        required=True,
        default="",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "-y", "--magnitude", help="The magnitude to run.", required=False, default=""
    )
    parser.add_argument(
        "-c",
        "--compare-to-previous",
        help="Compare to previous versions of HAND.",
        required=False,
        action="store_true",
    )
    parser.add_argument(
        "-a",
        "--archive-results",
        help='Automatically copy results to the "previous_version" archive for test_id. For admin use only.',
        required=False,
        action="store_true",
    )
    parser.add_argument(
        "-i",
        "--inclusion-area",
        help="Path to shapefile. Contingency metrics will be produced from pixels inside of shapefile extent.",
        required=False,
        default="",
    )
    parser.add_argument(
        "-ib",
        "--inclusion-area-buffer",
        help="Buffer to use when masking contingency metrics with inclusion area.",
        required=False,
        default="0",
    )
    parser.add_argument(
        "-l",
        "--light-run",
        help="Using the light_run option will result in only stat files being written, and NOT grid files.",
        required=False,
        action="store_true",
    )
    parser.add_argument(
    ...
    if not os.path.exists(
        os.path.join(os.environ["outputDataDir"], args["fim_run_dir"])
    ):
        print(
            TRED_BOLD
            + "Warning: "
        ...
        print(
            WHITE_BOLD
            + "Please provide the parent directory name for fim_run.sh outputs. These outputs are usually written in a subdirectory, e.g. outputs/123456/123456."
            + ENDC
        )
        print()
        exit_flag = True
    ...
    try:
        ...
        print(
            ...
            + "Error: "
            + WHITE_BOLD
            + "The provided inclusion_area_buffer (-ib) "
            + CYAN_BOLD
            + args["inclusion_area_buffer"]
            + WHITE_BOLD
    ...
    if args["magnitude"] == "":
        if "ble" in args["test_id"].split("_"):
            ...
            print(
                ...
                + args["magnitude"]
                + WHITE_BOLD
                + " is invalid. ble options include: 100yr, 500yr. ahps options include action, minor, moderate, major."
                + ENDC
            )
            exit_flag = True
