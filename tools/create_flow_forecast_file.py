...
def create_flow_forecast_file(huc, ble_geodatabase, nwm_geodatabase, output_parent_dir, ble_xs_layer_name = 'XS', nwm_stream_layer_name = 'nwm_streams', nwm_feature_id_field ='ID'):
    ...
    def fill_missing_flows(forecast: pd.DataFrame, nwm_river_layer: pd.DataFrame):
        """
        This function will fill in missing flows in the forecast dataframe. It will do this by finding segments that don't intersect with the forecast dataframe and then finding the upstream and downstream segments. It will then assign the average of the two to the missing segment.
        ...
        n = 1
        while True:
            # Merge flows with nwm_river_layer
            merged = nwm_river_layer.merge(forecast, left_on='ID', right_on='feature_id', how='left') # this will be used to find upstream and downstream flows
            ...
            for i, row in nonintersects.iterrows():
                ...
                if len(upstreams) > 0 and not upstreams['discharge'].isnull().values.any(): # if all upstream segments have flows
                ...
                if len(downstream) > 0:
                    ...
                    if (downstream['discharge'].isnull().sum() + downstream_upstreams['discharge'].isnull().sum()) == 1: # all segments have flow except for the one being filled in
                        new_flow.append(downstream['discharge'].iloc[0] - downstream_upstreams['discharge'].sum())

                # Add row data to forecast
                if len(new_flow) > 0:
                    forecast = pd.concat([forecast, pd.DataFrame({'feature_id': row['ID'], 'discharge': np.mean(new_flow)}, index=[0])]).reset_index(drop=True)
                    updated = True
