...
def create_flow_forecast_file(
    ...
    ble_huc_layer_name="S_HUC_Ar",
    ble_huc_id_field="HUC_CODE",
    nwm_stream_layer_name="RouteLink_FL_2020_04_07",
    ...
    nwm_river_layer = gpd.read_file(
        nwm_geodatabase, bbox=xs_layer, layer=nwm_stream_layer_name
    )
    ...
    for i, flow in enumerate(flow_fields):
        ...
        forecast = forecast.rename(
            columns={nwm_feature_id_field: "feature_id", flow: "discharge"}
        )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Produce forecast flow files from BLE datasets"
    )
    parser.add_argument(
        "-b",
        "--ble-geodatabase",
    ...
    parser.add_argument(
        "-n",
        "--nwm-geodatabase",
        help="NWM geodatabase (.gdb file extension).",
        required=True,
    )
    parser.add_argument(
        "-o",
    ...
    parser.add_argument(
        "-hu",
        "--ble-huc-layer-name",
        help='BLE huc layer. Default layer is "S_HUC_Ar" (sometimes it is "S_HUC_ar").',
        required=False,
        default="S_HUC_Ar",
    )
    parser.add_argument(
        "-huid",
        "--ble-huc-id-field",
        help='BLE id field in the ble-huc-layer-name. Default field is "HUC_CODE".',
        required=False,
        default="HUC_CODE",
    )
    parser.add_argument(
        ...
        help='NWM streams layer. Default layer is "RouteLink_FL_2020_04_07")',
        required=False,
        default="RouteLink_FL_2020_04_07",
    )
    parser.add_argument(
        "-f",
