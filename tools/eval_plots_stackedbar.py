...
def eval_plot_stack_data_prep(metric_csv, versions=[]):
    ...
    if versions:
        ...
        for v in versions:
            assert v in metrics.version.unique(), (
...
def diff_bar_plots(versions, metric_csv, category, outfig, stat="CSI"):
    ...
    assert len(versions) == 2, (
        "You can only produce the comparison plots when comparing 2 fim versions. "
        f"You have input {len(versions)}."
    )
    # CSI, MCC, and TPR scores are better when closer to 1,
    # however PND and FAR scores are better when closer to 0
    ...
    for i, site in enumerate(data.nws_lid.unique()):
        ...
        ax[i].set_yticks(
            np.arange(len(subplot_data)), labels=subplot_data.index.map(inverted_mag).astype(str)
        )
...
def iter_benchmarks(
    metric_csv, workspace, versions=[], individual_plots=False, show_iqr=False, diff_stat=False
):
    ...
    for (benchmark_source, extent_configuration), benchmark_metrics in benchmark_by_source:
        ...
        output_png = Path(output_workspace) / (
            f"{benchmark_source}_{extent_configuration.lower()}_"
            f"stackedbar{'_indiv'if individual_plots else ''}.png"
        )
        if diff_stat:
            output_png = (
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        "Only 2 versions can be used with this option. Input one of the following statistics "
        "to be used for comparison: ('CSI', 'MCC', 'TPR', 'PND', 'FAR').",
        default=False,
        required=False,
    )
