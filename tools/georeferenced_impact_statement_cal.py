...
"""
    ...
    for branch in branch_list:
        ...
        catchments_path = os.path.join(branch_sub, f'gw_catchments_reaches_filtered_addedAttributes_crosswalked_{branch}.gpkg')
        ...
        hydroid_list = catchments_results['HydroID'].unique().tolist() # List of all overlapped catchments in the branch

        polygon_types = {
            'Action' : {'polygon' : target_polygon[target_polygon['ImpactStatus'] == 'Action'], 'flow_col' : 'ActionFlow', 'weighted_variables'
                        : None}, 
            'Minor' : {'polygon' : target_polygon[target_polygon['ImpactStatus'] == 'Minor'], 'flow_col' : 'MinorFlow', 'weighted_variables'
                        : None},
            'Moderate' : {'polygon' : target_polygon[target_polygon['ImpactStatus'] == 'Moderate'], 'flow_col' : 'ModerateFlow', 'weighted_variables'
                        : None},
            'Major' : {'polygon' : target_polygon[target_polygon['ImpactStatus'] == 'Major'], 'flow_col' : 'MajorFlow', 'weighted_variables'
                        : None}
        }
        ...
        for catchment in hydroid_list:
            ...
            with rasterio.open(rem_path) as src:
                for poly_type, poly_data in polygon_types.items():
                    ...
                    if polygon.crs != src.crs:
                        action_pol = action_pol.to_crs(src.crs)
            ...
            df_ = pd.DataFrame({'HydroID' : catchment_array, 'discharge' : new_Q, 'stage' : stage_column, 'branch_id': branch_array})
            cal_data.append(df_)
    ...
    new_hydrotable.loc[(new_hydrotable['impact_sta'] == 'True'), 'discharge_cms'] = new_hydrotable['discharge']
