...
def update_rating_curve(
    fim_directory, output_csv, htable_path, output_src_json_file, huc6
):
    ...
    df_gmed = df_gmed[
        df_gmed.hydroid != 0
    ]  # remove entries that do not have a valid hydroid
    ...
    if "orig_discharge_cms" in df_htable.columns:
        ...
        df_htable.rename(
            columns={
                "orig_discharge_cms": "discharge_cms",
                "default_ManningN": "ManningN",
            },
            inplace=True,
        )
    else:
    ...
    output_calc_n_csv = os.path.join(
        fim_directory, huc6, "calc_src_n_vals_" + huc6 + ".csv"
    )
    ...
    df_htable["discharge_cms"].mask(
        df_htable["orig_discharge_cms"] == 0.0, 0.0, inplace=True
    )
    df_htable["discharge_cms"].mask(
        df_htable["orig_discharge_cms"] == -999, -999, inplace=True
    )
...
def ingest_points_layer(points_layer, fim_directory, wbd_path):
    ...
    for huc6 in huc6_list:
        ...
        catchments_path = os.path.join(
            fim_directory, huc6, "catchments_" + huc6 + ".tif"
        )
        ...
        if not os.path.exists(htable_path):
            ...
            continue
        output_src_json_file = os.path.join(
            fim_directory, huc6, "rating_curves_" + huc6 + ".json"
        )
        ...
        output_csv = os.path.join(
            fim_directory, huc6, "user_supplied_n_vals_" + huc6 + ".csv"
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-w", "--wbd-path", help="Path to national HUC6 layer.", required=True
    )
