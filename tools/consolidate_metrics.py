...
def Consolidate_metrics(
    ...
    # find matching rows
    consolidated_metrics_df = find_matching_rows_by_attribute_value(
        consolidated_metrics_df, 'version', versions
    )
...
def impute_missing_MS_with_FR(consolidated_metrics_df):
    ...
    indices_of_ec = {
        ec: consolidated_metrics_df.index[consolidated_metrics_df.loc[:, 'extent_config'] == ec]
    ...
    for ec in extent_config:
        ...
        if len(unique_version) > 1:
            raise ValueError(
                f"{ec} version has more than one extent. Only pass one version per extent configuration."
            )
    ...
    fr_entries.loc[:, 'version'] = consolidated_metrics_df.loc[
        indices_of_ec['MS'], 'version'
    ].unique()

    consolidated_metrics_df = pd.concat((consolidated_metrics_df, fr_entries)).reset_index(
        drop=True
    )
...
def apply_metrics_to_pivot_table(consolidated_metrics_pivot):
    ...
    column_generator = (
        row_wise_function(consolidated_metrics_pivot, met_name, met_func)
...
def consolidate_metrics(benchmark, zones=['total_area']):
    ...
    for zone in zones:
        file_pattern_to_glob = os.path.join(
            TEST_CASES_DIR, f'{benchmark}_test_cases', '**', f'{zone}_stats.csv'
        )
    ...
    return pd.concat((metadata_df, metrics_df), axis=1)
...
def metadata_dataframe_from_file_names_for_ble(file_names):
    ...
    return pd.DataFrame(file_name_index_dict)
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '-b', '--benchmarks', help='Allowed benchmarks', required=False, default='all', nargs="+"
    )
    parser.add_argument(
        '-v', '--versions', help='Allowed versions', required=False, default='all', nargs="+"
    )
    parser.add_argument(
        '-z', '--zones', help='Allowed zones', required=False, default='total_area', nargs="+"
    )
    parser.add_argument(
        '-o', '--metrics-output-csv', help='File path to outputs csv', required=False, default=None
    )
    parser.add_argument(
        '-i',
        '--impute-missing_ms',
        help='Imputes FR metrics in HUCS with no MS. Only supports one version per extent config',
        required=False,
        action='store_true',
        default=False,
    )
