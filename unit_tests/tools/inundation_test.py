...
class test_inundate(unittest.TestCase):
    ...
    @pytest.mark.skip(
        reason="Inundate_gms will be rebuilt in the future, so this test will be left broken."
    )
    def test_inundate_create_inundation_raster_single_branch_success(self):
        ...
        assert in_polys[0] == None, "Expected no inundation_polys records"

        msg = f"Expected file {params['expected_inundation_raster']} but it does not exist."
        assert os.path.exists(params["expected_inundation_raster"]) == True, msg
