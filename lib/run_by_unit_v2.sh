...
$libDir/split_flows.py $outputDataDir/demDerived_reaches.gpkg "$PROJ" $outputDataDir/dem_thalwegCond_filled.tif $outputDataDir/demDerived_reaches_split.gpkg $outputDataDir/demDerived_reaches_split_points.gpkg $maxSplitDistance_meters $manning_n $slope_min
Tcount
...
date -u
Tstart
[ ! -f $outputDataDir/slopes_d8_thalwegCond_filled_clipped_masked.tif ] && \
gdal_calc.py --type=Float32 --overwrite --co "COMPRESS=LZW" --co "BIGTIFF=YES" --co "TILED=YES" -A $outputDataDir/slopes_d8_thalwegCond_filled_clipped.tif -B $outputDataDir/gw_catchments_reaches_clipped_addedAttributes.tif --calc="(A*(B>0))+((B<=0)*-1)" --NoDataValue=-1 --outfile=$outputDataDir/"slopes_d8_thalwegCond_filled_clipped_masked.tif"
Tcount
