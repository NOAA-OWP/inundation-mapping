...
if __name__ == '__main__':
    ...
    attributes_vector_file, attribute, subset_vectors, out_files, verbose = args["vector_attributes"], args["attribute_id"], args["subset_vectors"], args["out_files"], args["verbose"]
    ...
    for subset_vector,out_file in tqdm(zip(subset_vectors,out_files),disable=(not verbose),total=len(subset_vectors)):

        if verbose:
            print("Query \'{}\' by attribute in \'{}\' ...".format(out_file.split('/')[-1].split('.')[0],
                                                               attributes_vector_file.split('/')[-1].split('.')[0]))
        
        stream_polys.query_vectors_by_branch(subset_vector,out_file)
