...
def finalize_srcs(srcbase,srcfull,hydrotable,output_srcfull=None,output_hydrotable=None):
    ...
    srcbase['Discharge (m3s-1)'] = srcbase['WetArea (m2)']* \
                                          pow(srcbase['HydraulicRadius (m)'],2.0/3)* \
                                          pow(srcbase['SLOPE'],0.5)/srcbase['ManningN']
...
if __name__ == '__main__':
    ...
    parser.add_argument('-r','--output-srcfull', help='Output crosswalked synthetic rating curve table', required=False,default=None)
