...
hucNumber="$1"
outputHucDataDir=$outputDataDir/$hucNumber
if [ ! -d "$outputHucDataDir" ]; then mkdir $outputHucDataDir ;fi
...
ogr2ogr -f GPKG -dialect sqlite -sql "select ST_buffer(geom, $bufferDistance) from 'WBDHU6'" $outputHucDataDir/wbd_projected_buffered.gpkg $outputHucDataDir/wbd_projected.gpkg
Tcount
...
gdalwarp -cutline $outputHucDataDir/wbd_projected_buffered.gpkg -crop_to_cutline -ot Int32 -r bilinear -of "GTiff" -overwrite -co "BLOCKXSIZE=512" -co "BLOCKYSIZE=512" -co "TILED=YES" -co "COMPRESS=LZW" -co "BIGTIFF=YES" $inputDataDir/mosaic.vrt $outputHucDataDir/dem.tif
Tcount
...
# date -u
# Tstart
# [ ! -f $outputHucDataDir/dem_thalwegCond_filled.tif ] && \
# $taudemDir/flowdircond -p $outputHucDataDir/flowdir_d8_burned_filled_flows.tif -z $outputHucDataDir/dem_burned_filled.tif -zfdc $outputHucDataDir/dem_thalwegCond_filled.tif
# Tcount
...
# Tstart
# [ ! -f $outputHucDataDir/dem_thalwegCond_filled.tif ] && \
# rd_depression_filling $outputHucDataDir/dem_thalwegCond.tif $outputHucDataDir/dem_thalwegCond_filled.tif
# Tcount
...
# date -u
# Tstart
# [ ! -f $outputHucDataDir/flowdir_d8_thalwegCond_filled.tif ] && [ ! -f $outputHucDataDir/slopes_d8_burned_filled.tif ] && \
# mpiexec -n $ncores_fd $taudemDir2/d8flowdir -fel $outputHucDataDir/dem_thalwegCond_filled.tif -p $outputHucDataDir/flowdir_d8_thalwegCond_filled.tif -sd8 $outputHucDataDir/slopes_d8_thalwegCond_filled.tif
# Tcount
...
$libDir/rem.py -d $outputHucDataDir/dem_thalwegCond.tif -w $outputHucDataDir/gw_catchments_pixels.tif -o $outputHucDataDir/rem.tif -n $ndv
Tcount
...
ogr2ogr -progress -f GPKG -clipsrc $outputHucDataDir/wbd_projected.gpkg $outputHucDataDir/demDerived_reaches_split_clipped.gpkg $outputHucDataDir/demDerived_reaches_split.gpkg
Tcount
...
# date -u
# Tstart
# [ ! -f $outputHucDataDir/inundation_library.gpkg ] && \
