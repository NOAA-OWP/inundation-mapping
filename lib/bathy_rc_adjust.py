...
def bathy_rc_lookup(input_src_base,input_bathy_fileName,output_bathy_fileName,output_bathy_streamorder_fileName):
    ...
    output_bathy['Top Width Diff (m)'].mask(output_bathy['Stage']== 0,inplace=True) # mask out stage = 0 rows in SRC
    ...
    output_bathy['XS Bankfull Area Ratio'].mask((output_bathy['XS Area Diff (m2)']<0) | (output_bathy['XS Area (m2)'] == 0),inplace=True)  # masking negative XS Area Diff and XS Area = 0
    output_bathy['XS Area Diff (m2)'].mask((output_bathy['XS Area Diff (m2)']<0) | (output_bathy['XS Area (m2)'] == 0),inplace=True) # masking negative XS Area Diff and XS Area = 0
    output_bathy['XS Area Diff (m2)'].mask(output_bathy['XS Bankfull Area Ratio']>10,inplace=True) # remove bogus values (crosswalk issues or bad bankfull data points)
    output_bathy['XS Bankfull Area Ratio'].mask(output_bathy['XS Bankfull Area Ratio']>10,inplace=True) # remove bogus values (crosswalk issues or bad bankfull data points)

    print('Average: bankfull XS Area crosswalk difference (m): ' + str(output_bathy['XS Area Diff (m2)'].mean()))
    print('Minimum: bankfull XS Area crosswalk difference (m): ' + str(output_bathy['XS Area Diff (m2)'].min()))
    print('Maximum: bankfull XS Area crosswalk difference (m): ' + str(output_bathy['XS Area Diff (m2)'].max()))
    print('STD: bankfull XS Area crosswalk difference (m): ' + str(output_bathy['XS Area Diff (m2)'].std()))

    ## Bin XS Bankfull Area Ratio by stream order
    stream_order_bathy_ratio = output_bathy[['order_','Stage','XS Bankfull Area Ratio']]
    stream_order_bathy_ratio['Stage'].mask(stream_order_bathy_ratio['XS Bankfull Area Ratio'].isnull(),inplace=True) ## mask stage values when XS Bankfull Area Ratio is null
    ...
    stream_order_bathy_ratio = (stream_order_bathy_ratio.ffill()+stream_order_bathy_ratio.bfill())/2  # fill XS Bankfull Area Ratio and Stage values if no values were found in the grouby calcs
    stream_order_bathy_ratio = stream_order_bathy_ratio.bfill().ffill() # fill first and last stream order values if needed
    ...
    #print(output_bathy.head)
    output_bathy.to_csv(output_bathy_fileName,index=False)
    stream_order_bathy_ratio.to_csv(output_bathy_streamorder_fileName,index=False)
