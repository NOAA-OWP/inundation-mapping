...
'''
The script ingests a RAS2FIM rating curve csv and a NWM flow recurrence interval database. The gage location will be associated to the corresponding hydroID and attributed with the HAND elevation value
...
- Log any signifant differences (or negative HAND values) btw the NWM flow value and closest RAS2FIM rating flow
- Produce log file
...
- ras_rc_filepath:     RAS2FIM rating curve database (produced by reformat_ras_rating_curve.py in ras2fim repo)
...
def create_ras2fim_rating_database(ras_rc_filepath, ras_elev_df, nwm_recurr_filepath, log_dir):
    ...
    # convert RAS2FIM flow from cfs to cms
    ras_rc_df['discharge_cms'] = ras_rc_df.flow / 35.3147
    ras_rc_df = ras_rc_df.drop(columns=["flow"])
    ...
    for interval in recurr_intervals:
        ...
        # Calculate flow difference (variance) to check for large discrepancies btw NWM flow and RAS2FIM rating closest flow
        ...
        # Log any signifant differences btw the NWM flow value and closest RAS2FIM rating flow (this ensures that we consistently sample the RAS2FIM rating curves at known intervals - NWM recur flow)
...
if __name__ == '__main__':
    ## Parse arguments.
    parser = argparse.ArgumentParser(description='Adjusts rating curve with database of RAS2FIM reach average rating curves (calculated WSE/flow).')
    parser.add_argument('-run_dir','--run-dir',help='Parent directory of FIM run.',required=True)
    parser.add_argument('-ras_rc','--ras2fim-ratings',help='Path to RAS2FIM rating curve (reach avg) csv file',required=True)
