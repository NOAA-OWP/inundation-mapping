...
def bathy_rc_lookup(
    ...
    modified_src_base = input_src_base.merge(
        input_bathy.loc[
            :, ["feature_id", "BANKFULL_WIDTH (m)", "BANKFULL_XSEC_AREA (m2)"]
        ],
        how="left",
        on="feature_id",
    )
    ...
    if modified_src_base["BANKFULL_WIDTH (m)"].count() == 0:
        ...
        find_thalweg_notch = find_thalweg_notch[
            find_thalweg_notch["SA_div_flag"].notnull()
        ]
        ...
        modified_src_base = modified_src_base.merge(
            find_thalweg_notch.loc[:, ["HydroID", "Thalweg_burn_elev"]],
            how="left",
            on="HydroID",
        )
        ...
        output_bathy["XS Bankfull Area Ratio"].mask(
            (output_bathy["XS Area Diff (m2)"] < 0)
            | (output_bathy["XS Area (m2)"] == 0),
            inplace=True,
        )
        ## masking negative XS Area Diff and XS Area = 0
        ...
        output_bathy = output_bathy.merge(
        ...
        modified_src_base["lookup_stage_diff"] = (
            modified_src_base[["median_stage_bankfull", "Thalweg_burn_elev"]].max(
                axis=1
            )
        ...
        xs_area_hydroid_lookup["bathy_calc_xs_area"] = (
            ...
            * xs_area_hydroid_lookup["mean_xs_area_ratio"]
        ) - xs_area_hydroid_lookup["XS Area (m2)"]
        ...
        modified_src_base["XS Area (m2)"] = (
            modified_src_base["orig_XS Area (m2)"]
            + modified_src_base["bathy_calc_xs_area"]
        )
        ...
        modified_src_base["Discharge (m3s-1)"].mask(
            modified_src_base["Stage"] == modified_src_base["Thalweg_burn_elev"],
            0,
            inplace=True,
        )
        modified_src_base["Discharge (m3s-1)"].mask(
