...
def bathy_rc_lookup(
    ...
    if input_src_base.feature_id.dtype != 'int':
        input_src_base.feature_id = input_src_base.feature_id.astype(int)
    ...
    input_bathy = input_bathy.rename(
        columns={
            'COMID': 'feature_id',
    ...
    modified_src_base = input_src_base.merge(
        input_bathy.loc[:, ['feature_id', 'BANKFULL_WIDTH (m)', 'BANKFULL_XSEC_AREA (m2)']],
        how='left',
        on='feature_id',
    )
    ...
    if modified_src_base['BANKFULL_WIDTH (m)'].count() == 0:
        print(
            'No matching feature_id found between input bathy data and src_base --> No bathy calculations added to SRC!'
        )
        ...
        modified_src_base['SA_div'] = modified_src_base['SurfaceArea (m2)'].div(
        ...
        modified_src_base['SA_div'].mask(
            (modified_src_base['Stage'] == 0)
            | (modified_src_base['SA_div'] < sa_ratio_flag)
        ...
        modified_src_base = modified_src_base.rename(
            columns={'Discharge (m3s-1)': 'Discharge (m3s-1)_nobathy'}
        )
        modified_src_base['XS Area (m2)_bathy_adj'] = (
            modified_src_base['XS Area (m2)'] + modified_src_base['bathy_calc_xs_area']
        )
        modified_src_base['Volume (m3)_bathy_adj'] = (
            modified_src_base['XS Area (m2)_bathy_adj'] * modified_src_base['LENGTHKM'] * 1000
        )
        modified_src_base['WetArea (m2)_bathy_adj'] = (
            modified_src_base['Volume (m3)_bathy_adj'] / modified_src_base['LENGTHKM'] / 1000
        )
        modified_src_base['HydraulicRadius (m)_bathy_adj'] = (
            modified_src_base['WetArea (m2)_bathy_adj'] / modified_src_base['WettedPerimeter (m)']
        )
        ...
        modified_src_base['HydraulicRadius (m)_bathy_adj'].mask(
            (modified_src_base['HydraulicRadius (m)_bathy_adj'] > thal_hyd_radius_flag)
