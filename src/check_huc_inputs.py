...
def __check_for_membership(hucs, accepted_hucs_set):
    ...
    for huc in hucs:
        if huc not in accepted_hucs_set:
            raise KeyError(
                "HUC {} not found in available inputs. Edit HUC inputs or acquire datasets and try again".format(
                    huc
                )
            )
...
def check_hucs(hucs):
    ...
    accepted_hucs = __read_included_files(
        os.path.join(os.environ["inputDataDir"], "huc_lists")
    )
