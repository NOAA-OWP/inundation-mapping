...
def adjust_headwaters(huc, nhd_streams, nwm_headwaters, nws_lids, headwater_id):
    ...
    nhd_streams_adj = nhd_streams.loc[
        (nhd_streams.headwaters_id > 0)
    ...
    if nwm_headwaters["site_id"].dtype != "int":
        nwm_headwaters["site_id"] = nwm_headwaters["site_id"].astype(int)
    headwater_limited = nwm_headwaters.merge(
        nhd_streams_adj[["headwaters_id", "mainstem"]],
        left_on="site_id",
        right_on="headwaters_id",
        how="right",
    )
    headwater_limited = headwater_limited.drop(columns=["headwaters_id"])

    nws_lid_limited = nws_lids.merge(
        nhd_streams[["nws_lid"]], left_on="site_id", right_on="nws_lid", how="right"
    )
    ...
    if headwater_pts is not None:
        ...
        for index, point in headwater_pts.iterrows():
            ...
            if point.pt_type == "nwm_headwater":
                ...
                closest_stream = nhd_streams_adj.loc[
                    nhd_streams_adj["headwaters_id"] == point[headwater_id]
                ]
            else:
                # Closest segment to ahps site
                closest_stream = nhd_streams.loc[
                    nhd_streams["nws_lid"] == point[headwater_id]
                ]
            ...
            pointdistancetoline = pygeos.linear.line_locate_point(
                streambin_geom, pointbin_geom
            )
            referencedpoint = pygeos.linear.line_interpolate_point(
                streambin_geom, pointdistancetoline
            )
            ...
            if point.pt_type == "nwm_headwater":
                ...
                sortline = pd.DataFrame(
                    {"geom": cumulative_line, "dist": relativedistlst}
                ).sort_values("dist")
                ...
                try:
                    ...
                    nhd_streams.loc[
                        nhd_streams.NHDPlusID == closest_stream.NHDPlusID.values[0],
                        "geometry",
                    ] = LineString(line1)
                except:
        ...
        nhd_streams = nhd_streams.drop(
            columns=["is_relevant_stream", "headwaters_id", "downstream_of_headwater"]
        )
        ...
        if len(snapped_ahps) > 0:
            snapped_ahps_points = gpd.GeoDataFrame(
                {
                    "pt_type": "nws_lid",
                    headwater_id: nws_lid,
                    "mainstem": True,
                    "geometry": snapped_ahps,
                },
                geometry="geometry",
                crs=PREP_PROJECTION,
            )
        ...
        nhd_headwater_points_adj = gpd.GeoDataFrame(
            {
                "pt_type": "NHDPlusID",
                headwater_id: nhd_headwater_streams_adj["NHDPlusID"],
                "mainstem": False,
                "geometry": hw_points,
            },
            geometry="geometry",
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="adjust headwater stream geometery based on headwater start points"
    )
    parser.add_argument("-f", "--huc", help="huc number", required=True)
    parser.add_argument(
        "-l", "--nhd-streams", help="NHDPlus HR geodataframe", required=True
    )
    parser.add_argument(
        "-p", "--nwm-headwaters", help="Headwater points layer", required=True, type=str
    )
    parser.add_argument(
        "-s",
    ...
    parser.add_argument(
        "-a",
        "--adj-headwater-points-fileName",
        help="Output adj headwater points layer name",
        required=False,
        type=str,
        default=None,
    )
    parser.add_argument(
        ...
        help="Output headwater points layer name",
        required=False,
        type=str,
        default=None,
    )
    ...
    parser.add_argument(
        "-i", "--headwater-id", help="Headwater id column name", required=True
    )
    ...
    if subset_nhd_streams_fileName is not None:
        adj_streams_gdf.to_file(
            args["subset_nhd_streams_fileName"],
            driver=getDriver(args["subset_nhd_streams_fileName"]),
        )
    ...
    if adj_headwater_points_fileName is not None:
        adj_headwaters_gdf.to_file(
            args["adj_headwater_points_fileName"],
            driver=getDriver(args["adj_headwater_points_fileName"]),
        )
