...
def adjust_headwaters(huc, nhd_streams, nwm_headwaters, nws_lids, headwater_id):
    ...
    nhd_streams_adj = nhd_streams.loc[
        (nhd_streams.headwaters_id > 0) & (nhd_streams.downstream_of_headwater == False), :
    ].copy()
        ...
        for index, point in headwater_pts.iterrows():
            ...
            if point.pt_type == 'nwm_headwater':
                ...
                try:
                    ...
                    nhd_streams.loc[
                        nhd_streams.NHDPlusID == closest_stream.NHDPlusID.values[0], 'geometry'
                    ] = LineString(line1)
                except:
...
if __name__ == '__main__':
    ...
    if headwater_points_fileName is not None:
        headwater_points_fileName.to_file(
            args['headwater_points_fileName'], driver=getDriver(args['headwater_points_fileName'])
        )
