...
def adjust_thalweg_laterally(
    ...
    for ji, window in elevation_raster_object.block_windows(
        ...
        minimized_dem_window = minimized_dem_window.reshape(window_shape).astype(
            np.float32
        )

        dem_lateral_thalweg_adj_object.write(
            minimized_dem_window, window=window, indexes=1
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-e", "--elevation_raster", help="Raster of elevation.", required=True
    )
    parser.add_argument(
        "-s",
        "--stream_raster",
    ...
    parser.add_argument(
        "-a",
        "--allocation_raster",
        help="Raster of thalweg allocation zones.",
        required=True,
    )
    parser.add_argument(
        "-d",
