...
def output_cleanup(
    ...
    if is_production and not is_viz_post_processing:
        whitelist_directory(
            output_folder_path, production_whitelist, additional_whitelist
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "output_folder_path", type=str, help="Path to the outputs for the specific huc"
    )
    parser.add_argument(
        "-w",
        "--additional_whitelist",
        type=str,
        help="List of additional files to keep in a production run",
    )
    parser.add_argument(
        "-p",
        "--is_production",
        help="Keep only white-listed files for production runs",
        action="store_true",
    )
    parser.add_argument(
        "-v",
        "--is_viz_post_processing",
        help="Formats output files to be useful for Viz",
        action="store_true",
    )
