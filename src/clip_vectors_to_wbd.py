...
def subset_vector_layers(
    ...
    great_lakes = gpd.read_file(great_lakes_filename, mask=wbd_buffer).reset_index(
        drop=True
    )

    if not great_lakes.empty:
        print(
            "Masking Great Lakes for HUC{} {}".format(hucUnitLength, hucCode),
            flush=True,
        )
        ...
        wbd_buffer.to_file(
            wbd_buffer_filename, driver=getDriver(wbd_buffer_filename), index=False
        )
    ...
    print(
        "Subsetting NWM Lakes for HUC{} {}".format(hucUnitLength, hucCode), flush=True
    )
    ...
    print(
        "Subsetting NLD levee lines for HUC{} {}".format(hucUnitLength, hucCode),
        flush=True,
    )
    ...
    print(
        "Subsetting NHD Headwater Points for HUC{} {}".format(hucUnitLength, hucCode),
        flush=True,
    )
    ...
    # Subset nhd streams
    print(
        "Querying NHD Streams for HUC{} {}".format(hucUnitLength, hucCode), flush=True
    )
    ...
    if len(nhd_streams) > 0:
        ...
        threshold_segments = gpd.overlay(
            nhd_streams, wbd_buffer, how="symmetric_difference"
        )
    ...
    print(
        "Subsetting NWM Catchments for HUC{} {}".format(hucUnitLength, hucCode),
        flush=True,
    )
    ...
    # Subset nwm streams
    print(
        "Subsetting NWM Streams and deriving headwaters for HUC{} {}".format(
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-d", "--hucCode", help="HUC boundary ID", required=True, type=str
    )
    parser.add_argument("-w", "--nwm-streams", help="NWM flowlines", required=True)
    parser.add_argument(
        "-s", "--nhd-streams", help="NHDPlus HR burnline", required=True
    )
    parser.add_argument("-l", "--nwm-lakes", help="NWM Lakes", required=True)
    parser.add_argument(
        "-r",
        "--nld-lines",
        help="Levee vectors to use within project path",
        required=True,
    )
    parser.add_argument("-g", "--wbd", help="HUC boundary", required=True)
    parser.add_argument(
        "-f", "--wbd-buffer", help="Buffered HUC boundary", required=True
    )
    ...
    parser.add_argument(
        "-v", "--landsea", help="LandSea - land boundary", required=True
    )
    parser.add_argument(
        "-c", "--subset-nhd-streams", help="NHD streams subset", required=True
    )
    parser.add_argument(
        "-z",
        "--subset-nld-lines",
        help="Subset of NLD levee vectors for HUC",
        required=True,
    )
    parser.add_argument("-a", "--subset-lakes", help="NWM lake subset", required=True)
    parser.add_argument(
        "-n", "--subset-catchments", help="NWM catchments subset", required=True
    )
    parser.add_argument(
        "-e",
        "--subset-nhd-headwaters",
        help="NHD headwaters subset",
        required=True,
        default=None,
    )
    parser.add_argument(
        "-b", "--subset-nwm-streams", help="NWM streams subset", required=True
    )
    ...
    parser.add_argument(
        "-gl", "--great-lakes-filename", help="Great Lakes layer", required=True
    )
    parser.add_argument(
        "-wb",
        "--wbd-buffer-distance",
        help="WBD Mask buffer distance",
        required=True,
        type=int,
    )
    parser.add_argument(
        "-lb",
        "--lake-buffer-distance",
        help="Great Lakes Mask buffer distance",
        required=True,
        type=int,
    )
