...
def crosswalk_usgs_gage(
    ...
    closest_catchment = gpd.sjoin(
        usgs_gages, input_catchment, how="left", op="within"
    ).reset_index(drop=True)
    closest_hydro_id = closest_catchment.filter(
        items=[
            "location_id",
            "HydroID",
            "min_thal_elev",
    ...
    usgs_gages = usgs_gages.loc[
        usgs_gages.location_id.isin(list(closest_hydro_id.location_id))
    ]
    ...
    for index, gage in usgs_gages.iterrows():
        ...
        hydro_id = closest_hydro_id.loc[
            closest_hydro_id.location_id == gage.location_id
        ].HydroID.item()
        str_order = str(
            int(
                closest_hydro_id.loc[
                    closest_hydro_id.location_id == gage.location_id
                ].order_.item()
            )
        )
        ...
        gage_distance_to_line = pygeos.linear.line_locate_point(
            stream_bin_geom, gage_bin_geom
        )
        referenced_gage = pygeos.linear.line_interpolate_point(
            stream_bin_geom, gage_distance_to_line
        )
        ...
        dem_m_elev = round(
            list(rasterio.sample.sample_gen(dem_m, shply_referenced_gage.coords))[
                0
            ].item(),
            2,
        )
        dem_adj_elev = round(
            list(rasterio.sample.sample_gen(dem_adj, shply_referenced_gage.coords))[
                0
            ].item(),
            2,
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-gages", "--usgs-gages-filename", help="USGS gages", required=True
    )
    ...
    parser.add_argument(
        "-cat",
        "--input-catchment-filename",
        help="DEM derived catchments",
        required=True,
    )
    parser.add_argument(
        "-wbd", "--wbd-buffer-filename", help="WBD buffer", required=True
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="Table to append data",
        required=True,
    )
    parser.add_argument(
        "-e",
        "--extent",
        help="extent configuration entered by user when running fim_run.sh",
        required=True,
    )
