...
def process_branch(sub_branch_path, branch):
    ...
    input_flows_file = os.path.join(sub_branch_path, f'demDerived_reaches_split_filtered_addedAttributes_crosswalked_{branch}.gpkg')
    ...
    input_src_full['HydraulicRadius (m)'] = input_src_full['WetArea (m2)'] / input_src_full['WettedPerimeter (m)']
