...
def aggregate_fim_outputs(args):
    ...
    for huc in huc_list:
        ...
        if len(huc) > 6:
            ...
            if os.path.isfile(aggregate_hydrotable):
                hydrotable.to_csv(
                    aggregate_hydrotable, index=False, mode="a", header=False
                )
            else:
    ...
    with open(aggregate_hydrotable) as csvf:
        ...
        for row in csvReader:
            if (
                row["HydroID"].lstrip("0") in src_data
                and "nwm_feature_id" not in src_data[row["HydroID"].lstrip("0")]
            ):
                src_data[row["HydroID"].lstrip("0")]["nwm_feature_id"] = row[
                    "feature_id"
                ]
    ...
    if huc6 not in huc_list:
        ...
        rem_list = [
            os.path.join(fim_out_dir, huc, "rem_zeroed_masked.tif")
            for huc in subset_huc6_list
        ]
        ...
        catchment_list = [
            os.path.join(
                fim_out_dir, huc, "gw_catchments_reaches_filtered_addedAttributes.tif"
            )
            for huc in subset_huc6_list
        ]
        ...
        catchment_filename = os.path.join(
            fim_out_dir, huc6, "gw_catchments_reaches_filtered_addedAttributes.tif"
        )
...
def reproject_raster(raster_name, reprojection):
    ...
    with rasterio.open(raster_name) as src:
        ...
        raster_proj_rename = os.path.split(raster_name)[1].replace(
            "_prepprj.tif", ".tif"
        )
...
if __name__ == "__main__":
    ...
    drop_folders = ["logs"]
    huc_list = [
        huc for huc in os.listdir(fim_outputs_directory) if huc not in drop_folders
    ]
    huc6_list = [
        str(huc[0:6])
        for huc in os.listdir(fim_outputs_directory)
        if huc not in drop_folders
    ]
