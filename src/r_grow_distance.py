...
def r_grow_distance(input_raster, grass_workspace, proximity_dtype, allocation_dtype):
    ...
    PERMANENT = Session()
    PERMANENT.open(
        gisdb=grass_gisdb, location=grass_location, create_opts=projected_file
    )
    PERMANENT.close()
    ...
    temporary_session.open(
        ...
        mapset=grass_mapset,
        create_opts=projected_file,
    )

    # Import input raster into temporary session.
    imported_grass_raster = input_raster_name + "@" + grass_mapset
    gscript.run_command(
        "r.in.gdal", input=input_raster, output=imported_grass_raster, quiet=True
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-i", "--in_raster", help="raster to perform r.grow.distance", required=True
    )
    parser.add_argument(
        "-g", "--grass_workspace", help="Temporary GRASS workspace", required=True
    )
    parser.add_argument(
        "-p", "--prox_dtype", help="Output proximity raster datatype", required=True
    )
    parser.add_argument(
        "-a", "--allo_dtype", help="Output allocation raster datatype", required=True
    )
