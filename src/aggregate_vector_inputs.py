...
def identify_nwm_ms_streams(
    nwm_streams_filename, ahps_filename, nwm_streams_all_filename
):
    ...
    nwm_streams.loc[
        nwm_streams.ID.isin(list(ahps_headwaters.nwm_featur)), "is_headwater"
    ] = True
    ...
    nwm_streams = nwm_streams.drop(
        ["is_relevant_stream", "is_headwater"], axis=1, errors="ignore"
    )
...
def find_nwm_incoming_streams(nwm_streams_, wbd, huc_unit):
    ...
    huc_intersection = gpd.GeoDataFrame(
        {
            "geometry": intersecting_points,
            "NHDPlusID": nhdplus_ids,
            "mainstem": mainstem_flag,
        },
        crs=nwm_streams.crs,
        geometry="geometry",
    )
...
def collect_stream_attributes(nhdplus_vectors_dir, huc):
    ...
    vaa_filename = os.path.join(
        nhdplus_vectors_dir, huc, "NHDPlusFlowLineVAA" + str(huc) + ".gpkg"
    )
    flowline_filename = os.path.join(
        nhdplus_vectors_dir, huc, "NHDFlowline" + str(huc) + ".gpkg"
    )
