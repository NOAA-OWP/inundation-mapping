...
def Reset_mannings(hydrofabric_dir,mannings_value,overwrite_files=False):
    ...
    src_table_filePaths, hydro_table_filePaths, stream_network_filePaths = make_file_paths_for_inputs(hydrofabric_dir)
    ...
    for i,(srcFP,hydFP) in enumerate(zip(src_table_filePaths,hydro_table_filePaths)):
        ...
        small_segments = identify_small_reaches(stream_network,min_catchment_area=None,min_stream_length=None)
...
def make_file_paths_for_inputs(hydrofabric_dir):
    ...
    stream_network_filePath_to_glob = os.path.join(hydrofabric_dir,'**','demDerived_reaches_split_filtered_addedAttributes_crosswalked*.gpkg')
...
def identify_small_reaches(stream_network,min_catchment_area=None,min_stream_length=None):
    ...
    for stream_index in stream_network.index:
        ...
        if stream_network["areasqkm"][stream_index] < min_catchment_area and stream_network["LengthKm"][stream_index] < min_stream_length and stream_network["LakeID"][stream_index] < 0:
            ...
            if len(stream_network.loc[stream_network['NextDownID'] == short_id]['HydroID']) > 1:
                max_order = max(stream_network.loc[stream_network['NextDownID'] == short_id]['order_']) # drainage area would be better than stream order but we would need to calculate

                if len(stream_network.loc[(stream_network['NextDownID'] == short_id) & (stream_network['order_'] == max_order)]['HydroID']) == 1:
                    update_id = stream_network.loc[(stream_network['NextDownID'] == short_id) & (stream_network['order_'] == max_order)]['HydroID'].item()

                else:
                    update_id = stream_network.loc[(stream_network['NextDownID'] == short_id) & (stream_network['order_'] == max_order)]['HydroID'].values[0] # get the first one (same stream order, without drainage area info it is hard to know which is the main channel)
            ...
            # no upstream segments; multiple downstream segments
                ...
                max_order = max(stream_network.loc[stream_network.From_Node==to_node]['HydroID']['order_']) # drainage area would be better than stream order but we would need to calculate

                if len(stream_network.loc[(stream_network['NextDownID'] == short_id) & (stream_network['order_'] == max_order)]['HydroID']) == 1:
                    update_id = stream_network.loc[(stream_network.From_Node==to_node) & (stream_network['order_'] == max_order)]['HydroID'].item()

                else:
                    update_id = stream_network.loc[(stream_network.From_Node==to_node) & (stream_network['order_'] == max_order)]['HydroID'].values[0] # get the first one (same stream order, without drainage area info it is hard to know which is the main channel)
...
def replace_discharges_of_small_segments(sml_segs, src_table, hydro_table):
    ...
    for index, segment in sml_segs.iterrows():
        ...
        for src_index, src_stage in new_values.iterrows():
            src_table.loc[(src_table['HydroID']== short_id) & (src_table['Stage']== src_stage[0]),['Discharge (m3s-1)']] = src_stage[1]
