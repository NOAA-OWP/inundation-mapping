...
def update_raster_profile(args):
    ...
    dem_m = np.where(
        data == int(no_data), nodata_val, (data / 100).astype(rasterio.float32)
    )
...
def reproject_raster(
    input_raster_name, reprojection, blocksize=None, reprojected_raster_name=None
):
    ...
    with rasterio.open(input_raster_name) as src:
        ...
        if src.crs.to_string() != reprojection:
            ...
            assert reprojected_raster_name.endswith(
                ".tif"
            ), "output raster needs to be a tif"
