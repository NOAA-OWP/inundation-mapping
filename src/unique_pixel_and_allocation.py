...
def stream_pixel_zones(stream_pixels, unique_stream_pixels, grass_workspace):
    ...
    distance_grid, allocation_grid = r_grow_distance(
        unique_stream_pixels, grass_workspace, "Float32", "Float64"
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-s", "--stream", help="raster to perform r.grow.distance", required=True
    )
    parser.add_argument(
        "-o",
        "--out",
        help="output raster of unique ids for each stream pixel",
        required=True,
    )
    parser.add_argument(
