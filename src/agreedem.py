...
def agreedem(
        ...
        with rasterio.open(smo_output, "w", **smo_profile) as raster:
            for ji, window in elev.block_windows(1):
                ...
                river_data_window = np.where(
                    elev_mask_window == True, river_raw_data_window, 0
                )
    ...
    vectdist_grid, vectallo_grid = r_grow_distance(
        smo_output, grass_workspace, "Float32", "Float32"
    )
    ...
    bufdist_grid, bufallo_grid = r_grow_distance(
        buf_output, grass_workspace, "Float32", "Float32"
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-r", "--rivers", help="flows grid boolean layer", required=True
    )
    ...
    parser.add_argument(
        "-g", "--grass_workspace", help="Temporary GRASS workspace", required=True
    )
    parser.add_argument("-o", "--output", help="Path to output raster", required=True)
    parser.add_argument(
        "-b",
        "--buffer",
        help="Buffer distance (m) on either side of channel",
        required=True,
    )
    ...
    parser.add_argument(
        "-t",
        "--del",
        help="Optional flag to delete intermediate datasets",
        action="store_true",
    )
