...
def add_crosswalk(
    ...
    bathy_src_calc = (
        os.environ["bathy_src_modification"] == "True"
    )  # env variable to toggle on/off the bathy calc and src modifications
    ...
    if extent == "FR":
        ...
        # Create crosswalk
        crosswalk = gpd.sjoin(
            input_flows_midpoint, input_nwmcat, how="left", op="within"
        ).reset_index()
        ...
        for index, stream in crosswalk_missing.iterrows():
            ...
            distances = [
                stream.geometry.distance(poly) for poly in input_nwmcat.geometry
            ]
            ...
            # update crosswalk
            crosswalk.loc[
                crosswalk.HydroID == stream.HydroID, "feature_id"
            ] = input_nwmcat.iloc[nwmcat_index].name
            crosswalk.loc[
                crosswalk.HydroID == stream.HydroID, "AreaSqKM"
            ] = input_nwmcat.iloc[nwmcat_index].AreaSqKM
            crosswalk.loc[
                crosswalk.HydroID == stream.HydroID, "Shape_Length"
            ] = input_nwmcat.iloc[nwmcat_index].Shape_Length
            crosswalk.loc[
                crosswalk.HydroID == stream.HydroID, "Shape_Area"
            ] = input_nwmcat.iloc[nwmcat_index].Shape_Area

        crosswalk = crosswalk.filter(items=["HydroID", "feature_id"])
        crosswalk = crosswalk.merge(
            input_nwmflows[["feature_id", "order_"]], on="feature_id"
        )
    ...
    for stream_index in output_flows.index:
        if (
            ...
            if (
                len(output_flows.loc[output_flows["NextDownID"] == short_id]["HydroID"])
                > 1
            ):
                try:
                    max_order = max(
                        output_flows.loc[output_flows["NextDownID"] == short_id][
                            "order_"
                        ]
            ...
            # single upstream segments
                ...
                update_id = output_flows.loc[output_flows.To_Node == from_node][
                    "HydroID"
                ].item()

            # no upstream segments; multiple downstream segments
            elif (
                len(output_flows.loc[output_flows.From_Node == to_node]["HydroID"]) > 1
            ):
                try:
                    max_order = max(
                        output_flows.loc[output_flows.From_Node == to_node]["HydroID"][
                            "order_"
                        ]
                    )  # drainage area would be better than stream order but we would need to calculate
                except:
            ...
            # no upstream segments; single downstream segment
                ...
                update_id = output_flows.loc[output_flows.From_Node == to_node][
                    "HydroID"
                ].item()

            else:
                update_id = output_flows.loc[output_flows.HydroID == short_id][
                    "HydroID"
                ].item()

            str_order = output_flows.loc[output_flows.HydroID == short_id][
                "order_"
            ].item()
    ...
    if extent == "FR":
        ...
        output_src = output_src.merge(
            crosswalk[["HydroID", "feature_id"]], on="HydroID"
        )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Crosswalk for MS/FR networks; calculate synthetic rating curves; update short rating curves"
    )
    parser.add_argument(
        "-d",
        "--input-catchments-fileName",
        help="DEM derived catchments",
        required=True,
    )
    parser.add_argument(
        "-a", "--input-flows-fileName", help="DEM derived streams", required=True
    )
    parser.add_argument(
        "-s",
        "--input-srcbase-fileName",
        help="Base synthetic rating curve table",
        required=True,
    )
    parser.add_argument(
        "-u",
        "--input-bathy-fileName",
        help="Text file with bankfull bathy variables",
        required=True,
    )
    parser.add_argument(
        "-v",
        "--output-bathy-fileName",
        help="Output bathy/bankfull crosswalk and calculated bathy variables",
        required=True,
    )
    parser.add_argument(
        "-e",
        "--output-bathy-order-fileName",
        help="Output bathy/bankfull stream order avg calculated bathy variables",
        required=True,
    )
    parser.add_argument(
        "-g",
        "--output-bathy-thalweg-fileName",
        help="Output bathy thalweg calculation",
        required=True,
    )
    parser.add_argument(
        "-i",
        "--output-bathy-xs-lookup-fileName",
        help="Output bathy XS Area lookup calculation",
        required=True,
    )
    parser.add_argument(
        "-l",
        "--output-catchments-fileName",
        help="Subset crosswalked catchments",
        required=True,
    )
    parser.add_argument(
        "-f",
        "--output-flows-fileName",
        help="Subset crosswalked streams",
        required=True,
    )
    parser.add_argument(
        "-r",
    ...
    parser.add_argument(
        "-j",
        "--output-src-json-fileName",
        help="Output synthetic rating curve json",
        required=True,
    )
    parser.add_argument(
        "-x", "--output-crosswalk-fileName", help="Crosswalk table", required=True
    )
    parser.add_argument(
        "-t", "--output-hydro-table-fileName", help="Hydrotable", required=True
    )
    parser.add_argument(
        "-w", "--input-huc-fileName", help="HUC8 boundary", required=True
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "-k",
        "--small-segments-filename",
        help="output list of short segments",
        required=True,
    )
    parser.add_argument(
        "-c",
