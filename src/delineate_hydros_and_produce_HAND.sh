...
if [ "$mask_leveed_area_toggle" = "True" ] && [ -f $outputHucDataDir/LeveeProtectedAreas_subset.gpkg ]; then
    ...
    date -u
    Tstart
    python3 -m memory_profiler $srcDir/mask_dem.py -dem $outputCurrentBranchDataDir/dem_meters_$current_branch_id.tif -nld $outputHucDataDir/LeveeProtectedAreas_subset.gpkg -out $outputCurrentBranchDataDir/dem_meters_$current_branch_id.tif -i $current_branch_id -b0 $branch_zero_id -csv $outputHucDataDir/levee_levelpaths.csv
    Tcount
fi
