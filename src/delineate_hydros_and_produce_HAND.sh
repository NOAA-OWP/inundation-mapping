...
date -u
Tstart
python3 -m memory_profiler $srcDir/unique_pixel_and_allocation.py -s $tempCurrentBranchDataDir/demDerived_streamPixels_$current_branch_id.tif -o $tempCurrentBranchDataDir/demDerived_streamPixels_ids_$current_branch_id.tif
Tcount
...
date -u
Tstart
python3 -m memory_profiler $srcDir/adjust_thalweg_lateral.py -e $tempCurrentBranchDataDir/dem_meters_$current_branch_id.tif -s $tempCurrentBranchDataDir/demDerived_streamPixels_$current_branch_id.tif -a $tempCurrentBranchDataDir/demDerived_streamPixels_ids_"$current_branch_id"_allo.tif -d $tempCurrentBranchDataDir/demDerived_streamPixels_ids_"$current_branch_id"_dist.tif -t 50 -o $tempCurrentBranchDataDir/dem_lateral_thalweg_adj_$current_branch_id.tif -th $thalweg_lateral_elev_threshold
Tcount
...
date -u
Tstart
$srcDir/reachID_grid_to_vector_points.py -r $tempCurrentBranchDataDir/demDerived_streamPixels_$current_branch_id.tif -i featureID -p $tempCurrentBranchDataDir/flows_points_pixels_$current_branch_id.gpkg
Tcount
...
date -u
Tstart
$srcDir/make_rem.py -d $tempCurrentBranchDataDir/dem_thalwegCond_"$current_branch_id".tif -w $tempCurrentBranchDataDir/gw_catchments_pixels_$current_branch_id.tif -o $tempCurrentBranchDataDir/rem_$current_branch_id.tif -t $tempCurrentBranchDataDir/demDerived_streamPixels_$current_branch_id.tif
Tcount
...
if [ -f $tempHucDataDir/LandSea_subset.gpkg ]; then
    ...
    date -u
    Tstart
    gdal_rasterize -ot Int32 -burn $ndv -a_nodata $ndv -init 1 -co "COMPRESS=LZW" -co "BIGTIFF=YES" -co "TILED=YES" -te $xmin $ymin $xmax $ymax -ts $ncols $nrows $tempHucDataDir/LandSea_subset.gpkg $tempCurrentBranchDataDir/LandSea_subset_$current_branch_id.tif
    Tcount
fi
...
date -u
Tstart
gdal_polygonize.py -8 -f GPKG $tempCurrentBranchDataDir/gw_catchments_reaches_$current_branch_id.tif $tempCurrentBranchDataDir/gw_catchments_reaches_$current_branch_id.gpkg catchments HydroID
Tcount
...
date -u
Tstart
python3 -m memory_profiler $srcDir/filter_catchments_and_add_attributes.py -i $tempCurrentBranchDataDir/gw_catchments_reaches_$current_branch_id.gpkg -f $tempCurrentBranchDataDir/demDerived_reaches_split_$current_branch_id.gpkg -c $tempCurrentBranchDataDir/gw_catchments_reaches_filtered_addedAttributes_$current_branch_id.gpkg -o $tempCurrentBranchDataDir/demDerived_reaches_split_filtered_$current_branch_id.gpkg -w $tempHucDataDir/wbd8_clp.gpkg -u $hucNumber
Tcount
...
date -u
Tstart
$srcDir/make_stages_and_catchlist.py -f $tempCurrentBranchDataDir/demDerived_reaches_split_filtered_$current_branch_id.gpkg -c $tempCurrentBranchDataDir/gw_catchments_reaches_filtered_addedAttributes_$current_branch_id.gpkg -s $tempCurrentBranchDataDir/stage_$current_branch_id.txt -a $tempCurrentBranchDataDir/catch_list_$current_branch_id.txt -m $stage_min_meters -i $stage_interval_meters -t $stage_max_meters
Tcount
...
date -u
Tstart
$taudemDir/catchhydrogeo -hand $tempCurrentBranchDataDir/rem_zeroed_masked_$current_branch_id.tif -catch $tempCurrentBranchDataDir/gw_catchments_reaches_filtered_addedAttributes_$current_branch_id.tif -catchlist $tempCurrentBranchDataDir/catch_list_$current_branch_id.txt -slp $tempCurrentBranchDataDir/slopes_d8_dem_meters_masked_$current_branch_id.tif -h $tempCurrentBranchDataDir/stage_$current_branch_id.txt -table $tempCurrentBranchDataDir/src_base_$current_branch_id.csv
Tcount
