...
if [ "$mask_leveed_area_toggle" = "True" ] && [ -f $tempHucDataDir/LeveeProtectedAreas_subset.gpkg ]; then
    ...
    date -u
    Tstart
    python3 -m memory_profiler $srcDir/mask_dem.py -dem $tempCurrentBranchDataDir/dem_meters_$current_branch_id.tif \
        -nld $tempHucDataDir/LeveeProtectedAreas_subset.gpkg -out $tempCurrentBranchDataDir/dem_meters_$current_branch_id.tif \
        -s $tempHucDataDir/nwm_subset_streams_levelPaths.gpkg -i $current_branch_id -b0 $branch_zero_id
    Tcount
fi
...
$taudemDir/aread8 -p $tempCurrentBranchDataDir/flowdir_d8_burned_filled_$current_branch_id.tif \
    ...
    -wg $tempCurrentBranchDataDir/headwaters_$current_branch_id.tif -nc
Tcount
...
$taudemDir/threshold -ssa $tempCurrentBranchDataDir/flowaccum_d8_burned_filled_$current_branch_id.tif \
    -src $tempCurrentBranchDataDir/demDerived_streamPixels_$current_branch_id.tif -thresh 1
Tcount
...
date -u
Tstart
python3 -m memory_profiler $srcDir/unique_pixel_and_allocation.py -s $tempCurrentBranchDataDir/demDerived_streamPixels_$current_branch_id.tif \
...
date -u
Tstart
python3 -m memory_profiler $srcDir/adjust_thalweg_lateral.py -e $tempCurrentBranchDataDir/dem_meters_$current_branch_id.tif \
    ...
    -d $tempCurrentBranchDataDir/demDerived_streamPixels_ids_"$current_branch_id"_dist.tif -t 50 \
    -o $tempCurrentBranchDataDir/dem_lateral_thalweg_adj_$current_branch_id.tif -th $thalweg_lateral_elev_threshold
Tcount
...
gdal_calc.py --quiet --type=Int32 --overwrite --co "COMPRESS=LZW" --co "BIGTIFF=YES" --co "TILED=YES" \
    ...
    -B $tempCurrentBranchDataDir/demDerived_streamPixels_$current_branch_id.tif --calc="A/B" \
    --outfile="$tempCurrentBranchDataDir/flowdir_d8_burned_filled_flows_$current_branch_id.tif" --NoDataValue=0
Tcount
...
date -u
Tstart
mpiexec -n $ncores_fd $taudemDir2/d8flowdir -fel $tempCurrentBranchDataDir/dem_lateral_thalweg_adj_$current_branch_id.tif \
...
date -u
Tstart
$taudemDir/streamnet -p $tempCurrentBranchDataDir/flowdir_d8_burned_filled_$current_branch_id.tif \
    ...
    -ord $tempCurrentBranchDataDir/streamOrder_$current_branch_id.tif -tree $tempCurrentBranchDataDir/treeFile_$current_branch_id.txt \
    -coord $tempCurrentBranchDataDir/coordFile_$current_branch_id.txt -w $tempCurrentBranchDataDir/sn_catchments_reaches_$current_branch_id.tif \
...
$srcDir/split_flows.py -f $tempCurrentBranchDataDir/demDerived_reaches_$current_branch_id.shp \
    -d $tempCurrentBranchDataDir/dem_thalwegCond_$current_branch_id.tif -s $tempCurrentBranchDataDir/demDerived_reaches_split_$current_branch_id.gpkg \
    -p $tempCurrentBranchDataDir/demDerived_reaches_split_points_$current_branch_id.gpkg -w $tempHucDataDir/wbd8_clp.gpkg \
    -l $tempHucDataDir/nwm_lakes_proj_subset.gpkg -n $tempCurrentBranchDataDir/nwm_subset_streams_levelPaths_$current_branch_id.gpkg
Tcount
...
date -u
Tstart
mpiexec -n $ncores_gw $taudemDir/gagewatershed -p $tempCurrentBranchDataDir/flowdir_d8_burned_filled_$current_branch_id.tif \
    -gw $tempCurrentBranchDataDir/gw_catchments_reaches_$current_branch_id.tif \
    -o $tempCurrentBranchDataDir/demDerived_reaches_split_points_$current_branch_id.gpkg -id $tempCurrentBranchDataDir/idFile_$current_branch_id.txt
Tcount
...
date -u
Tstart
$srcDir/reachID_grid_to_vector_points.py -r $tempCurrentBranchDataDir/demDerived_streamPixels_$current_branch_id.tif \
    -i featureID -p $tempCurrentBranchDataDir/flows_points_pixels_$current_branch_id.gpkg
Tcount
...
Tstart
mpiexec -n $ncores_gw $taudemDir/gagewatershed -p $tempCurrentBranchDataDir/flowdir_d8_burned_filled_"$current_branch_id".tif \
    -gw $tempCurrentBranchDataDir/gw_catchments_pixels_$current_branch_id.tif -o $tempCurrentBranchDataDir/flows_points_pixels_$current_branch_id.gpkg \
...
$srcDir/make_rem.py -d $tempCurrentBranchDataDir/dem_thalwegCond_"$current_branch_id".tif \
    -w $tempCurrentBranchDataDir/gw_catchments_pixels_$current_branch_id.tif -o $tempCurrentBranchDataDir/rem_$current_branch_id.tif \
...
gdal_calc.py --quiet --type=Float32 --overwrite --co "COMPRESS=LZW" --co "BIGTIFF=YES" --co "TILED=YES" \
    -A $tempCurrentBranchDataDir/rem_$current_branch_id.tif -B $tempCurrentBranchDataDir/gw_catchments_reaches_$current_branch_id.tif \
    --calc="(A*(A>=0)*(B>0))" --NoDataValue=$ndv --outfile=$tempCurrentBranchDataDir/"rem_zeroed_masked_$current_branch_id.tif"
Tcount
...
if [ -f $tempHucDataDir/LandSea_subset.gpkg ]; then
    ...
    date -u
    Tstart
    gdal_rasterize -ot Int32 -burn $ndv -a_nodata $ndv -init 1 -co "COMPRESS=LZW" -co "BIGTIFF=YES" -co "TILED=YES" -te $xmin $ymin $xmax $ymax \
        -ts $ncols $nrows $tempHucDataDir/LandSea_subset.gpkg $tempCurrentBranchDataDir/LandSea_subset_$current_branch_id.tif
    Tcount
fi
...
date -u
Tstart
python3 -m memory_profiler $srcDir/filter_catchments_and_add_attributes.py -i $tempCurrentBranchDataDir/gw_catchments_reaches_$current_branch_id.gpkg \
    ...
    -o $tempCurrentBranchDataDir/demDerived_reaches_split_filtered_$current_branch_id.gpkg -w $tempHucDataDir/wbd8_clp.gpkg \
    -u $hucNumber
Tcount
...
date -u
Tstart
gdal_rasterize -ot Int32 -a HydroID -a_nodata 0 -init 0 -co "COMPRESS=LZW" -co "BIGTIFF=YES" -co "TILED=YES" -te $xmin $ymin $xmax $ymax \
    -ts $ncols $nrows $tempCurrentBranchDataDir/gw_catchments_reaches_filtered_addedAttributes_$current_branch_id.gpkg \
...
gdal_calc.py --quiet --type=Float32 --overwrite --co "COMPRESS=LZW" --co "BIGTIFF=YES" --co "TILED=YES" \
    ...
    -B $tempCurrentBranchDataDir/gw_catchments_reaches_filtered_addedAttributes_$current_branch_id.tif --calc="A*(B>0)" \
    --NoDataValue=$ndv --outfile=$tempCurrentBranchDataDir/slopes_d8_dem_meters_masked_$current_branch_id.tif
Tcount
...
date -u
Tstart
$srcDir/make_stages_and_catchlist.py -f $tempCurrentBranchDataDir/demDerived_reaches_split_filtered_$current_branch_id.gpkg \
    -c $tempCurrentBranchDataDir/gw_catchments_reaches_filtered_addedAttributes_$current_branch_id.gpkg \
    -s $tempCurrentBranchDataDir/stage_$current_branch_id.txt -a $tempCurrentBranchDataDir/catch_list_$current_branch_id.txt \
    -m $stage_min_meters -i $stage_interval_meters -t $stage_max_meters
Tcount
...
if  [ -f $tempCurrentBranchDataDir/LandSea_subset_$current_branch_id.tif ]; then
    ...
    Tstart
    gdal_calc.py --quiet --type=Float32 --overwrite --co "COMPRESS=LZW" --co "BIGTIFF=YES" --co "TILED=YES" \
        -A $tempCurrentBranchDataDir/rem_zeroed_masked_$current_branch_id.tif -B $tempCurrentBranchDataDir/LandSea_subset_$current_branch_id.tif \
        --calc="(A*B)" --NoDataValue=$ndv --outfile=$tempCurrentBranchDataDir/"rem_zeroed_masked_$current_branch_id.tif"
    Tcount
fi
...
$taudemDir/catchhydrogeo -hand $tempCurrentBranchDataDir/rem_zeroed_masked_$current_branch_id.tif \
    ...
    -slp $tempCurrentBranchDataDir/slopes_d8_dem_meters_masked_$current_branch_id.tif -h $tempCurrentBranchDataDir/stage_$current_branch_id.txt \
...
python3 -m memory_profiler $srcDir/add_crosswalk.py \
    ...
    -w $tempHucDataDir/wbd8_clp.gpkg -b $b_arg \
    -y $tempCurrentBranchDataDir/nwm_catchments_proj_subset.tif -m $manning_n -z $z_arg \
