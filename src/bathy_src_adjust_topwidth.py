...
def bathy_rc_lookup(args):
    ...
    if modified_src_base['BANKFULL_WIDTH (m)'].count() == 0:
        ...
        modified_hydro_table = modified_src_base.loc[:,['HydroID','Stage','barc_on','Volume (m3)','WetArea (m2)','HydraulicRadius (m)','Discharge (m3s-1)']]
        modified_hydro_table = modified_hydro_table.rename(columns={'Stage' : 'stage','Discharge (m3s-1)':'discharge_cms'})
        df_htable = pd.read_csv(input_htable_fileName,dtype={'HUC': str})
        df_htable = df_htable.drop(['barc_on'], axis=1) # drop the default "barc_on" variable from add_crosswalk.py
        if not set(['orig_discharge_cms','orig_Volume (m3)','orig_WetArea (m2)','orig_HydraulicRadius (m)']).issubset(df_htable.columns): # check if "orig_" attributes do NOT already exist (likely generated from previous BARC run)
            df_htable = df_htable.rename(columns={'discharge_cms':'orig_discharge_cms','Volume (m3)':'orig_Volume (m3)','WetArea (m2)':'orig_WetArea (m2)','HydraulicRadius (m)':'orig_HydraulicRadius (m)'})
        else:
            df_htable = df_htable.drop(['discharge_cms','Volume (m3)','WetArea (m2)','HydraulicRadius (m)'], axis=1) # drop the previously modified columns - to be replaced with updated version
