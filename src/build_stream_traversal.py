        ...
        try:
            ...
            if not hydro_id in streams.columns:
                print(
                    "Required field "
                    + hydro_id
                    + " does not exist in input. Generating.."
                )
                ...
                streams["seqID"] = (
                    (streams.groupby("HUC8id").cumcount(ascending=True) + 1)
                    .astype("str")
                    .str.zfill(4)
                )
                ...
                streams = streams.rename(columns={"hydro_id": hydro_id}).sort_values(
                    hydro_id
                )
            ...
            for urow in streams[
                [next_down_id, to_node, from_node, hydro_id]
            ].iterrows():
                tonodecol = urow[1][1]
                ...
                try:
                    ...
                    pass
                streams.loc[
                    streams[hydro_id] == hydroIDcol, [next_down_id]
                ] = nextdownIDcol
