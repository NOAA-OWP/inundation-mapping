...
def burn_in_levees(dem_filename, nld_filename, out_dem_filename):
    ...
    nld_m = np.where(
        nld_data == int(no_data), -9999.0, (nld_data * 0.3048).astype(rasterio.float32)
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-dem", "--dem-filename", help="DEM filename", required=True, type=str
    )
    parser.add_argument(
        "-nld", "--nld-filename", help="NLD filename", required=True, type=str
    )
    parser.add_argument(
