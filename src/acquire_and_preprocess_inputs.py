...
from utils.shared_functions import delete_file, getDriver, pull_file, run_system_command
...
def subset_wbd_to_nwm_domain(wbd, nwm_file_to_use):
    ...
    intersecting_indices = [
        not (gpd.read_file(nwm_file_to_use, mask=b).empty) for b in wbd.geometry
    ]
...
def pull_and_prepare_wbd(
    ...
    nwm_file_to_use = os.path.join(
        path_to_saved_data_parent_dir, nwm_dir_name, nwm_file_to_use
    )
    ...
    if not os.path.exists(multilayer_wbd_geopackage) or overwrite_wbd:
        ...
        wbd_hu8.to_file(
            multilayer_wbd_geopackage,
            layer="WBDHU8",
            driver=getDriver(multilayer_wbd_geopackage),
            index=False,
        )  # Save.
        wbd_hu8.HUC8.to_csv(
            nwm_huc_list_file_template.format("8"), index=False, header=False
        )
...
def pull_and_prepare_nwm_hydrofabric(
    path_to_saved_data_parent_dir, path_to_preinputs_dir, num_workers
):
    """
    ...
    nwm_hydrofabric_directory = os.path.join(
        path_to_saved_data_parent_dir, "nwm_hydrofabric"
    )
    ...
    for nwm_layer in [
        ...
        "nwm_lakes",
        "nwm_catchments",
    ]:  # I had to project the catchments and waterbodies because these 3 layers had varying CRSs.
        print("Operating on " + nwm_layer)
        output_gpkg = os.path.join(nwm_hydrofabric_directory, nwm_layer + "_proj.gpkg")
        procs_list.append(
            [
                'ogr2ogr -overwrite -progress -f GPKG -t_srs "{projection}" {output_gpkg} {nwm_hydrofabric_gdb} {nwm_layer}'.format(
                    projection=PREP_PROJECTION,
                    output_gpkg=output_gpkg,
                    nwm_hydrofabric_gdb=nwm_hydrofabric_gdb,
                    nwm_layer=nwm_layer,
                )
            ]
        )
...
def pull_and_prepare_nhd_data(args):
    ...
    nhd_raster_parent_dir = os.path.join(
        nhd_raster_extraction_parent, "HRNHDPlusRasters" + huc
    )
    ...
    if not os.path.exists(elev_cm_tif) or overwrite_nhd:
        ...
        os.system(
            "7za e {nhd_raster_extraction_path} -o{nhd_raster_parent_dir} elev_cm.tif -r ".format(
    ...
    if (
        not os.path.exists(nhd_gdb) or overwrite_nhd
    ):  # Only pull if not already pulled and processed.
        ...
        os.system(
            "7za x {nhd_vector_extraction_path} -o{nhd_vector_extraction_parent}".format(
                nhd_vector_extraction_path=nhd_vector_extraction_path,
        ...
        nhd.to_file(
            os.path.join(
                nhd_vector_extraction_parent, "NHDPlusBurnLineEvent" + huc + ".gpkg"
            ),
            driver="GPKG",
        )
        ...
        nhd = nhd.to_crs(PREP_PROJECTION)
        nhd.to_file(
            os.path.join(nhd_vector_extraction_parent, "NHDFlowline" + huc + ".gpkg"),
            driver="GPKG",
        )
        # extract attributes
        ...
        nhd.to_file(
            os.path.join(
                nhd_vector_extraction_parent, "NHDPlusFlowLineVAA" + huc + ".gpkg"
            ),
            driver="GPKG",
        )
...
def build_huc_list_files(path_to_saved_data_parent_dir, wbd_directory):
    ...
    nhd_plus_raster_dir = os.path.join(
        path_to_saved_data_parent_dir, NHDPLUS_RASTERS_DIRNAME
    )
    nhd_plus_vector_dir = os.path.join(
        path_to_saved_data_parent_dir, NHDPLUS_VECTORS_DIRNAME
    )
    ...
    for index, row in tqdm(wbd.iterrows(), total=len(wbd)):
        ...
        burnline = os.path.join(
            nhd_plus_vector_dir, huc[0:4], "NHDPlusBurnLineEvent" + huc[0:4] + ".gpkg"
        )
        if os.path.exists(burnline):
            nhd_test = len(
                gpd.read_file(burnline, mask=huc_mask)
            )  # this is slow, iterates through 2000+ HUC8s
...
def manage_preprocessing(
    hucs_of_interest, num_workers=1, overwrite_nhd=False, overwrite_wbd=False
):
    """
    ...
    if isinstance(hucs_of_interest, list):
        if len(hucs_of_interest) == 1:
            try:
                with open(
                    hucs_of_interest[0]
                ) as csv_file:  # Does not have to be CSV format.
    ...
    for huc in huc_list:
        ...
        nhd_raster_download_url = os.path.join(
            NHD_URL_PARENT, NHD_URL_PREFIX + huc + NHD_RASTER_URL_SUFFIX
        )
        nhd_raster_extraction_path = os.path.join(
            nhd_raster_dir, NHD_URL_PREFIX + huc + NHD_RASTER_URL_SUFFIX
        )

        # Construct URL and extraction path for NHDPlus vector. Organize into huc-level subdirectories.
        nhd_vector_download_url = os.path.join(
            NHD_URL_PARENT, NHD_URL_PREFIX + huc + NHD_VECTOR_URL_SUFFIX
        )
        ...
        nhd_vector_extraction_path = os.path.join(
            nhd_vector_download_parent,
            NHD_VECTOR_EXTRACTION_PREFIX + huc + NHD_VECTOR_EXTRACTION_SUFFIX,
        )

        # Append extraction instructions to nhd_procs_list.
        nhd_procs_list.append(
            [
                nhd_raster_download_url,
    ...
    wbd_directory = pull_and_prepare_wbd(
        path_to_saved_data_parent_dir,
        NWM_HYDROFABRIC_DIRNAME,
        NWM_FILE_TO_SUBSET_WITH,
        overwrite_wbd,
        num_workers,
    )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Acquires and preprocesses WBD and NHD data for use in fim_run.sh."
    )
    parser.add_argument(
        "-u",
        "--hucs-of-interest",
        help="HUC4, series of HUC4s, or path to a line-delimited file of HUC4s to acquire.",
        required=True,
        nargs="+",
    )
    ...
    parser.add_argument(
        "-n",
        "--overwrite-nhd",
        help="Optional flag to overwrite NHDPlus Data",
        required=False,
        action="store_true",
    )
    parser.add_argument(
        "-w",
        "--overwrite-wbd",
        help="Optional flag to overwrite WBD Data",
        required=False,
        action="store_true",
    )
