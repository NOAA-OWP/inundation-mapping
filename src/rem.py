...
def rel_dem(
    ...
    @njit
    def make_catchment_hydroid_dict(
        flat_value_raster, catchment_hydroid_dict, flat_catchments, thalweg_window
    ):

        for i, cm in enumerate(flat_catchments):
            if (
                thalweg_window[i] == 1
            ):  # Only allow reference hydroid to be within thalweg.
    ...
    for ji, window in hydroid_pixels_object.block_windows(
        ...
        catchment_hydroid_dict = make_catchment_hydroid_dict(
            hydroid_window, catchment_hydroid_dict, catchments_window, thalweg_window
        )
    ...
    @njit
    def make_catchment_min_dict(
        flat_dem, catchment_min_dict, flat_catchments, thalweg_window
    ):

        for i, cm in enumerate(flat_catchments):
            if (
                thalweg_window[i] == 1
            ):  # Only allow reference elevation to be within thalweg
    ...
    catchment_min_dict_df = pd.DataFrame.from_dict(
        catchment_min_dict, orient="index"
    )  # convert dict to dataframe
    catchment_min_dict_df.columns = ["Median_Thal_Elev_m"]
    catchment_hydroid_dict_df = pd.DataFrame.from_dict(
        catchment_hydroid_dict, orient="index"
    )  # convert dict to dataframe
    catchment_hydroid_dict_df.columns = ["HydroID"]
    merge_df = catchment_hydroid_dict_df.merge(
        catchment_min_dict_df, left_index=True, right_index=True
    )
    ...
    min_by_hydroid = merge_df.groupby(
        ["HydroID"]
    ).min()  # min value of all med_thal_elev for pixel catchments in each HydroID reach
    min_by_hydroid.columns = ["min_thal_elev"]
    med_by_hydroid = merge_df.groupby(
        ["HydroID"]
    ).median()  # median value of all med_thal_elev for pixel catchments in each HydroID reach
    med_by_hydroid.columns = ["med_thal_elev"]
    max_by_hydroid = merge_df.groupby(
        ["HydroID"]
    ).max()  # max value of all med_thal_elev for pixel catchments in each HydroID reach
    max_by_hydroid.columns = ["max_thal_elev"]
    input_reaches = gpd.read_file(dem_reaches_filename)
    input_reaches = input_reaches.merge(
        min_by_hydroid, on="HydroID"
    )  # merge dataframes by HydroID variable
    input_reaches = input_reaches.merge(
        med_by_hydroid, on="HydroID"
    )  # merge dataframes by HydroID variable
    input_reaches = input_reaches.merge(
        max_by_hydroid, on="HydroID"
    )  # merge dataframes by HydroID variable
    input_reaches.to_file(
        dem_reaches_filename, driver=getDriver(dem_reaches_filename), index=False
    )
    # ------------------------------------------------------------------------------------------------------------------------ #
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Relative elevation from pixel based watersheds"
    )
    parser.add_argument(
        "-d", "--dem", help="DEM to use within project path", required=True
    )
    parser.add_argument(
        "-w",
        "--watersheds",
    ...
    parser.add_argument(
        "-i",
        "--hydroid",
        help="HydroID raster to use within project path",
        required=True,
    )
    parser.add_argument(
        "-s",
        "--dem_reaches_in_out",
        help="DEM derived reach layer to join HAND reference elevation attribute",
        required=True,
    )
