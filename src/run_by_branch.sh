...
# ogr2ogr -f GPKG -t_srs $DEFAULT_FIM_PROJECTION_CRS -where $branch_id_attribute="$current_branch_id" \
#    $tempCurrentBranchDataDir/nwm_headwaters_$current_branch_id.gpkg $tempHucDataDir/nwm_headwaters.gpkg
Tcount
...
$srcDir/clip_rasters_to_branches.py -d $current_branch_id -b $tempHucDataDir/branch_polygons.gpkg \
    -i $branch_id_attribute -r $tempBranchDataDir/$branch_zero_id/dem_meters_$branch_zero_id.tif \
    $tempBranchDataDir/$branch_zero_id/flowdir_d8_burned_filled_$branch_zero_id.tif \
    -c $tempCurrentBranchDataDir/dem_meters.tif $tempCurrentBranchDataDir/flowdir_d8_burned_filled.tif -v
Tcount
...
date -u
Tstart
read fsize ncols nrows ndv xmin ymin xmax ymax cellsize_resx \
    cellsize_resy<<<$($srcDir/getRasterInfoNative.py $tempCurrentBranchDataDir/dem_meters_$current_branch_id.tif)
Tcount
...
Tstart
gdal_rasterize -ot Int32 -burn 1 -init 0 -co "COMPRESS=LZW" -co "BIGTIFF=YES" -co "TILED=YES" -te $xmin $ymin $xmax $ymax \
    -ts $ncols $nrows $tempCurrentBranchDataDir/nwm_subset_streams_levelPaths_dissolved_headwaters_$current_branch_id.gpkg \
...
if [ -f $tempHucDataDir/usgs_subset_gages.gpkg ]; then
    ...
    python3 $srcDir/usgs_gage_crosswalk.py -gages $tempHucDataDir/usgs_subset_gages.gpkg \
        ...
        -dem $tempCurrentBranchDataDir/dem_meters_$current_branch_id.tif -dem_adj $tempCurrentBranchDataDir/dem_thalwegCond_$current_branch_id.tif \
        -outtable $tempCurrentBranchDataDir/usgs_elev_table.csv -b $current_branch_id
    Tcount
fi
