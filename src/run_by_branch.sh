...
ogr2ogr -f GPKG -t_srs $huc_CRS -where $branch_id_attribute="$current_branch_id" \
    $tempCurrentBranchDataDir/nwm_subset_streams_levelPaths_extended_dissolved_$current_branch_id.gpkg \
    $tempHucDataDir/nwm_subset_streams_levelPaths_extended_dissolved.gpkg
...
gdal_rasterize -q -ot Int32 -burn 1 -init 0 -co "COMPRESS=LZW" -co "BIGTIFF=YES" -co "TILED=YES" \
    ...
    -ts $ncols $nrows $tempCurrentBranchDataDir/nwm_subset_streams_levelPaths_extended_dissolved_$current_branch_id.gpkg \
