...
def find_unique_hucs(inputsDir, hucLength):
    ...
    for vaa_file in tqdm(huc4_vaa_list):
        ...
        unique_hucs = np.append(
            unique_hucs, reachCodes.apply(lambda x: x[0:hucLength]).unique()
        )

    unique_hucs = pd.Series(unique_hucs)
    unique_hucs.to_csv(
        join(inputsDir, "included_huc{}.lst".format(hucLength)),
        header=False,
        index=False,
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-i",
        "--inputs-directory",
        help="Basins polygons to use within project path",
        required=True,
    )
    parser.add_argument(
        "-l",
        "--huc-length",
        help="Basins polygons to use within proiject path",
        required=True,
        type=int,
    )
