...
def subset_nhd_network(
    ...
    for index, row in selected_wbd8.iterrows():
        ...
        if huc.startswith(str(huc4)):
            ...
            if mainstem_flag == False:
                ...
                if headwaters_mask.is_headwater.dtype != "int":
                    headwaters_mask.is_headwater = headwaters_mask.is_headwater.astype(
                        "int"
                    )
                if headwaters_mask.is_colocated.dtype != "int":
                    headwaters_mask.is_colocated = headwaters_mask.is_colocated.astype(
                        "int"
                    )
                headwaters_mask = headwaters_mask.loc[
                    headwaters_mask.is_headwater == True
                ]
            ...
            if not streams_subset.empty:
                ...
                for index, point in headwaters_mask.iterrows():
                    ...
                    distances = pygeos.measurement.distance(
                    ...
                    if mainstem_flag == True:
                        ...
                        if point.is_colocated == True:
                            ...
                            distance_to_line = point.geometry.distance(
                                Point(closest_stream.geometry.coords[-1])
                            )
                            print(
                                f"{point.nws_lid} distance on line {closest_stream.NHDPlusID}:  {np.round(distance_to_line,1)}"
                            )

                            if (
                                not closest_stream.NHDPlusID
                                in distance_from_upstream.keys()
                            ):
                                distance_from_upstream[closest_stream.NHDPlusID] = [
                                    point.nws_lid,
                                    distance_to_line,
                                ]

                            elif (
                                distance_from_upstream[closest_stream.NHDPlusID][1]
                                > distance_to_line
                            ):
                                distance_from_upstream[closest_stream.NHDPlusID] = [
                                    point.nws_lid,
                            ...
                            headwater_point_name = distance_from_upstream[
                                closest_stream.NHDPlusID
                            ][0]
                ...
                headwater_streams = headwater_streams.append(
                    streams_subset[["NHDPlusID", headwater_col, id_col, "HUC8"]]
                )

    headwater_streams = headwater_streams.sort_values(
        headwater_col, ascending=False
    ).drop_duplicates(
        "NHDPlusID"
    )  # keeps headwater=True for conflicting duplicates
    ...
    nhd_streams = get_downstream_segments(
        nhd_streams.copy(), headwater_col, mainstem_flag
    )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Reduce NHDPlus HR network based on headwater points"
    )
    parser.add_argument(
        "-n", "--huc-number", help="HUC number", required=True, type=str
    )
    ...
    parser.add_argument(
        "-t", "--nhd-streams", help="NHDPlus HR geodataframe", required=True
    )
    parser.add_argument(
        "-a",
        "--headwaters-filename",
    ...
    parser.add_argument(
        "-i", "--headwater-id", help="Headwater points ID column", required=True
    )
    parser.add_argument(
        "-c",
    ...
    subset_streams_gdf = subset_nhd_network(
        huc_number,
        huc4_mask,
        selected_wbd8,
        nhd_streams,
