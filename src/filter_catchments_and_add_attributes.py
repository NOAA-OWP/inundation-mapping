...
def filter_catchments_and_add_attributes(
    ...
    select_flows = tuple(
        map(str, map(int, wbd[wbd.HUC8.str.contains(huc_code)][FIM_ID]))
    )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="filter_catchments_and_add_attributes.py"
    )
    parser.add_argument(
        "-i",
        "--input-catchments-filename",
        help="input-catchments-filename",
        required=True,
    )
    parser.add_argument(
        "-f", "--input-flows-filename", help="input-flows-filename", required=True
    )
    parser.add_argument(
        "-c",
        "--output-catchments-filename",
        help="output-catchments-filename",
        required=True,
    )
    parser.add_argument(
