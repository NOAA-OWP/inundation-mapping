...
def Subset_catch_list(
    ...
    stream_network = StreamNetwork.from_file_fim(stream_network, branch_id_attribute=branch_id_attribute)
    ...
    catch_list = catch_list.merge(
        stream_network.loc[:, ["HydroID", branch_id_attribute]], on='HydroID', how='inner'
    )
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '-s', '--stream-network', help='Stream Network with HydroIDs and Branch IDs', required=True
    )
    parser.add_argument(
