...
def process_roads_fimpact_in_huc(
    hand_grid_raster: str,
    splitted_road_vector: str,
    output_path: str,
) -> gpd.GeoDataFrame:
    """
    ...
    - splitted_road_vector (str): REQUIRED. Path to a GeoPackage (GPKG) file containing the splitted road centerline vectors.
    ...
    #read roads data
    roads_gdf = gpd.read_file(splitted_road_vector)
...
if __name__ == "__main__":
    ...
    Sample usage :
        ...
        -r outputs/roads/results/roads_02050206_splitted.gpkg
    ...
    parser.add_argument(
        '-g',
        '--hand_grid_raster',
        help='REQUIRED: Path for HAND grid raster file',
        required=True,
    )

    
    parser.add_argument(
        '-r', '--splitted_road_vector', 
        help='REQUIRED: Path to a GPKG file containing the splitted (on nwm_catchments) osm roads centerline ', 
        required=True
    )
    ...
    parser.add_argument(
        '-o',
        '--output_path',
        help='REQUIRED: Path where the output GPKG file will be saved',
        required=True,
    )

    args = vars(parser.parse_args())

    process_roads_fimpact_in_huc(**args)
