...
def rasterize_parquet(
    ...
    if attribute:
        if attribute not in gdf.columns:
            print(
                f"Error: Attribute column '{attribute}' not found in the input parquet file.", file=sys.stderr
            )
            sys.exit(1)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-ot", default="Int32", help="Output data type (e.g., Int32, Float32, Byte). Default: Int32"
    )
    parser.add_argument(
        "-burn",
        type=float,
        default=1.0,
        help="Fixed value to burn into the raster if -a is not used. Default: 1",
    )
    parser.add_argument(
        "-a",
        dest="attribute",
        default=None,
        help="Name of the attribute column in the parquet file to burn into the raster grid",
    )
    parser.add_argument(
        "-init", type=float, default=0.0, help="Value to initialize the raster band with. Default: 0"
    )
    parser.add_argument(
        "-a_nodata", type=float, default=-9999.0, help="Assign a specified nodata value to output bands"
    )
    ...
    parser.add_argument(
        "-co",
        action="append",
        help="Creation options for output format (e.g., -co BIGTIFF=YES). Can be specified multiple times.",
    )
    ...
    parser.add_argument(
        "-ts", nargs=2, type=int, required=True, metavar=('NCOLS', 'NROWS'), help="Target size: ncols nrows"
    )
