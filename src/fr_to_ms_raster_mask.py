...
def fr_to_ms_raster_mask(
    ...
    print(
        "Limiting rasters to buffer area ({} meters) around model streams".format(
            str(ms_buffer_dist)
        )
    )
    ...
    with rasterio.open(dem_fr) as src:
        out_image, out_transform = rasterio.mask.mask(
            src, [split_flows_ms_buffer], crop=True
        )
        out_meta = src.meta
    ...
    with rasterio.open(
        os.path.join(os.path.dirname(dem_fr), dem_ms_filename), "w", **out_meta
    ) as dest:
    ...
    with rasterio.open(
        os.path.join(os.path.dirname(fdr_fr), fdr_ms_filename), "w", **out_meta
    ) as dest:
        dest.write(out_image)
    ...
    with rasterio.open(
        os.path.join(os.path.dirname(slope_fr), slope_ms_filename), "w", **out_meta
    ) as dest:
        dest.write(out_image)
    ...
    with rasterio.open(
        os.path.join(os.path.dirname(str_pixel_fr), str_pixel_ms_filename),
        "w",
        **out_meta
    ) as dest:
        dest.write(out_image)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-m", "--fdr-ms-filename", help="fdr-ms-filename", required=True
    )
    parser.add_argument(
        "-n", "--dem-ms-filename", help="dem-ms-filename", required=True
    )
    parser.add_argument(
        "-o", "--slope-ms-filename", help="slope-ms-filename", required=True
    )
    parser.add_argument("-p", "--str-pixel-fr", help="str-pixel-fr", required=True)
    parser.add_argument(
        "-q", "--str-pixel-ms-filename", help="str-pixel-ms-filename", required=True
    )
