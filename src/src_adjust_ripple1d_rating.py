...
Inputs
- huc_dir:                  fim directory containing individual HUC output dirs
# - ripple1d inputs dir:      input directory with HUC level ripple1d rating curves
- ripple1d_rc_filepath:     ripple1d rating curve database filepath
...
- branch_jobs:               number of multi-processing jobs to use
...
def create_ripple1d_rating_database(
    ...
    for interval in recurr_intervals:
        ...
        datestamp = check_file_age(ripple_rc_filename)
...
def run_prep(
    ...
    if branch_jobs > available_cores:
        branch_jobs = available_cores - 1
        print(
    ...
    huc = os.path.basename(os.path.normpath(huc_dir))
    csv_elev = 'ripple1d_elev_table.csv'
    ripple1d_elev_path = os.path.join(huc_dir, csv_elev)

    if not os.path.exists(ripple1d_elev_path):
        log_file.write(f'Ripple1D data is not available for huc {huc}.\n')
        print(f'Ripple1D data is not available for huc {huc}.\n')
        return

    log_file.write(f'Ripple1D data available and will perform SRC adjustments for huc {huc}\n')
    ...
    if ripple1d_elev_df is None:
        ...
        warn_err = 'WARNING: ripple1d_elev_df is empty - check that ' + csv_elev + ' files exist in huc_dir!'
        print(warn_err)
        log_file.write(warn_err)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-huc_dir', '--huc-dir', help='Parent directory of FIM run.', required=True)
    # parser.add_argument(
    #     '-ripple1d_input',
    #     '--ripple1d-dir',
    ...
    ripple_rc_filename = args['ripple1d_ratings']
