...
shared_data = {"connected": False, "current_saving_job": ""}
...
if os.path.exists("/data/outputs/current_jobs.json"):
    with open("/data/outputs/current_jobs.json") as f:
        ...
        for job_name in current_jobs.keys():
            if (
                "is_actively_saving" in current_jobs[job_name]
                and current_jobs[job_name]["is_actively_saving"] == True
            ):
    ...
    while True:
        ...
        for job_name in current_jobs.keys():
            ...
            if current_jobs[job_name]["status"] == "Cancelled":
                ...
                if (
                    current_jobs[job_name]["time_elapsed"] > 0
                    and current_jobs[job_name]["container_state"] != "exited"
                ):
                    subprocess.call(f"docker container stop {job_name}", shell=True)
                    subprocess.call(f"docker container rm {job_name}", shell=True)

                print(
                    "output_handler finished, deleted temp source files and output files"
                )
            ...
            if current_jobs[job_name]["status"] in active_statuses:
                current_jobs[job_name]["time_elapsed"] = math.ceil(
                    time.time() - current_jobs[job_name]["time_started"]
                )
            ...
            if current_jobs[job_name]["status"] == "In Queue":
                ...
                if current_jobs[job_name]["hucs_type"] == "6":
                    potential_active_cores = (
                        current_jobs[job_name]["parallel_jobs"] * 5 + total_active_cores
                    )
                else:
                    potential_active_cores = (
                        current_jobs[job_name]["parallel_jobs"] + total_active_cores
                    )
                ...
                if potential_active_cores <= MAX_ALLOWED_CPU_CORES:
                    ...
                    print(
                        f"cd /data/temp && git clone -b {branch} {GITHUB_REPO} {job_name} && chmod -R 777 {job_name} && cp .env {job_name}/tools/.env"
                    )
                    subprocess.call(
                        f"cd /data/temp && git clone -b {branch} {GITHUB_REPO} {job_name} && chmod -R 777 {job_name} && cp .env {job_name}/tools/.env",
                        shell=True,
                    ...
                    print(
                        f"docker run -d --name {job_name} -v {DATA_PATH}:/data/ -v {DATA_PATH}temp/{job_name}/:/foss_fim {DOCKER_IMAGE_PATH} fim_run.sh -u \"{hucs}\" -e {extent} -c {config_path} -n {nice_name} -o {'' if dev_run else '-p'} {'-v' if viz_run else ''} -j {parallel_jobs}"
                    )
                    subprocess.call(
                        f"docker run -d --name {job_name} -v {DATA_PATH}:/data/ -v {DATA_PATH}temp/{job_name}/:/foss_fim {DOCKER_IMAGE_PATH} fim_run.sh -u \"{hucs}\" -e {extent} -c {config_path} -n {nice_name} -o {'' if dev_run else '-p'} {'-v' if viz_run else ''} -j {parallel_jobs}",
                        shell=True,
            ...
            if (
                ...
                exit_code_raw = (
                    os.popen(
                        f"docker inspect {job_name}" + " --format='{{.State.ExitCode}}'"
                    )
                    .read()
                    .splitlines()
                )
                ...
                subprocess.call(
                    f"docker logs {job_name} >& /data/outputs/{current_jobs[job_name]['nice_name']}/logs/docker.log",
                    shell=True,
                )
                subprocess.call(f"docker container rm {job_name}", shell=True)
                ...
                if current_jobs[job_name]["job_type"] == "fim_run":
                    for path, folders, files in os.walk(
                        f"/data/outputs/{current_jobs[job_name]['nice_name']}"
                    ):
                        for file in files:
                            current_jobs[job_name]["output_files_saved"][
                                os.path.join(path, file)
                            ] = 0

                    current_jobs[job_name]["total_output_files_length"] = len(
                        current_jobs[job_name]["output_files_saved"].keys()
                    )
                    ...
                    if os.path.isdir(
                        f"/data/previous_fim/{current_jobs[job_name]['nice_name']}"
                    ):
                        shutil.rmtree(
                            f"/data/previous_fim/{current_jobs[job_name]['nice_name']}"
                        )
            ...
            if current_jobs[job_name]["status"] == "Ready for Synthesize Test Cases":
                ...
                print(
                    f"docker run -d --name {job_name} -v {DATA_PATH}:/data/ -v {DATA_PATH}temp/{job_name}/:/foss_fim -w /foss_fim/tools {DOCKER_IMAGE_PATH} /foss_fim/tools/synthesize_test_cases.py -c PREV --fim-version {nice_name} --job-number {parallel_jobs} -m /data/test_cases/metrics_library/all_official_versions.csv"
                )
                subprocess.call(
                    f"docker run -d --name {job_name} -v {DATA_PATH}:/data/ -v {DATA_PATH}temp/{job_name}/:/foss_fim -w /foss_fim/tools {DOCKER_IMAGE_PATH} /foss_fim/tools/synthesize_test_cases.py -c PREV --fim-version {nice_name} --job-number {parallel_jobs} -m /data/test_cases/metrics_library/all_official_versions.csv",
                    shell=True,
            ...
            if (
                ...
                current_jobs[job_name]["exit_code"] = exit_code
                subprocess.call(
                    f"docker logs {job_name} >& /data/previous_fim/{current_jobs[job_name]['nice_name']}/logs/synthesize_test_cases_docker.log",
                    shell=True,
                )
                subprocess.call(f"docker container rm {job_name}", shell=True)

                current_jobs[job_name]["output_files_saved"][
                    "/data/test_cases/metrics_library/all_official_versions.csv"
                ] = 0
                current_jobs[job_name]["output_files_saved"][
                    f"/data/previous_fim/{current_jobs[job_name]['nice_name']}/logs/synthesize_test_cases_docker.log"
                ] = 0
                current_jobs[job_name]["total_output_files_length"] = len(
                    current_jobs[job_name]["output_files_saved"].keys()
                )
            ...
            if (
                ...
                current_jobs[job_name]["exit_code"] = exit_code
                subprocess.call(
                    f"docker logs {job_name} >& /data/previous_fim/{current_jobs[job_name]['nice_name']}/logs/eval_plots_docker.log",
                    shell=True,
                )
                subprocess.call(f"docker container rm {job_name}", shell=True)

                current_jobs[job_name]["output_files_saved"][
                    f"/data/previous_fim/{current_jobs[job_name]['nice_name']}/logs/eval_plots_docker.log"
                ] = 0
                for path, folders, files in os.walk(
                    "/data/test_cases/metrics_library/all_official_versions_viz"
                ):
                    for file in files:
                        current_jobs[job_name]["output_files_saved"][
                            os.path.join(path, file)
            ...
            if current_jobs[job_name]["status"] == "Ready for Generate Categorical FIM":
                ...
                print(
                    f"docker run -d --name {job_name} -v {DATA_PATH}:/data/ -v {DATA_PATH}temp/{job_name}/:/foss_fim -w /foss_fim/tools {DOCKER_IMAGE_PATH} /foss_fim/tools/generate_categorical_fim.py -f {nice_name} -j {parallel_jobs}"
                )
                subprocess.call(
                    f"docker run -d --name {job_name} -v {DATA_PATH}:/data/ -v {DATA_PATH}temp/{job_name}/:/foss_fim -w /foss_fim/tools {DOCKER_IMAGE_PATH} /foss_fim/tools/generate_categorical_fim.py -f /data/previous_fim/{nice_name} -j {parallel_jobs}",
                    shell=True,
                )
                current_jobs[job_name]["container_state"] = "running"
            ...
            if (
                ...
                current_jobs[job_name]["exit_code"] = exit_code
                subprocess.call(
                    f"docker logs {job_name} >& /data/previous_fim/{current_jobs[job_name]['nice_name']}/logs/generate_categorical_fim.log",
                    shell=True,
                )
                subprocess.call(f"docker container rm {job_name}", shell=True)
                ...
                if os.path.isdir(f"/data/catfim/{current_jobs[job_name]['nice_name']}"):
                    for path in glob.glob(
                        f"/data/catfim/{current_jobs[job_name]['nice_name']}/**/mapping/*",
                        recursive=True,
                    ):
                        if not os.path.isdir(path):
                            shutil.move(
                                path,
                                f"/data/catfim/{current_jobs[job_name]['nice_name']}_temp",
                            )
                            filename = os.path.basename(path)
                            current_jobs[job_name]["output_files_saved"][
                                os.path.join(
                                    f"/data/catfim/{current_jobs[job_name]['nice_name']}",
                                    filename,
                                )
                            ] = 0
                ...
                current_jobs[job_name]["output_files_saved"][
                    f"/data/previous_fim/{current_jobs[job_name]['nice_name']}/logs/generate_categorical_fim.log"
                ] = 0
                current_jobs[job_name]["total_output_files_length"] = len(
                    current_jobs[job_name]["output_files_saved"].keys()
                )
            ...
            if current_jobs[job_name]["status"] == "Ready to Save File" and (
                shared_data["current_saving_job"] == ""
                or shared_data["current_saving_job"] == current_jobs[job_name]
            ):
                ...
                for path in current_jobs[job_name]["output_files_saved"]:
                    if current_jobs[job_name]["output_files_saved"][path] != -1:
                        output_to_save = {
                            "path": path,
                            "chunk_index": current_jobs[job_name]["output_files_saved"][
                                path
                            ],
                        }
                ...
                if is_done:
                    ...
                    if current_jobs[job_name]["job_type"] == "release":
                        ...
                        if os.path.isdir(f"{outputs_path}/aggregate_fim_outputs"):
                            shutil.move(
                                f"{outputs_path}/aggregate_fim_outputs", destination
                            )
                    ...
                    current_jobs[job_name]["status"] = (
                        "Completed"
                        if current_jobs[job_name]["exit_code"] == 0
                        else "Error"
                    )
            ...
            if (
                ...
                or current_jobs[job_name]["status"] == "Error"
            ) and time.time() >= current_jobs[job_name]["time_started"] + current_jobs[
                job_name
            ][
                "time_elapsed"
            ] + 900:
        ...
        job_updates = [
            {
                ...
                "current_output_files_saved_length": job[
                    "current_output_files_saved_length"
                ],
            }
        ...
        with open("/data/outputs/current_jobs.json.temp", "w") as f:
            ...
            shutil.move(
                "/data/outputs/current_jobs.json.temp",
                "/data/outputs/current_jobs.json",
            )
...
def ws_add_job_to_queue(data):
    ...
    if job_type == "fim_run":
        ...
        parallel_jobs = (
            parallel_jobs
            if parallel_jobs <= MAX_ALLOWED_CPU_CORES
            else MAX_ALLOWED_CPU_CORES
        )
