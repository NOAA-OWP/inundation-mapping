...
socketio = SocketIO(
    app,
    cors_allowed_origins=[SOCKET_URL, FRONTEND_URL, "http://fim_node_connector:6000"],
)

shared_data = {"handler_sid": None, "updater_sid": None}
...
def ws_ready_for_output_handler(data):
    ...
    with open(path, "rb") as binary_file:
        ...
        if len(file_chunk) == 0:
            ...
            emit(
                "file_saved",
                {"job_name": job_name, "file_path": path},
    ...
    if job_type == "fim_run":
        ...
        if len(validation_errors) == 0:
            for extent in extents:
                ...
                print(
                    f"adding job {job_name} {branch} {preset} {hucs} {parallel_jobs} {hucs_type} {extent.lower()} {config_path} {dev_run} {viz_run}"
                )
                emit(
                    "add_job_to_queue",
                    {
        ...
        job_name_base = (
            f"fim_3_{job_version_major}_{job_version_minor}_{job_version_patch}"
        )
        ...
        huc_lists = [
            "/data/inputs/huc_lists/included_huc8.lst",
            "/data/inputs/huc_lists/included_huc8_ms.lst",
        ]
        extents = ["FR", "MS"]
