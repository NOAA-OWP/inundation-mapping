...
%DIF DEL owp_fim4_expanded_original.tex   Thu Mar 30 15:26:56 2023
%DIF ADD owp_fim4_expanded.tex            Thu Apr  6 11:24:52 2023
...
%
%
\DIFdelbegin %DIFDELCMD < \authors{Fernando Aristizabal\affil{1,2,3}, Fernando Salas\affil{3}, Gregory Petrochenkov\affil{4}, Trevor Grout\affil{1,3}, Brian Avant\affil{1,3}, Bradford Bates\affil{1,3}, Ryan Spies\affil{1,3}, Nick Chadwick\affil{3}, Zachary Wills\affil{3,5}, Jasmeet Judge\affil{2}}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \authors{Fernando Aristizabal\affil{1,2,3}, Fernando Salas\affil{3}, Gregory Petrochenkov\affil{1,3}, Trevor Grout\affil{1,3}\thanks{Colorado Basin River Forecast Center, National Weather Service, National Oceanic and Atmospheric Administration, Salt Lake City, UT, USA}, Brian Avant\affil{1,3}, Bradford Bates\affil{1,3}, Ryan Spies\affil{1,3}, Nick Chadwick\affil{3}, Zachary Wills\affil{1,3}, Jasmeet Judge\affil{2}}
\DIFaddend %
%
...
\DIFdelbegin %DIFDELCMD < \affiliation{4}{Hydrologic Applied Innovations Lab, New York Water Science Center, United States Geological Survey, Troy, NY, USA}
%DIFDELCMD < \affiliation{5}{Cooperative Institute for Satellite Earth System Studies, University of Maryland, College Park, MD, USA}
%DIFDELCMD < %%%
\DIFdelend %
...
%
\begin{keypoints}
\item Flood maps derived from Height Above Nearest Drainage (HAND) are subject to nearest \DIFdelbegin \DIFdel{drainage line }\DIFdelend \DIFaddbegin \DIFadd{flowpath }\DIFaddend limitations that affect inundation skill.
...
\begin{abstract}
Height Above Nearest Drainage (HAND), a drainage normalizing terrain index, is a means able of producing \DIFdelbegin \DIFdel{high resolution }\DIFdelend flood inundation maps (FIMs) from the National Water Model (NWM) at large scales and high resolutions using reach-averaged synthetic rating curves. 
We highlight here that HAND is limited to producing inundation only when sourced from its nearest \DIFdelbegin \DIFdel{drainage line}\DIFdelend \DIFaddbegin \DIFadd{flowpath}\DIFaddend , thus lacks the ability to source inundation from multiple fluvial sources.
...
The United States National Water Model (NWM) provides flood forecasts \DIFdelbegin \DIFdel{for the entire country so that adequate warnings can be raised to the public to enable }\DIFdelend \DIFaddbegin \DIFadd{to adequately warn people for }\DIFaddend safe evacuations and protective measures \DIFdelbegin \DIFdel{.
In order to convert flow rates }\DIFdelend \DIFaddbegin \DIFadd{across the entire country.
To convert streamflow }\DIFaddend from the NWM to flood inundation maps (FIM), a model, \DIFdelbegin \DIFdel{known as }\DIFdelend Height Above Nearest Drainage (HAND), is used that \DIFdelbegin \DIFdel{converts }\DIFdelend \DIFaddbegin \DIFadd{translates }\DIFaddend elevation data from height above mean sea-level to height above the nearest river\DIFdelbegin \DIFdel{bottom}\DIFdelend .
This model suffers from issues in mapping performance because inundation sourced from rivers is only considered from the nearest river\DIFdelbegin \DIFdel{line}\DIFdelend .
...
This is done by splitting the stream network into continuous river segments known as level paths (LPs) \DIFdelbegin \DIFdel{.
These LPs have no tributaries, thus are known to be stream lines with a unit stream order indicating no branching}\DIFdelend \DIFaddbegin \DIFadd{which removes the effects of tributaries}\DIFaddend .
HAND is computed independently for each LP and the resulting FIMs are mosaiced together to form one seamless map.
\DIFaddbegin \DIFadd{By computing HAND and catchments on an LP scale, catchments and FIMs are allowed to overlap which can account for multiple river sources of inundation especially at river confluences.
}\DIFaddend We compared these HAND derived FIMs to maps from physically-based models and found improvement in mapping performance.
%
...
AHPS \DIFaddbegin \DIFadd{utilizes a variety of techniques including distributed modeling \mbox{%DIFAUXCMD
\cite{reed2004overall,koren2004hydrology,duan2003global}}\hskip0pt%DIFAUXCMD
...
, hydraulic routing \mbox{%DIFAUXCMD
\cite{cajina2002fldview,fread1973technique}}\hskip0pt%DIFAUXCMD
, multi-sensor precipitation methods \mbox{%DIFAUXCMD
\cite{seo2002real,kondragunta2001outlier,breidenbach1999accounting,bonnin1996noaa}}\hskip0pt%DIFAUXCMD
, flood inundation mapping \mbox{%DIFAUXCMD
\cite{cajina2002fldview}}\hskip0pt%DIFAUXCMD
, and calibration techniques \mbox{%DIFAUXCMD
\cite{zhang2003hydrologic,hogue2003multi,duan2003global,gupta2003advances,parada2003multi}}\hskip0pt%DIFAUXCMD
.
AHPS }\DIFaddend provides forecasting services in the form of ensemble streamflows at more than 3,600 locations and flood inundation maps (FIM) at more than 150 of those points shown in Figure \ref{fig:forecast_points}.
...
\begin{figure}[H]
\centering
\DIFdelbeginFL %DIFDELCMD < \includegraphics[scale=1.0]{figures/forecast_points.jpg}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[scale=0.75]{figures/forecast_points.jpg}
\DIFaddendFL \caption{Forecast points with and without Flood Inundation Maps (FIM) in United States' Advanced Hydrologic Prediction System (AHPS).
...
The Height Above Nearest Drainage (HAND) terrain model is one such technique that can be used, along with synthetic rating curves (SRC), to convert 1-dimensional (1D) riverine discharges to stages, and finally to inundation extents\DIFdelbegin \DIFdel{and depths}\DIFdelend .
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Height Above Nearest Drainage}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
\DIFdelbegin \DIFdel{HAND normalizes topography along the nearest drainage path and it has been demonstrated to be a good proxy and indicator of a series of important environmental conditions including soil environments, landscape classes, soil gravitational potentials, geomorphologies, soil moisture, and groundwater dynamics \mbox{%DIFAUXCMD
\cite{renno2008hand,nobre2011height}}\hskip0pt%DIFAUXCMD
. 
}\DIFdelend \citeA{nobre2016hand} showed evidence for utilizing the drainage normalizing HAND dataset as a proxy for flood potential to make static flood inundation maps from known stages.
...
This collection of methods, coupling HAND with SRCs, have been experimented with and compared to other sources of FIM including engineering scale models, in-situ observation, and remote sensing based observation with solid results in large spatial scale applications \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{godbout2019error,johnson2019integrated,garousi2019terrain,nobre2016hand,afshari2018comparison,zheng2018geoflood,teng2015rapid,teng2017flood,zhang2018comparative}}\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{godbout2019error,johnson2019integrated,garousi2019terrain,nobre2016hand,afshari2018comparison,zheng2018geoflood,teng2015rapid,teng2017flood,zhang2018comparative,hocini2020performance}}\hskip0pt%DIFAUXCMD
}\DIFaddend .
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
Additionally, in order to flood an area, HAND assumes all areas eligible for inundation must drain to some nearest \DIFdelbegin \DIFdel{stream line }\DIFdelend \DIFaddbegin \DIFadd{flowpath }\DIFaddend which is used for catchment allocation and relative elevation calculation \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
...
\cite{nobre2016hand,nobre2011height,liu2016cybergis,liu2020height,maidment2017conceptual,garousi2019terrain,zheng2018river,zheng2018geoflood,johnson2019integrated,renno2008hand,hocini2020performance}}\hskip0pt%DIFAUXCMD
}\DIFaddend . 
...
When used for FIM applications, HAND assumes only fluvial inundation sourced from its nearest \DIFdelbegin \DIFdel{drainage line }\DIFdelend \DIFaddbegin \DIFadd{flowpath }\DIFaddend is accounted for \cite{nobre2016hand,mcgehee2016modified}.
...
We illustrate how computing HAND independently for \DIFdelbegin \DIFdel{stream lines }\DIFdelend \DIFaddbegin \DIFadd{flowpaths }\DIFaddend of unit stream order can significantly enhance FIM performance by accounting for multiple sources of fluvial inundation that may exist in certain regions and flow scenarios.
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
Originally, NFIE utilized the NHD Medium Resolution (MR) to \DIFdelbegin \DIFdel{etch or burn flowlines }\DIFdelend \DIFaddbegin \DIFadd{enforce flowpaths }\DIFaddend prior to further conditioning but more recent work has advanced this to the more current NHDPlus High Resolution (NHDPlusHR) which better agrees with the 10 m DEM from the NHDPlusHR program \cite{liu2020height}. 
...
\DIFaddbegin \label{ssec:owp_fim}
\DIFaddend %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
In order to mitigate the ever increasing threat of flooding to life and property, an operational capability is required to extend NWM streamflow forecasts to river stages \DIFdelbegin \DIFdel{, inundation extents, and inundation depths}\DIFdelend \DIFaddbegin \DIFadd{and inundation extents}\DIFaddend .
...
In previous studies, FIM skill has been shown sensitivity to the drainage density of the stream network employed as the datum for HAND which is closely related to the maximum Horton-Strahler stream order of the network \mbox{%DIFAUXCMD
...
\DIFadd{Figure \ref{fig:catchment_boundaries_issue} illustrates the exact situation our solution proposes to address where two tributaries converge with a higher order stream segment. 
...
\caption{\DIFaddFL{The figure represents an agreement map between a HAND derived FIM and one produced from the Base Level Engineering (BLE) program for a 100 yr magnitude event at HUC8 12090301.
...
\DIFadd{We seek to resolve this catchment boundary problem or nearest drainage limitation by discretizing the target stream network into stream networks of reduced, unit stream order to avoid the constraining of catchments by those belonging to lower order neighbors.
...
\DIFaddend Here we demonstrate how reducing a HAND processing unit's stream network into discrete \DIFdelbegin \DIFdel{level paths (LPs ) }\DIFdelend \DIFaddbegin \DIFadd{LPs }\DIFaddend of singular, effective stream order, can enhance FIM skill by accounting for multiple possible sources of fluvial inundation.
This capability is introduced progressively as MS (whose network represents about 4\% of FR network) and to a higher degree \DIFdelbegin \DIFdel{Generalized Mainstems (GMS ) }\DIFdelend \DIFaddbegin \DIFadd{GMS }\DIFaddend (covers entire FR network) which will be explained later on.
...
A series of hydro-conditioning steps enforces the location of \DIFdelbegin \DIFdel{stream lines}\DIFdelend \DIFaddbegin \DIFadd{flowpaths}\DIFaddend , monotonically decreasing elevations, excavated bathymetry, stream thalweg breaching, and levee enforcement.
...
\begin{figure}[H]
\centering
\DIFdelbeginFL %DIFDELCMD < \includegraphics[scale=6.0]{figures/methods_overview.jpg}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[scale=0.17]{figures/methods_overview.jpg}
\DIFaddendFL \caption{
...
Source code is made available for this project \DIFdelbegin \DIFdel{on GitHub }\DIFdelend where a user could consult the Readme.md page for more information on how to acquire the datasets and reproduce the pipeline \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{inundationMapping2022}}\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{inundationMapping2022,imHS2023}}\hskip0pt%DIFAUXCMD
}\DIFaddend .
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Datasets}
...
Instructions for accessing data processed for OWP FIM are provided on the project's GitHub \DIFaddbegin \DIFadd{or HydroShare }\DIFaddend page via an Amazon Web Services (AWS) S3 bucket furnished by the Earth Science Information Partners (ESIP) \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{esipData2022}}\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{esipData2022,inundationMapping2022,imHS2023}}\hskip0pt%DIFAUXCMD
}\DIFaddend .
...
These BurnLines used in conjunction with the hydrofabric of the NWM V2.1 to help define \DIFdelbegin \DIFdel{flowlines }\DIFdelend \DIFaddbegin \DIFadd{flowpaths }\DIFaddend for OWP FIM while the NWM hydrofabric is also used to define reservoirs for exclusion and catchments to cross-walk against for forecasting purposes \cite{nwm2022hydrofabric}.
...
\hline
USGS & NHDPlusHR BurnLineEvents & \DIFdelbegin \DIFdel{Stream lines }\DIFdelend \DIFaddbegin \DIFadd{Flowpaths }\DIFaddend used by NHDPlusHR for hydro-enforcement. & \cite{nhdplus2022vectors} \\
\hline
...
\hline
NOAA-OWP & NWM Streams & \DIFdelbegin \DIFdel{Stream network center lines }\DIFdelend \DIFaddbegin \DIFadd{Flowpaths }\DIFaddend used by NWM for routing and forecasting adapted from NHDPlus V2 NHDFlowline\_Network feature class. & \cite{nwm2022hydrofabric} \\
\hline
...
\hline
InFRM & Flood Inundation Extents & Inundation \DIFdelbegin \DIFdel{depths }\DIFdelend \DIFaddbegin \DIFadd{extents }\DIFaddend produced by InFRM BLE HEC-RAS 1D for 1\% and 0.2\% recurrence interval events. & \cite{fema2021estimated} \\
\hline
\end{tabular}
%\end{table}
\end{sidewaystable}
...
Hydro-conditioning is implemented to obtain many objectives including enforcing the location of hydrologically relevant features such as \DIFdelbegin \DIFdel{flowlines}\DIFdelend \DIFaddbegin \DIFadd{flowpaths}\DIFaddend , lakes, or drainage divides whether natural or anthropogenic. 
...
\DIFdelbegin %DIFDELCMD < \label{ssec:stream_network_enforcment}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \label{ssec:stream_network_enforcement}
\DIFaddend %
\DIFdelbegin \DIFdel{The location }\DIFdelend \DIFaddbegin \DIFadd{Both the location and geometry }\DIFaddend of the stream network \DIFdelbegin \DIFdel{is enforced to ensure general agreement }\DIFdelend \DIFaddbegin \DIFadd{are enforced to agree }\DIFaddend with the NWM \DIFdelbegin \DIFdel{network which is used for forecasting the streamflow inputs}\DIFdelend \DIFaddbegin \DIFadd{stream network and to enforce synthetic bathymetry and generate more hydrologically correct catchments}\DIFaddend .
...
%DIFDELCMD < %%%
\DIFdel{However, to better match the drainage density of the NWM FR V2.1 stream network , which is based on the NHDPlus V2, the BurnlineEvents are pruned utilizing a nearest neighbor search around the NWM flowlines.
...
Any headwater portion in the NHDPlusHR or any other stream not traversed are pruned away }\DIFdelend \DIFaddbegin \DIFadd{This network goes through a stream density reduction procedure }\DIFaddend to better match the \DIFdelbegin \DIFdel{resolution and spatial locations }\DIFdelend \DIFaddbegin \DIFadd{stream density }\DIFaddend of the NWM stream network\DIFdelbegin \DIFdel{and its headwater points.
...
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{This figure illustrates some of the datasets that result from the pruning of NHDPlusHR Beta BurnlineEvents (dotted black) to the stream density of the NWM FR V2.1 density (blue).
...
%DIFDELCMD < %%%
\DIFdel{The pruned stream network is then utilized to hydro-enforce the DEM with a methodology developed by \mbox{%DIFAUXCMD
\citeA{hellweger1997agree} }\hskip0pt%DIFAUXCMD
known as the }\DIFdelend \DIFaddbegin \DIFadd{.
This reduced density NHDPlusHR network is now used for hydro-enforcement along with the }\DIFaddend AGREE DEM Surface Reconditioning System \DIFdelbegin \DIFdel{. 
...
Using the AGREE method as opposed to simple thalweg burning techniques helps prevent distortions in the delineation of streams as well as the catchment boundaries \mbox{%DIFAUXCMD
\cite{saunders1995grid,saunders1996gis,mizgalewicz1996modeling,hellweger1997agree,quenzer1998gis,baker2006comparison}}\hskip0pt%DIFAUXCMD
.
\mbox{%DIFAUXCMD
\citeA{baker2006comparison} }\hskip0pt%DIFAUXCMD
...
}\DIFdelend \DIFaddbegin \DIFadd{to excavate channels into the DEM \mbox{%DIFAUXCMD
\cite{hellweger1997agree}}\hskip0pt%DIFAUXCMD
.
}\DIFaddend Downsides to the technique include the possibility of exhibiting parallel streams where the burned stream and real stream are both represented \cite{hellweger1997agree,saunders1999preparation} and some distortion of the catchment boundaries can also be observed \cite{saunders1999preparation,saunders1996gis}.
Some of these drawbacks are addressed by additional conditioning techniques applied later on.
\DIFaddbegin \DIFadd{For more details on how we reduce the density of the NHDPlusHR stream network along with technical implementation information on the AGREE DEM procedure, please see \ref{sec:app_stream_network_enforcement}.
}\DIFaddend %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%
Local depressions are naturally occurring features of a DEM but must be addressed \DIFdelbegin \DIFdel{if }\DIFdelend \DIFaddbegin \DIFadd{to derive }\DIFaddend a connected drainage network with continuous catchments \DIFdelbegin \DIFdel{are to be derived }\DIFdelend for flood modeling purposes with HAND.
...
These datasets include the HAND or relative elevation model (REM) raster, reach-level catchments raster/polygons, DEM-derived \DIFdelbegin \DIFdel{streamlines}\DIFdelend \DIFaddbegin \DIFadd{flowpaths}\DIFaddend , SRCs, and cross-walk table.
\DIFaddbegin \DIFadd{Within the context of this section, we refer to HAND as a grid of relative elevations to detrend elevations away from mean level towards elevations referenced to the nearest flowpath (See Section \ref{sssec:hand}).
...
}\DIFaddend The following sub-sections describe how the subset and hydro-enforced geospatial datasets are converted into the FIM hydrofabric.
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsubsection{Flow Directions and Flats Resolution}
\label{ssec:flow_direction_and_flat_resolution}
%
To facilitate the generation of a connected stream network and its associated catchments from the conditioned DEM, the depression-filled DEM is used to derive connectivity in the form of D-8 flow directions \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{o1984extraction}}\hskip0pt%DIFAUXCMD
}\DIFaddend .
\DIFdelbegin \DIFdel{D-8 seeks to allocate a drainage direction for every pixel based on the adjacent eight pixel neighborhood with the steepest slope \mbox{%DIFAUXCMD
\cite{o1984extraction}}\hskip0pt%DIFAUXCMD
.
...
%DIFDELCMD < %%%
\DIFdelend Flat resolution from flats endemic to the DEM or from depression filled regions is a costly, non-trivial procedure which was originally addressed by \citeA{garbrecht1997assignment} where flats are resolved by incrementing elevations iteratively.
\DIFdelbegin \DIFdel{Software implementations have developed means to partition the problem and resolve flats iteratively with communication across processes \mbox{%DIFAUXCMD
\cite{tarboton2009generalized,tesfa2011extraction,wallis2009parallel,tarboton2005terrain}}\hskip0pt%DIFAUXCMD
.
The excessive iteration and communication leads to poor computational performance which motivated further work on how to optimize flat resolution \mbox{%DIFAUXCMD
\cite{survila2016scalable,barnes2014efficient}}\hskip0pt%DIFAUXCMD
.
The established literature in this niche field of hydrology discusses how prevalent flats can be in given study areas and how difficult the problem is from both computational and hydrologic stand-points \mbox{%DIFAUXCMD
...
.
}\DIFdelend OWP FIM utilized a CyberGIS implementation of the D-8 flow direction algorithm with the accelerated resolution of flats which we found \DIFdelbegin \DIFdel{to be very }\DIFdelend efficient and effective \cite{survila2016scalable,cybergis2016}.
\DIFaddbegin \DIFadd{For more information on the derivations of flow directions and resolving flats, please see \ref{sec:app_flow_direction_and_flat_resolution}.
}\DIFaddend %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
This is accomplished by using the seed points generated from the stream network enforcement process (Section \DIFdelbegin \DIFdel{\ref{ssec:stream_network_enforcment}}\DIFdelend \DIFaddbegin \DIFadd{\ref{ssec:stream_network_enforcement} and \ref{sec:app_stream_network_enforcement}}\DIFaddend ).
These seeds points are \DIFdelbegin \DIFdel{pruned }\DIFdelend headwater locations of the NHDPlusHR Beta BurnlineEvents layer that spatially correspond to the headwater definitions in the stream network of the NWM V2.1.
...
\DIFaddbegin \DIFadd{This parameter was held constant across all versions of HAND that we used which are introduced in Section \ref{ssec:stream_order_reduction}.
}\DIFaddend Small reaches can lead to unrealistic variances in channel geometries while oversized reaches can lead to grouping too much slope variance into one discretization of the stream network.
...
Outlet points are set at the pixel center points of the delineated \DIFdelbegin \DIFdel{stream lines }\DIFdelend \DIFaddbegin \DIFadd{flowpaths }\DIFaddend explained in Section \ref{sssec:deriving_fim_stream_network}.
...
\DIFaddbegin \label{sssec:hand}
\DIFaddend %
Once the pixel level catchments are derived, the final relative elevations can be computed.
\DIFdelbegin \DIFdel{The elevation of every thalweg pixel is subtracted from }\DIFdelend \DIFaddbegin \DIFadd{To compute these relative elevations, we utilize the same technique found in previous HAND implementations \mbox{%DIFAUXCMD
\cite{zheng2018geoflood,zheng2018river,nobre2011height,nobre2016hand,maidment2017conceptual,garousi2019terrain}}\hskip0pt%DIFAUXCMD
.
For each pixel level catchment described in Section \ref{sssec:catchments}, we subtract }\DIFaddend the elevations of the non-thalweg pixels \DIFdelbegin \DIFdel{within the same, corresponding pixel-level catchmentdescribed in Section \ref{sssec:catchments}.
The }\DIFdelend \DIFaddbegin \DIFadd{from the thalweg pixel of that catchment.
Contrary to some of the some other previous HAND implementations, the }\DIFaddend DEM used for this operation is the DEM resulting from the thalweg conditioning procedures described in Section \ref{sssec:stream_thalweg_elevation_conditioning} \DIFdelbegin \DIFdel{.
Outside }\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{djokic2019arc}}\hskip0pt%DIFAUXCMD
.
This DEM utilizes the native elevations in regions outside }\DIFaddend of the excavated channel from the AGREE DEM method \DIFdelbegin \DIFdel{, the native non-drainage enforced elevations are used to reduce sources of error in relative elevations due to pit filling }\DIFdelend \cite{djokic2019arc}.
...
While this is required for the generation of DEM-derived catchments and \DIFdelbegin \DIFdel{stream lines}\DIFdelend \DIFaddbegin \DIFadd{flowpaths}\DIFaddend , it is not necessarily required for the computation of the relative elevations.
...
Previously not reported in literature to our knowledge in this form, the reach averaged Manning's formula is derived \DIFdelbegin \DIFdel{to be 
...
\DIFaddbegin \DIFadd{We constrain the equation by selecting 84 stage values (y in Eq. \ref{eq:reach_averaged_mannings_equation}) from 0 to 25 meters in depth at a third of a meter increments to calculate the discharge values for each stage value. 
Due to the varying nature of stage (y in Equation \ref{eq:reach_averaged_mannings_equation}) as explained, the terms V(y) and B(y) also update according to previous work with reach averaged SRCs \mbox{%DIFAUXCMD
\cite{zheng2018river}}\hskip0pt%DIFAUXCMD
.
}\DIFaddend 
...
We note significant opportunity to enhance CFIM skill by better localizing Manning's n according to available data including but not limited to land cover, land use, stream order, stream geometry, drainage area, reach length, and discharge percentiles \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{garousi2019terrain,johnson2019integrated}}\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{garousi2019terrain,johnson2019integrated,godbout2019error,zheng2018river}}\hskip0pt%DIFAUXCMD
}\DIFaddend .
...
\DIFdelbegin \DIFdel{After all the parameters to the Manning's equation have been determined with either hydrofabric sampling or user parameterization, we select 84 stage values (y in Eq. \ref{eq:reach_averaged_mannings_equation}) from 0 to 25 meters in depth at a third of a meter increments to calculate the discharge values for each stage value. 
}\DIFdelend %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsubsection{Cross-walking with NWM Stream Network}
\label{sssec:cross_walking_networks}
%
The DEM based stream network derived in Section \ref{sssec:deriving_fim_stream_network} must be associated with NWM reach identifiers so that a discharge can be converted to stage and later inundation extents\DIFdelbegin \DIFdel{and depths}\DIFdelend .
...
For two subsequent HAND methods, MS and GMS, discussed in Sections \ref{sssec:nws_mainstems} and \ref{sssec:generalized_mainstems}, respectively, we find the mid-point of the derived stream reach \DIFdelbegin \DIFdel{line }\DIFdelend described in Section \ref{sssec:deriving_fim_stream_network} and find the NWM catchment that contains the mid-point.
...
\DIFdelbegin \DIFdel{HAND produced FIMs are limited in only providing inundation sourced from the nearest drainage line, however, depending on flow conditions and topography, a given area may have multiple contributing fluvial sources of surface water inundation .
The forecasting community, in reviewing HAND, have noted significant negative effects at the confluence of lower flow tributaries with higher flow rivers for which the phrase ``catchment boundary issue'' has been termed.
In previous studies, FIM skill has been shown to be sensitive to the drainage density of the stream network employed as the datum for HAND which is closely related to the maximum Horton-Strahler stream order of the network \mbox{%DIFAUXCMD
\cite{zhang2018comparative,mcgehee2016modified,li2020evaluation,nobre2016hand}}\hskip0pt%DIFAUXCMD
.
...
%DIFDELCMD < %%%
\DIFdel{Figure \ref{fig:catchment_boundaries_issue} illustrates the exact situation our solution proposes to address where two tributaries converge with a higher order stream segment. 
An actual map with OWP FIM is generated using the NWM full-resolution stream network and compared with a FEMA 100 yr extent (see Section \ref{ssec:evaluation} for more details) showing significant under-prediction in inundation extent.
...
%DIFDELCMD < \centering
%DIFDELCMD < \includegraphics[scale=1.0]{figures/catchment_boundaries_issue.jpg}
%DIFDELCMD < %%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{The figure represents an agreement map between a HAND derived FIM and one produced from the Base Level Engineering (BLE) program for a 100 yr magnitude event at HUC8 12090301.
Agreement maps are symbolized by false negatives (FN), true negatives (TN), false positives (FP), and true positives (TP) where inundated represents the positive condition (see Section \ref{ssec:evaluation} for more details).
...
%DIFAUXCMD
%DIFDELCMD < \label{fig:catchment_boundaries_issue}
%DIFDELCMD < \end{figure}
%DIFDELCMD < %%%
%DIF < 
%DIFDELCMD < 
...
%DIFDELCMD < %%%
\DIFdel{We seek to resolve this catchment boundary problem or nearest drainage limitation by discretizing the target stream network into stream networks of reduced, unit }\DIFdelend \DIFaddbegin \DIFadd{As introduced in Section \ref{ssec:owp_fim}, HAND based FIMs fail to account for multiple sources of fluvial inundation which leads to flow constrictions at high flow confluences.
We hypothesized that reducing the scale of HAND computation down to }\DIFaddend stream \DIFdelbegin \DIFdel{order to avoid the constraining of catchments by those belonging to lower order neighbors.
...
This creates much larger and overlapping catchments that can source fluvial inundation from multiple reaches as required by the given river stage at current flow conditions.
We present two successive methods, National Weather Service MS (Section \ref{sssec:nws_mainstems}) and GMS (Section \ref{sssec:generalized_mainstems}), implemented that reduce the effective Horton-Strahler stream orders of the networks employed and test our presented hypothesis that unary stream order networks enhance FIM performance skill with HAND by expanding the nearest drainage definition to increase potential inundationareas.
}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{networks of unit Horton-Strahler stream order can help account for multiple fluvial sources of inundation.
}\DIFaddend To clarify the phrase ``reducing Horton-Strahler stream order'' used extensively in this paper, every FIM used in evaluation contains a flood extent sourced from every NWM forecast point in the given evaluation domain.
...
LPs group \DIFdelbegin \DIFdel{flowlines }\DIFdelend \DIFaddbegin \DIFadd{flowpaths }\DIFaddend by maximizing the length of each \DIFdelbegin \DIFdel{flow path }\DIFdelend \DIFaddbegin \DIFadd{flowpath }\DIFaddend and minimizing the number of LP identifiers within a given domain \cite{moore2019user,mckay2012nhdplus}. 
In order to derive LPs for the NWM FR stream network at the HUC8 processing area, we first compute arbolate sums which are defined as the cumulative drainage distance of all upstream \DIFdelbegin \DIFdel{drainage lines}\DIFdelend \DIFaddbegin \DIFadd{flowpaths}\DIFaddend .
...
The figure shows computed arbolate sums and unique LP identifiers on a \DIFdelbegin \DIFdel{HUC12 (120903010404}\DIFdelend \DIFaddbegin \DIFadd{headwater subset of HUC8 (12020002}\DIFaddend ) for clarity but were computed at the corresponding HUC8.
...
For the time being, we designate this issue \DIFdelbegin \DIFdel{to be }\DIFdelend out of scope.
...
The only exception to this is the use of the NWM stream network directly for use with hydro-enforcement by burning these \DIFdelbegin \DIFdel{lines }\DIFdelend \DIFaddbegin \DIFadd{flowpaths }\DIFaddend and seeding from its headwater points directly instead of going through the NHDPlusHR network as described in Section \DIFdelbegin \DIFdel{\ref{ssec:stream_network_enforcment} }\DIFdelend \DIFaddbegin \DIFadd{\ref{ssec:stream_network_enforcement} and \ref{sec:app_stream_network_enforcement}}\DIFaddend .
This decision was motivated by the difficulty in deriving LPs in the NWM stream network with high agreement with the LPs derived for the NHDPlusHR \DIFdelbegin \DIFdel{stream lines}\DIFdelend \DIFaddbegin \DIFadd{flowpaths}\DIFaddend .
...
Once the NWM FR stream network is discretized into LPs, we independently compute HAND using each LP as the target stream network \DIFdelbegin \DIFdel{to be }\DIFdelend used.
...
The catchments and \DIFdelbegin \DIFdel{stream lines }\DIFdelend \DIFaddbegin \DIFadd{flowpaths }\DIFaddend for HAND computed at the FR scale are illustrated in Figure \ref{fig:gms_methods_2}a where the respective inundation at the 100 yr magnitude is heavily constrained by the limited catchment extents especially at junctions.
...
\caption{Illustrates the NWM Full Resolution V2.1 stream network discretized into level paths (LP), symbolized by unique colors, as well as the values of the arbolate sums \DIFdelbeginFL \DIFdelFL{to the nearest whole }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{in }\DIFaddendFL km \DIFdelbeginFL \DIFdelFL{distance}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{units}\DIFaddendFL .
The LPs were derived on a HUC8 level (\DIFdelbeginFL \DIFdelFL{12090301}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{12020002}\DIFaddendFL ) but only illustrated for a \DIFdelbeginFL \DIFdelFL{HUC12 (120903010404) }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{small, headwater subset of this HUC8 }\DIFaddendFL for clarity \DIFaddbeginFL \DIFaddFL{purposes}\DIFaddendFL .
\DIFdelbeginFL \DIFdelFL{The mainstem of this HUC12 runs from the red ellipses to the outlet denoted by the black ellipses.
}\DIFdelendFL Arbolate sums are defined as the cumulative drainage distances of all upstream \DIFdelbeginFL \DIFdelFL{stream lines}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{flowpaths}\DIFaddendFL .
...
In (a), we illustrate all NWM \DIFdelbeginFL \DIFdelFL{stream lines }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{flowpaths }\DIFaddendFL symbolized by their LP with 372 unique LP IDs in this HUC.
...
In (c) and (d), we show the \DIFdelbeginFL \DIFdelFL{stream lines}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{flowpaths}\DIFaddendFL , catchments, and inundation extents of the two tributaries.
...
This process allows \DIFaddbeginFL \DIFaddFL{to consider }\DIFaddendFL for multiple, possible contributing sources of fluvial inundation \DIFdelbeginFL \DIFdelFL{to be considered }\DIFdelendFL thus enhancing FIM skill.
}
...
\DIFaddbegin \DIFadd{While we do not evaluate FIM depths in this study, we do compute depths first as to threshold them to produce extents.
}\DIFaddend For each forecast stage, $S_i$, one can express the formula for $F_{ij}$, a binary variable denoting inundation condition in Equation \ref{eq:hand_fim} in terms of $D_{ij}$ by simply thresholding at zero depths.
%
\begin{linenomath*}
\begin{equation}
...
\DIFaddbegin \DIFadd{Lastly, we apply Equation \ref{eq:hand_fim} to the mosaiced depths for the MS and GMS cases in order to obtain FIM extents which is the subject of our evaluations.
}\DIFaddend Figures \ref{fig:gms_methods}a and \ref{fig:gms_methods}b, illustrate how inundation maps are created for lower stream order processing units then mosaiced together.
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
\DIFdelbegin \DIFdel{Possible benchmark FIM candidates for evaluation purposesinclude }\DIFdelend \DIFaddbegin \DIFadd{OWP FIM is linked with the NWM for operational purposes, utilizing streamflow inputs, to produce FIM at a continental scale.
We evaluated various validation data sources for operational use, each with its own strengths and weaknesses, including }\DIFaddend high water marks \DIFaddbegin \DIFadd{(HWMs) \mbox{%DIFAUXCMD
\cite{musser2017characterization,watson2017characterization,breaker2016characterization}}\hskip0pt%DIFAUXCMD
}\DIFaddend , remote sensing \DIFdelbegin \DIFdel{observation, crowd-sourced information, }\DIFdelend \DIFaddbegin \DIFadd{observations \mbox{%DIFAUXCMD
\cite{aristizabal2020high,aristizabal2021mapping}}\hskip0pt%DIFAUXCMD
, }\DIFaddend and modeled extents \DIFdelbegin \DIFdel{.
These sources are all subject to limitations for evaluating a continental scale model like OWP FIMsuch as but not limited to a lack of spatial coverage, signal interference, lack of streamflow data, inaccurate streamflow data, physics-based assumptions, and errors in input data.
...
}\DIFdelend \DIFaddbegin \DIFadd{from hyper-resolution, local models \mbox{%DIFAUXCMD
...
\DIFdelbegin \DIFdel{Evaluation }\DIFdelend \DIFaddbegin \DIFadd{Our HAND based approach coupled with SRCs requires streamflow as input and is agnostic as to the source of that streamflow whether forecasted, observed, or probablistic.
Due to this fact, evaluation }\DIFaddend of our relative elevation CFIM method \DIFdelbegin \DIFdel{is }\DIFdelend \DIFaddbegin \DIFadd{was }\DIFaddend conducted by comparison to the HEC-RAS 1D models produced within FEMA region 6 \cite{fema2021base,fema2021estimated,us2022hydrologic}.
This dataset was selected due to its large spatial coverage, availability of cross-sections with streamflow information, higher level of sophistication when compared to HAND, engineering scale detail, and a storied use in the literature as an evaluation dataset \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
...
\cite{cook2009effect,rajib2016large,zheng2018geoflood,afshari2018comparison,wing2017validation,criss2022stage,follum2017autorapid,bates201630m,wing2017validation,hu2021real,li2022accounting,li2022comprehensive,zheng2018geoflood}}\hskip0pt%DIFAUXCMD
}\DIFaddend .
...
\DIFaddbegin \DIFadd{Since the BLE does account for reservoir inundation, some of the BLE reservoir inundation extents extend beyond the NWM reservoir geometries contributing to false negatives (FNs), or under-prediction.
}\DIFaddend 
...
Figure \ref{fig:ble_evaluation_method} illustrates both NWM V2.1 and BLE \DIFdelbegin \DIFdel{stream lines }\DIFdelend \DIFaddbegin \DIFadd{flowpaths }\DIFaddend as well as the BLE cross-sections that have recurrence discharges associated with them.
...
Additionally, the stream network of the InFRM furnished models are of higher stream densities and bifurcation ratios, as evident in Figure \ref{fig:ble_evaluation_method}, leading to a significant amount of \DIFdelbegin \DIFdel{false negatives (FN) (under-prediction) }\DIFdelend \DIFaddbegin \DIFadd{FNs }\DIFaddend along headwater streams with unit Horton-Strahler order due to the lack of representation of these additional headwater streams in the NWM network.
...
\caption{
Illustrates Base Level Engineering (BLE) cross sections and \DIFdelbeginFL \DIFdelFL{stream lines }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{flowpaths }\DIFaddendFL at the HUC8 12100203 near the confluences of West Fork Plum Creek and Clear Fork Plum Creek with Plum Creek.
BLE cross sections are intersected with NWM reaches and the median recurrence discharge for 1\% and 0.2\% levels are selected per NWM V2.1 Full Resolution (FR) \DIFdelbeginFL \DIFdelFL{stream lines}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{flowpaths}\DIFaddendFL .
Additionally, we illustrate the NWM V2.1 catchments to provide a sense of how many cross-sections may intersect a given NWM \DIFdelbeginFL \DIFdelFL{flowline}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{flowpaths}\DIFaddendFL .
The BLE stream network is also shown which is denser than the NWM V2.1 \DIFdelbeginFL \DIFdelFL{stream lines }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{flowpaths }\DIFaddendFL meaning there are several lower order streams represented in the BLE stream network that are not in the NWM V2.1 \DIFdelbeginFL \DIFdelFL{stream lines}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{flowpaths}\DIFaddendFL .
...
Some work by \citeA{gerapetritis2004behavior} denotes CSI a good proxy for measuring a forecasting system's utility in protecting life and property and has been shown \DIFdelbegin \DIFdel{to be optimized mathematically }\DIFdelend \DIFaddbegin \DIFadd{as mathematically optimized }\DIFaddend when $POD = 1 - FAR$.
...
\DIFdelbegin \DIFdel{To be more specific}\DIFdelend \DIFaddbegin \DIFadd{More specifically}\DIFaddend , Table \ref{tab:aggregate_metrics} sums the primary metrics, TP, FP, FN, and TN, across all HUC8s then recomputes the secondary metrics which was done to better account for large variances in HUC8 size.
...
Again, the main objective of the study is to introduce how computing HAND with disaggregated stream networks to those with unit stream order can enhance the fidelity of FIMs by capturing fluvial inundation from multiple sources as opposed to that of just the nearest \DIFdelbegin \DIFdel{drainage line}\DIFdelend \DIFaddbegin \DIFadd{flowpath}\DIFaddend .
...
\DIFaddbegin \DIFadd{This intuition is confirmed by previously mentioned work by \mbox{%DIFAUXCMD
\citeA{gerapetritis2004behavior} }\hskip0pt%DIFAUXCMD
where CSI was shown as a direct function of both POD and FAR where CSI is maximized when $POD = 1 - FAR$.
}\DIFaddend Upon investigation of the performance of HAND derived FIM, we note a general increase of catchment sizes for the MS and GMS methods when compared to the FR method as they are now delineated independently of any tributaries that would constrain catchment sizes otherwise.
...
In sub-figure \ref{fig:gms_enhancement}b, the agreement raster for the GMS technique is illustrated as well as the \DIFdelbegin \DIFdel{stream network lines }\DIFdelend \DIFaddbegin \DIFadd{flowpaths }\DIFaddend in green.
...
The consequence for inundation extents is a general increase in spatial coverage of the river's water which shows \DIFdelbegin \DIFdel{to be in much }\DIFdelend better agreement with the benchmark \DIFdelbegin \DIFdel{map}\DIFdelend \DIFaddbegin \DIFadd{maps}\DIFaddend .
The TPs are no longer bounded by the catchment lines and allowed to expand to their natural extents.
\DIFaddbegin \DIFadd{Additionally, this study has a limitation in that it did not assess FIM depths, so limited conclusions can be made about the impact of drainage order reduction techniques on the accuracy of FIM depths or water surface elevations.
}\DIFaddend 
...
Generally speaking, drainage areas are \DIFdelbegin \DIFdel{known to be }\DIFdelend hierarchical in nature so a given drainage area \DIFdelbegin \DIFdel{for a given }\DIFdelend \DIFaddbegin \DIFadd{of an }\DIFaddend outlet point can be decomposed into nested drainage areas for outlet points that lie in the original drainage area.
...
HAND is only equipped to handle riverine flooding from the nearest \DIFdelbegin \DIFdel{drainage line}\DIFdelend \DIFaddbegin \DIFadd{flowpaths}\DIFaddend .
Other relevant \DIFdelbegin \DIFdel{drainage lines }\DIFdelend \DIFaddbegin \DIFadd{flowpaths }\DIFaddend that produce fluvial flood waters are not considered here especially if the routing model used doesn't consider backwater effects.
...
While the channel slope and reach lengths are not exactly the same across methods, their averaged differences \DIFdelbegin \DIFdel{are very negligible }\DIFdelend \DIFaddbegin \DIFadd{were statistically insignificant }\DIFaddend which only leaves room for deviations in volume and bed area.
\DIFaddbegin \DIFadd{Specifically, we found FR to have an average reach length for the study region of 1354.8 km with a standard deviation 249.6km.
For GMS, we computed an average reach length of 1398.1 km with a standard deviation of 50.2 km.
}\DIFaddend Again, volume (V(y) or simply V) is synonymous to reach-averaged cross-sectional area and bed area (B(y) or B) is analogous to reach-averaged hydraulic radius but these associations only hold when reach length, L, is considered.
...
Additional conflation \DIFdelbegin \DIFdel{is identified in }\DIFdelend \DIFaddbegin \DIFadd{errors are realized by }\DIFaddend cross-walking the stream network used by the BLE maps and those of HAND.
\DIFaddbegin \DIFadd{The methods that intersect HEC-RAS cross-sections with NWM reaches could introduce errors that violate mass conservation.
...
}\DIFaddend Until a singular stream network is used for the NWM, BLE benchmark, and for HAND based FIM, conflation will continue being a source of error.
\DIFaddbegin \DIFadd{Future work could aim at producing a singular stream network that complies with the Open Geospatial Consortium (OGC) WaterML2.0 HY\_FEATURES data model for standard water feature representation \mbox{%DIFAUXCMD
\cite{simons2014waterml,boston2019water,looser2014international,blodgett2018ogc}}\hskip0pt%DIFAUXCMD
.
}\DIFaddend 
...
The bathymetry of the 10 m DEM from 3DEP is known \DIFdelbegin \DIFdel{to be }\DIFdelend \DIFaddbegin \DIFadd{as }\DIFaddend lacking proper representation thus leading to inadequate representation of volume and bed area with all three methods employed.
...
On a related note, the use of the AGREE DEM method discussed in Section \DIFdelbegin \DIFdel{\ref{ssec:stream_network_enforcment} }\DIFdelend \DIFaddbegin \DIFadd{\ref{ssec:stream_network_enforcement} and \ref{sec:app_stream_network_enforcement} }\DIFaddend also interacts with the bathymetry issue introducing several artificial geometry parameters that affect SRC shape and quality.
\DIFaddbegin \DIFadd{To reiterate in this discussion, Section \ref{ssec:evaluation} pointed out that the BLE's representation of reservoir inundation extends beyond the NWM's defined reservoir boundaries, which results in an under-prediction in the OWP FIM that is reflected in the computed metrics.
}\DIFaddend Due to focus on the nearest drainage problem, we leave future work related to SRC representation including roughness estimation, bathymetric data assimilation \DIFdelbegin \DIFdel{, and bathymetry adjustments as }\DIFdelend \DIFaddbegin \DIFadd{and adjustments as well as reservoir inundation as }\DIFaddend opportunities for major enhancements in HAND based FIM.

Lastly, after errors introduced by conflation, poor roughness estimation, bathymetric/elevation adjustment\DIFaddbegin \DIFadd{, and reservoir representation }\DIFaddend are accounted for, HAND still has another fundamental limitation that is inherently baked into how it works.
\DIFdelbegin \DIFdel{For HAND to be derived and thus }\DIFdelend \DIFaddbegin \DIFadd{To derive HAND and }\DIFaddend create a FIM for a given area, that area must entirely drain to the stream network and the stream network must also drain itself.
...
\begin{figure}[H]
\centering
\DIFdelbeginFL %DIFDELCMD < \includegraphics[scale=1.0]{figures/rating_curve_comparison.jpg}
%DIFDELCMD < \caption{%%%
\DIFdelFL{Illustrates average quantities for the three methods, FR, MS, and GMS, for each stage value (m). 
...
%DIFDELCMD < \label{fig:rating_curve_comparison}
%DIFDELCMD < \end{figure}
%DIFDELCMD < %%%
%DIF < 
%DIF < 
...
%DIFDELCMD < \centering
%DIFDELCMD < %%%
\DIFdelendFL \includegraphics[scale=1.0]{figures/gms_enhancement.jpg}
...
Catchment boundaries and \DIFdelbeginFL \DIFdelFL{stream lines }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{flowpaths }\DIFaddendFL are shown in white and dotted green, respectively.
...
\DIFaddbegin \begin{figure}[H]
    ...
    \caption{\DIFaddFL{Illustrates average quantities for the three methods, FR, MS, and GMS, for each stage value (m). 
    The values are (a) Discharge $m^3s^{-1}$, (b) Volume $m^3$, (c) Bed Area $m^2$, (d) a function of Volume and Bed Area, and (e) number of pixels.
    }}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
\DIFdelbegin \DIFdel{Floods present a significant, under-served, and expanding risk to life, property, and resources.
...
Scalable, open-source software, known as OWP FIM, was developed to produce HAND and associated datasets (catchments, SRCs, and cross-walking data) for the NWM forecasting area including Hawaii and Puerto Rico \mbox{%DIFAUXCMD
\cite{inundationMapping2022}}\hskip0pt%DIFAUXCMD
.
...
%DIFDELCMD < %%%
\DIFdelend The main contribution and conclusion of this work centers around a fundamental limitation in HAND based FIM which is a failure to account for multiple possible sources of fluvial inundation since HAND only considers inundation from the nearest \DIFdelbegin \DIFdel{drainage line}\DIFdelend \DIFaddbegin \DIFadd{flowpath}\DIFaddend .
...
Dissecting the stream network into regions of LPs with unit stream order is necessary because HAND has a ``nearest drainage'' limitation meaning it only accounts for riverine inundation sourced from the nearest \DIFdelbegin \DIFdel{drainage line}\DIFdelend \DIFaddbegin \DIFadd{flowpath}\DIFaddend .
...
While the AGREE DEM procedure is meant to add some bathymetry primarily motivated to enhance catchment and \DIFdelbegin \DIFdel{stream line }\DIFdelend \DIFaddbegin \DIFadd{flowpath }\DIFaddend delineation, it does introduce three parameters that have major implications in the quality of SRCs and the resulting FIMs.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
National Water Model (NWM) data used in this study includes the hydrofabric related datasets \cite{nwm2022hydrofabric} including catchments, \DIFdelbegin \DIFdel{streamlines}\DIFdelend \DIFaddbegin \DIFadd{flowpaths}\DIFaddend , and reservoirs \cite{nwm2022hydrofabric}.
...
Additionally, some FIM hydrofabric data including HAND grids, catchments, \DIFdelbegin \DIFdel{streamlines}\DIFdelend \DIFaddbegin \DIFadd{flowpaths}\DIFaddend , synthetic rating curves, and cross-walk tables are available on the ESIP bucket \cite{esipData2022}.

Software used in preprocessing data, producing FIM hydrofrabic, generating FIM, computing metrics, and conducting analysis is available from a publicly available Github repository \DIFaddbegin \DIFadd{and a HydroShare resource }\DIFaddend called ``inundation-mapping'' from the ``NOAA-OWP'' organization \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{inundationMapping2022}}\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{inundationMapping2022,imHS2023}}\hskip0pt%DIFAUXCMD
}\DIFaddend .
...
We would like to thank some notable contributors of this work including Chief Scientist at OWP, Fred Ogden\DIFaddbegin \DIFadd{, }\DIFaddend for his technical expertise.
Additionally, David Blodgett from the United States \DIFdelbegin \DIFdel{Geolgical }\DIFdelend \DIFaddbegin \DIFadd{Geological }\DIFaddend Survey (USGS) Water Mission Area was instrumental in helping define level paths and other hydrographic work.
More information on code availability, usage, and data retrieval for OWP FIM is available on GitHub \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{inundationMapping2022}}\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{and HydroShare \mbox{%DIFAUXCMD
\cite{inundationMapping2022,imHS2023}}\hskip0pt%DIFAUXCMD
}\DIFaddend .
...
\DIFaddend %% Optional Appendix goes here
%
...
%
%DIF < \appendix
%DIF < \section{Here is a sample appendix}
...
%DIF > %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%DIF > %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{\DIFadd{Stream Network Enforcement}}
\label{sec:app_stream_network_enforcement}
%DIF > %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\DIFadd{The location of the stream network is enforced to ensure general agreement with the NWM network which is used for forecasting the streamflow inputs.
The NHDPlusHR Beta BurnLineEvent layer is used to enforce stream locations in the NHDPlusHR workflow and best agrees with thalweg locations in the DEM used so it is also used here for hydro-enforcement \mbox{%DIFAUXCMD
\cite{moore2019user}}\hskip0pt%DIFAUXCMD
. 
}\DIFaddend 

\DIFaddbegin \DIFadd{However, to better match the drainage density of the NWM FR V2.1 stream network, which is based on the NHDPlus V2, the BurnlineEvents are reduced in density using a nearest neighbor search around the NWM flowpaths.
Headwater points are first derived for the NWM FR V2.1.
For every NWM headwater point, the nearest NHDPlusHR point is selected and placed into a set while those excluded are discarded.
Only the nearest point on the NHDPlusHR is used so any portion of the NHDPlusHR network upstream of this nearest point is discarded to avoid extending inundation too far above the modeling domain.
The points in this nearest neighbor set are then traversed downstream.
...
\caption{\DIFaddFL{This figure illustrates some of the datasets that result from the drainage density reduction of the NHDPlusHR Beta BurnlineEvents (dotted black) to the stream density of the NWM FR V2.1 density (blue).
The stream network used for forecasting, NWM FR V2.1, is of lower stream density than that of the NHDPlusHR which has better agreement with the thalweg locations in the DEM used.
...
\DIFadd{The reduced density stream network is then utilized to hydro-enforce the DEM with a methodology developed by \mbox{%DIFAUXCMD
\citeA{hellweger1997agree} }\hskip0pt%DIFAUXCMD
known as the AGREE DEM Surface Reconditioning System. 
The AGREE algorithm seeks to burn artificially deep thalweg elevations by a uniform value known as sharp drop. 
The modification continues by excavating an area of a given buffer distance from the thalweg by a depth proportional to the distance from the channel given by the smooth drop and buffer distance. 
The resulting enforcement of the thalweg and general bathymetric region results in a cross-section resembling an inverted triangular notch shape with a significantly lower elevation along the thalweg line only.
In total, the AGREE algorithm requires three parameters including the buffer distance, smooth drop, and sharp drop which were set to fixed values of 70 m, 1000 m, and 10 m, respectively, but available to the user via the parameter file.
As for the buffer distance, we conducted analysis varying this parameter from 50 to 90 m in 10 m increments and found that the 70 m value as a good compromise in improvement \mbox{%DIFAUXCMD
\cite{grout2020althydrocond}}\hskip0pt%DIFAUXCMD
.
...
Using the AGREE method as opposed to simple thalweg burning techniques helps prevent distortions in the delineation of streams as well as the catchment boundaries \mbox{%DIFAUXCMD
\cite{saunders1995grid,saunders1996gis,mizgalewicz1996modeling,hellweger1997agree,quenzer1998gis,baker2006comparison,grout2020althydrocond}}\hskip0pt%DIFAUXCMD
.
\mbox{%DIFAUXCMD
\citeA{baker2006comparison} }\hskip0pt%DIFAUXCMD
noted AGREE produced satisfactory results when compared to other enforcement techniques especially when computational costs are considered. 
Downsides to the technique include the possibility of exhibiting parallel streams where the burned stream and real stream are both represented \mbox{%DIFAUXCMD
\cite{hellweger1997agree,saunders1999preparation} }\hskip0pt%DIFAUXCMD
and some distortion of the catchment boundaries can also be observed \mbox{%DIFAUXCMD
\cite{saunders1999preparation,saunders1996gis}}\hskip0pt%DIFAUXCMD
.
Some of these drawbacks are addressed by additional conditioning techniques applied in the methodology.
%DIF > %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
}\section{\DIFadd{Flow Directions and Flats Resolution}}
\label{sec:app_flow_direction_and_flat_resolution}
%DIF > %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%DIF > 
\DIFadd{To facilitate the generation of a connected stream network and its associated catchments from the conditioned DEM, the depression-filled DEM is used to derive connectivity in the form of D-8 flow directions.
D-8 seeks to allocate a drainage direction for every pixel based on the adjacent eight pixel neighborhood with the steepest slope \mbox{%DIFAUXCMD
\cite{o1984extraction}}\hskip0pt%DIFAUXCMD
.
The horizontal component of slope is defined as one for the four neighboring pixels in the main cardinal directions while the intercardinal pixels are designated a horizontal component of $\sqrt{2}$ by means of the Pythagorean theorem. 
Flow directions are derived for non-depression filled regions trivially with the above procedure but to define connectivity for every grid cell the remaining flats corresponding to depression-filled cells must be resolved.
...
\DIFadd{Flat resolution from flats endemic to the DEM or from depression filled regions is a costly, non-trivial procedure which was originally addressed by \mbox{%DIFAUXCMD
\citeA{garbrecht1997assignment} }\hskip0pt%DIFAUXCMD
where flats are resolved by incrementing elevations iteratively.
Software implementations have developed means to partition the problem and resolve flats iteratively with communication across processes \mbox{%DIFAUXCMD
\cite{tarboton2009generalized,tesfa2011extraction,wallis2009parallel,tarboton2005terrain}}\hskip0pt%DIFAUXCMD
.
The excessive iteration and communication leads to poor computational performance which motivated further work on how to optimize flat resolution \mbox{%DIFAUXCMD
\cite{survila2016scalable,barnes2014efficient}}\hskip0pt%DIFAUXCMD
...
\cite{garbrecht1997assignment,tarboton2009generalized,tarboton2005terrain,survila2016scalable,barnes2014efficient,tesfa2011extraction,wallis2009parallel}}\hskip0pt%DIFAUXCMD
.
OWP FIM utilized a CyberGIS implementation of the D-8 flow direction algorithm with the accelerated resolution of flats which we found efficient and effective \mbox{%DIFAUXCMD
\cite{survila2016scalable,cybergis2016}}\hskip0pt%DIFAUXCMD
. 
}\DIFaddend %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
