...
The FIMs associate from each independent LP are then mosaiced together, effectively turning the stream network into discrete groups of homogeneous unit stream order by removing the influence of neighboring tributaries.
Improvement in mapping skill is observed when the extents are compared to HEC-RAS 1D models by significantly reducing false negatives at river junctions.
...
This framework scales to the entire NWN modeling domain including Hawaii and Puerto Rico to help forecasters provide timely and accurate flood warnings.
\end{abstract}
%
...
Previous authors dating back to the first HAND for FIM work by \citeA{nobre2016hand} have noted a sensitivity of mapping skill to stream threshold which is closely related to stream density and the maximum Horton-Strahler stream order (or simply stream order) of the processing unit employed \cite{zhang2018comparative,mcgehee2016modified,li2020evaluation}.
...
These results are presented in Figure \ref{fig:violin_plot} as violin plots and in Table \ref{tab:aggregate_metrics} as aggregated metrics with the results for MS and GMS presented as percentage changes from their respective FR values.To be more specific, Table \ref{tab:aggregate_metrics} sums the primary metrics, TP, FP, FN, and TN, across all HUC8's then recomputes the secondary metrics which was done to better account for large variances in HUC8 size.
...
This sub-figure shows the CSI distributions across all 49 HUC8's when Manning's n is set to 0.06.
Each independent, whole violin represents the distribution of FR, MS, or GMS while each half of the violin represents the distribution of that data divided up by magnitude (blue for 100yr and orange for 500yr).
...
To avoid having two trend lines per sub-figure, we elected to aggregate the two magnitudes togther as they tend to observe similar trends across the three models.
...
So overall and in other words, the entire distribution of HUC8's improves across the three methods.
Due to the means by which FIM is produced utilizing FR, MS, and GMS, we can say that the more we derive HAND on networks of unit drainage density and mosaic the resulting FIMs, the better those FIMs perform.
We move more details on the relationship betweeen stream order and FIM skill to the Discussion section (Section \ref{sec:discussion}).

Additional noteworthy trends in Figure \ref{fig:violin_plot} center around the all-around better performance of FIM's for those of higher Manning's n values and recurrence flows.
...
Most of this improvement is driven by significant increases in POD but, unfortunately, leading to a significant amount of overprediction as observed by the increase in FAR.
More work can be invested to better regionalize Manning's n values for FIM purposes with HAND.
We also observe additional trends associated with the magnitude or recurrence interval of the flow rates used with the higher flow rates exhibting better overall CSI, POD, and FAR values than the lower, 100 yr magnitude.
...
We believe, as we later argue for, that the increase in POD is primarily driven by an increase in the catchment sizes that is inhertently enabled by dividing up the stream network into independent stream networks of unit stream order.
Additionally, we note that reducing drainage order also has some minor influence on reducing inundation extents and the rate of false alarms.
We believe that this effect is driven by a reduction in some inundation areas led by a change in the stage-discharge relationship where a given streamflow now leads to lower river stage values when HAND is computed with target stream networks of unit drainage order.
We seek to explain that these two effects, catchment boundary enlargment and stage-discharge curve lowering, are highly interrelated and cannot be easily detangled.
...
As evident in the results of the study in Section \ref{sec:results}, a sizeable amount of the increase in CSI observed by reducing stream order for HAND computation can be attributed to increases in POD.
...
The water built up along the catchment boundaries can be thought of as a column of water in a cyclindrical container (catchment) that has exceeded the elevation of the containers rim.
...
Figure \ref{fig:gms_enhancement} best illustrates this behaviour in real context.
...
Sub-figure \ref{fig:gms_enhancement}a demonstrates the agreement raster for the FR stream network as well as it's respective catchment boundary lines symbolized in white and stream network shown in green.
Inspection of this sub-figure denotes a clear spatial pattern where TP's or areas correctly inundated are pooled alongside catchment boundary lines. 
On the other side of the catchment boundary, one can witness large swaths of FN's that should be inundated. 
The FN's also exhibit a spatial pattern as in they tend to collocate within catchments of the pictured mainstems tributaries.
This sort of behaviour was introduced early in the paper and shown qualitatively in Figure \ref{fig:catchment_boundaries_issue}.
...
Showing all the catchments for the tributaries that were all derived independently would convulate the image.
...
The TP's are no longer bounded by the catchment lines and allowed the expand to their natural extents.

We note here as a contribution of this study that a major inherant, limitation of HAND is the ``nearest drainage'' constraint or the idea that a given river reach only drains or, in HAND's case, inundates its immediate, unique drainage area or catchment.
In other words, HAND based FIM's are limited to producing fluvial inundation to only their nearest drainage area or catchment.
...
Generally speaking, drainage areas are known to be hiearchical in nature so a given drainage area for a given outlet point can be decomposed into nested drainage areas for outlet points that lie in the original drainage area.
...
Hence it's important to state that just like there are different sources of flooding such as fluvial, pluvial, groundwater, reservoir, and coastal, there can also be multiple sources of a riverine flood.
HAND is only equipped to handle rivirine flooding from the nearest drainage line.
...
The remaining portion of the improvement in CSI was found to come from a marginal yet noteable reduction in FAR.
Upon investigation of the spatial results in the agreement maps, we found some areas of slight reductions in FPs especially where changes in catchment boundaries may have occurred due to the reduction in stream order.
These observations pointed to changes in the rating curves introduced by stream order reduction and catchment definition adjustments.
...
The NWM stream network was derived using the NHDPlus MR  dataset which was derived from coarser DEMs than those used here. 
...
We suspect that a significant amount of the difference in performance between 100yr and 500yr magnitude events can be attributed to poor synthetic rating curve performance due to poor bathymetric representation.
...
Higher flow events tend to cover regions
...
\caption{OWP FIM inundation agreement, TP, FP, FN, and TN, with BLE HEC-RAS maps in HUC 11140105 at the 500yr recurrence magnitude.
...
Dissecting the stream network into regions of level-paths with unit stream order is necessary because HAND has a ``nearest drainage'' limitation meaning it only accounts for riverine inundation sourced from the nearest drainage line.
While this is all that is required in many, lower flow instances, typically larger flow scenerios can call for multi-source riverine flooding to occur especially at the junction of major rivers with their smaller tributaries.
Computing HAND independently for each level-path removes the boundary of neighboring catchments thus allowing for 
...
All data availability and open research information is cited in the Software and Datasets sections, \ref{ssec:datasets} and \ref{ssec:software}, respectively.
Please see those sections, Table \ref{tab:datasets}, and their References for more information.
