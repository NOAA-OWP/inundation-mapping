...
%
\begin{keypoints}
\item Flood maps derived from Height Above Nearest Drainage (HAND) are subject to nearest flowpath limitations that affect inundation skill.
...
Some work by \citeA{gerapetritis2004behavior} denotes CSI a good proxy for measuring a forecasting system's utility in protecting life and property and has been shown as mathematically optimized when $POD = 1 - FAR$.
...
This intuition is confirmed by previously mentioned work by \citeA{gerapetritis2004behavior} where CSI was shown as a direct function of both POD and FAR where CSI is maximized when $POD = 1 - FAR$.
...
In sub-figure \ref{fig:gms_enhancement}b, the agreement raster for the GMS technique is illustrated as well as the flowpaths in green.
...
The consequence for inundation extents is a general increase in spatial coverage of the river's water which shows better agreement with the benchmark maps.
...
Generally speaking, drainage areas are hierarchical in nature so a given drainage area of an outlet point can be decomposed into nested drainage areas for outlet points that lie in the original drainage area.
...
HAND is only equipped to handle riverine flooding from the nearest flowpaths.
Other relevant flowpaths that produce fluvial flood waters are not considered here especially if the routing model used doesn't consider backwater effects.
...
While the channel slope and reach lengths are not exactly the same across methods, their averaged differences were statistically insignificant which only leaves room for deviations in volume and bed area.
...
Future work could aim at producing a singular stream network that complies with the Open Geospatial Consortium (OGC) WaterML2.0 HY\_FEATURES data model for standard water feature representation \cite{simons2014waterml,boston2019water,looser2014international,blodgett2018ogc}.
...
The bathymetry of the 10 m DEM from 3DEP is known as lacking proper representation thus leading to inadequate representation of volume and bed area with all three methods employed.
...
To derive HAND and create a FIM for a given area, that area must entirely drain to the stream network and the stream network must also drain itself.
...
Catchment boundaries and flowpaths are shown in white and dotted green, respectively.
