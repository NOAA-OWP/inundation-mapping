...
if [ -f "$hucList" ]; then
    if [ "$jobLimit" -eq 1 ]; then
        parallel --verbose --lb  -j $jobLimit --joblog $logFile -- $libDir/time_and_tee_run_by_unit_MS.sh :::: $hucList
    else
        parallel --eta -j $jobLimit --joblog $logFile -- $libDir/time_and_tee_run_by_unit_MS.sh :::: $hucList
    fi
else
    if [ "$jobLimit" -eq 1 ]; then
        parallel --verbose --lb -j $jobLimit --joblog $logFile -- $libDir/time_and_tee_run_by_unit_MS.sh ::: $hucList
